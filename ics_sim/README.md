...
**`>NODE 메시지`** 는 두 갈래다 (2026-09-07). 목적지가 **우리가 받는 노드**면
(`ICS`·`K.IC`·`K.CB` …) 종전대로 프로세스 안에서 처리하고 — `>K.IC status` —
**남의 노드면 와이어로 내보낸다** — `>XIS HOSTS` → `ICS>XIS HOSTS`. `xis_host` 가
