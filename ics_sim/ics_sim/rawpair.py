...
#: 부호에 주의 -- 경계가 UT 오후인 곳(CTIO·KASI)만 양수다.
OBSDATE_SHIFT_MIN = {
    'KMTC': +7 * 60 + 30,     # 경계 UT 16:30  (현지 12:30, UT-4)
    'KMTS': -(10 * 60 + 30),  # 경계 UT 10:30  (현지 12:30, UT+2)
    'KMTA': -(1 * 60 + 30),   # 경계 UT 01:30  (현지 12:30, UT+11)
    'KMTK': +9 * 60,          # 경계 UT 15:00  (KST 00:00 -- 곧 KST 날짜다)
}

# ⭐ **KASI 만 규칙이 다르다** (운영자 2026-09-12).  관측 3사이트는 경계가
# **현지 12:30**(동지 관측 종료~시작의 중간)인데, KASI 는 관측 야간이 없는
# 실험실이라 그 근거가 성립하지 않는다.  대신 **KST 자정**을 쓴다 -- 보정 +9h
# 를 더한 뒤 날짜만 취하면 그것이 곧 **한국 날짜**라, 벤치에서 사람이 보는
# 날짜와 파일 이름이 같아진다.
# ⚠️ **2026-09-12 까지 KASI 는 보정 0**(UT 날짜 그대로)이었다.  그래서 그 전에
# 찍은 KASI 자료·로그의 `<YYYYMMDD>` 는 UT 날짜다 -- **KST 09:00 이전에 찍은
# 것은 하루 앞선 이름**을 갖는다.
