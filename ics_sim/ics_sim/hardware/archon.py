...
이미 저장소에 있는 자산:
  * `ics_archon/archon_kmtnet_labtest_v1.0.bigbuf.py`
        Archon 텍스트/바이너리 프로토콜로 노출·FETCH·raw spec v1.3 헤더까지
        하는 실동작 스크립트 (science = bigbuf 구성.  guide 는 smallbuf 판
        참조).  명령 시퀀스(POWERON, WCONFIG/APPLYALL, LOADPARAMS,
        STATUS/FRAME 폴링, 1 KiB 블록 FETCH)를 그대로 옮겨오면 된다.
        전신은 `ics_archon/__ref_archon_control/`(읽기 전용) · Archon 매뉴얼
        2부도 같은 폴더에 있다.
