...
⭐ 규격의 출처 어휘는 **`REDIS (dome control)`** 이고 규약은 **5.7.3절**이다 (v1.13 ·
D-021).  ⛔ 구 어휘 `TCS relay or REDIS` 는 규격이 걷었다 -- 한 카드의 출처를 두 갈래로
열어 두면 헤더만 보고 어느 쪽 값인지 가릴 수 없어서다.  ⭐ **운영자 확정 2026-09-11**:
돔 제어 프로그램이 redis 에 실어 두는 셋을 ICS/ICG 가 직접 읽는다.
