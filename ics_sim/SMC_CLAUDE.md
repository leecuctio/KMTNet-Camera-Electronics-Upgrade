...
- ✅ **`CCDTEMP` 카드 comment 의 `M` 제거는 이것과 별개**였고 **2026-08-30 운영자 지시로 이행 완료**됐다 (v1.9 발행과 함께 견본 3장 제자리 개정, 브랜치 기계 사본 3곳은 머지 `bed2f20` 에서 동반 수정).
- **시험**: `ics_sim` **329** · `ics_archon` **514** — 둘 다 전수 통과 (2026-09-06 실측).  ⛔ 종전 이 줄의 *"두 스위트를 동시에 돌리지 말 것 -- 부하로 간헐 실패한다"* 는 **오진이었다.**  그 시험(`test_shutdown_waits_for_frames…`)은 단독으로도
  **3/3 결정적으로 실패**했고, `git bisect` 가 `036976a`("ics_sim 공유층을 연다")를
  첫 bad 로 짚었다 -- 프레임별 발신이 저장 태스크 등록보다 앞서서 `drain_writers()`
  가 빈 대기열을 보고 **다 저장됐다고 오해**하던 것이다.  ⭐ 2026-09-06 에
  `_store_settled` 로 고쳤다 (10/10 통과).
...
- **raw ↔ MEF 키워드 대응표 (2026-08-13, 검토 대기)** — 289행 전수. MEF 쪽은 문서가 아니라 **converter 코드에서 기계 추출**했다(코드가 최종본이므로). **Archon setup·구성·유닛 텔레메트리 카드의 이름과 구성**이 핵심이다 — 실기가 이 이름으로 자료를 쌓은 뒤 이름이 바뀌면 그때까지의 아카이브가 영구히 안 읽힌다. 정본은 판정 원장 [`../raw_fits_spec/KMT_CEU_Raw_FITS_Header_and_Refs_in_MEF_Converter_v1.16.md`](../raw_fits_spec/KMT_CEU_Raw_FITS_Header_and_Refs_in_MEF_Converter_v1.16.md) (대응 관계 = `Use in MEF` 열, 판정 준거 = 0장), 미결 4건은 [`../raw_fits_spec/KMT_CEU_Raw_Rev_MEF_Impacts_and_Identity_v0.8.md`](../raw_fits_spec/KMT_CEU_Raw_Rev_MEF_Impacts_and_Identity_v0.8.md) §6, **ACT-011**. ⚠️ **`ics_sim` 의 현재 출력은 raw 쪽 사실의 근거가 아니다**(순환) — 원장 0장 참조. 경위는 DevNote **11.17**
