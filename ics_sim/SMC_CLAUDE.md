...
6. **raw 헤더를 확정 초안 v0.3.5 에 동기화** — HK 블록은 완료(2026-08-21: `CCDTEMP` 실측 대표 전환 · `DEWPRES` 문자열 sentinel `9.99e-9` · 신설 `DMPTEMP`/`WALLBRD`/`HEBOX`, DevNote 11.18). 남은 것: 노출·컨트롤러 블록 재편(`DARKTIME`/`TSHOPEN`/`TSHSHUT`/`NPHLINES`/`HEMODE`/`READMODE`/`CTR_CFG` 제거 · `CTRL1CFG`/`CTRL2CFG`/`TCSTIME` 신설 · `DATASRC` 값 체계 `ARCHON_SCIENCE`/`ARCHON_GUIDE`/`SIM` · `ICSBUILD` 형식 확인) + **카드 comment 지원**(`fitsout._apply_header()`). 근거: [`../raw_fits_spec/KMT_CEU_Raw_FITS_Header_and_Refs_in_MEF_Converter_v1.8.md`](../raw_fits_spec/KMT_CEU_Raw_FITS_Header_and_Refs_in_MEF_Converter_v1.8.md) — **확인 요망 9건이 그 머리말에 있다.**
