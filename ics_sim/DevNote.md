...
운영자가 레거시 raw/MEF 헤더 실물을 `../raw_fits_spec/__reference/Legacy raw fits header samples/` 에 넣어 줬다 — raw 2건(2017 SSO · 2021 CTIO)과 MEF 37건. **raw 헤더는 4년간 사실상 불변**(`INPUTFMT` 하나 차이)이어서 정착된 설계로 읽을 수 있다.
...
**아직 안 한 것**: 픽셀은 더미이고 크기도 실물(19200×9400, 파일당 344 MiB)이 아니다. `BITPIX=16`+`BZERO=32768`(규격 3장)과 4장 픽셀 배치도 실기 몫이다 — **구조와 규약만 맞췄다.** chip 2개를 X 방향으로 이어 붙이는 것까지는 흉내 낸다(`sim._join_x`).
...
| **파일명 날짜 기준 확정 (raw_fits_spec OI-10)** | `state.stamp_compact()` 은 **UTC 날짜**를 쓴다. 규격 2.3절의 종전 서술 "관측 야간 기준 날짜" 는 레거시·구현 어느 쪽과도 맞지 않아 UTC 로 정정했으나 **잠정이다.** 차이가 실재하는 곳은 **SAAO**(UTC+2) — 한 야간이 UTC 자정을 넘어 날짜가 둘로 갈린다. **이충욱과 협의해 확정**하고, 야간 기준으로 가면 야간 경계 정의와 함께 이 함수·규격 2.3·9장을 같이 고친다 | 협의 |
