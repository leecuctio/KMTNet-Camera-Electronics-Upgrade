...
| **fail-safe 이름에 CCD/컨트롤러 식별이 없다** | 파일명 fail-safe 의 대체 이름 `<yymmdd>.<nnn>.fits` 에는 검출기 식별이 없다 — 벤치에서 4개 CCD 가 `260811.000`~`003` 으로 흩어져 어느 것이 어느 검출기인지 알 수 없었다(11.12 말미). 레거시는 최후수단이라 감수했지만 raw pair 는 컨트롤러당 1파일이므로 **`MK`/`NT` 태그를 유지하는 규약**이 필요하다. `raw_fits_spec` open item 에 없는 항목 | ics_archon |
