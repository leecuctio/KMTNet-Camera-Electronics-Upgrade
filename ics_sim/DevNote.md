...
> ⭐ **이 노트는 `ics_sim` 몫이다** (운영자 지시 2026-08-30).  **실기
> (`ics_archon`) 관련 사항은 [`../ics_archon/DevNote.md`](../ics_archon/DevNote.md)**
> 에 있다 — Archon 제어층·실기 운용(프로토콜·ACF·전원·텔레메트리·버퍼·실측).
> ⚠️ 걸치는 작업은 **"어느 프로그램의 일인가"** 로 가른다: `ics_archon` 작업이
> 이 폴더의 파일을 고쳤어도 그것은 저쪽에 쓰고 여기엔 한 줄 + 링크만 둔다.
> ⚠️ **번호 체계가 다르다** — 저쪽은 **1장부터**, 여기는 `11.x` 다.  2026-08-26
> 이후의 실기 이력(구 11.31~11.37)은 저쪽 1~7장이고, 그 전(11.22~11.30)은
> 여기 남아 있다 (11장 끝 포인터 절 참조).
...
### ⏭️ 11.31 이후 — **`ics_archon` 노트로 갈랐다** (2026-08-30)

`ics_archon` 작업 이력이 11장의 절반을 넘어가면서 **노트를 둘로 갈랐다**
(운영자 지시).  **2026-08-26 이후의 실기 작업분(구 11.31~11.37)은
[`../ics_archon/DevNote.md`](../ics_archon/DevNote.md) 에 있다:**

| `ics_archon/DevNote.md` | 무엇 | 구 번호 |
|---|---|---|
| **1장** | v1.6 준수 재감사 — 검증 구조의 구멍과 낱말 세 뜻 | ~~11.31~~ |
| **2장** | 가이드 TVM 스크립트 판독 — 텔레메트리 감시 설계 + 부수 결함 5건 | ~~11.32~~ |
| **3장** | 층 1·2 감시·기록 구현 + labtest v1.3.4 이식 | ~~11.33~~ |
| **4장** | 참고 자료 전수 재검토 — 계보가 셋이라는 것 | ~~11.34~~ |
| **5장** | `CTRLnCFG` 를 ACF 경로에서 파생 — 별개 키 둘을 하나로 | ~~11.35~~ |
| **6장** | 세 저장소 전수 검토 — 판올림이 문서를 앞질러 간 자리들 | ~~11.36~~ |
| **7장** | 실기 실측이 닫은 것 넷 — 추정으로 열려 있던 자리들 | ~~11.37~~ |

⚠️ **11.22~11.30 은 여기 그대로 둔다.**  그 절들을 가리키는 인용이 저장소
곳곳(규격·PM 문서 포함)에 있어 번호를 바꾸면 참조가 통째로 깨진다.

**경계 규칙** (운영자 지시 2026-08-30): **`ics_sim` 관련만 여기**, `ics_archon`
관련은 저쪽이다.  ⭐ **걸치는 작업은 "어느 프로그램의 일인가" 로 가른다** — 파일
위치가 아니다.  `ics_archon` 작업이 이 폴더의 `ics_sim.ini`·`rawcards.py` 를
고쳤어도 그것은 `ics_archon` 작업이라 저쪽에 쓰고, 여기에는 **한 줄 + 링크**만
둔다.  ⚠️ **번호 체계가 다르다** — 저쪽은 1장부터, 여기는 `11.x` 다.
