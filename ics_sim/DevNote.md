...
그대로 먹혔다 -- `DETID` 필드가 없어져 오히려 pair 양쪽이 같은 키를 준다.
...
**`'None'` 이 자리에 실렸다.**  `parse.field_value()` 가 이미 sentinel 로 채워
...
### 11.31 v1.6 준수 재감사 -- 검증 구조의 구멍과 낱말 세 뜻 (2026-08-26)
...
### 11.32 가이드 TVM 스크립트 판독 -- 텔레메트리 감시 설계 + 부수 결함 5건 (2026-08-27)
...
### 11.33 층 1·2 감시·기록 구현 + labtest v1.3.4 이식 (2026-08-28)

11.32 가 설계하고 넘긴 **작업 A** 를 구현했다.  결과·규칙은
...
붙이지 않는다.**  설정으로 피하는 것이 아니라 본편을 내리고 쓴다.  이것은 11.32
...
11.32 의 "매뉴얼 필드 목록을 우리 구성으로 착각하지 말 것" 과 같은 계열이다 —
...
그래서 11.32 가 적은 "`ics_archon` 171 통과" 는 그때 이미 **169 통과 · 2 실패**
...
다툰다.  ⚠️ `ics_sim` 스위트와 **동시에 돌리지 말 것**(11.32 와 같다).

### 11.34 참고 자료 전수 재검토 -- 계보가 셋이라는 것, 그리고 아는 것을 안 쓰고 있었던 자리 (2026-08-28)
...
사람이 같은 폴더를 열고 같은 것을 "빠졌다" 로 읽는다 -- 11.32 가 층 3 해독
...
스위트와 동시에 돌리지 말 것(11.32·11.33 과 같다).


### 11.35 `CTRLnCFG` 를 ACF 경로에서 파생 -- 별개 키 둘을 하나로 (2026-08-29)
...
### 11.36 세 저장소 전수 검토 -- 판올림이 문서를 앞질러 간 자리들 (2026-08-29)
...
### 11.37 실기 실측이 닫은 것 넷 -- 추정으로 열려 있던 자리들 (2026-08-29)
...
2개, **30초로 넓히면 3개**가 필요하다.  운영자 결정은 **기본 2 + ini 조절**이고,
`[archon] wrote_window` 를 함께 선언해 **기동에서 짝을 검사**한다.
