...
### ⏭️ 11.35 이후 — **`ics_archon` 노트로 갈랐다** (2026-08-30)

`ics_archon` 작업 이력이 11장의 절반을 넘어가면서 **노트를 둘로 갈랐다**
(운영자 지시).  **2026-08-29 작업분(구 11.35~11.37)부터는
[`../ics_archon/DevNote.md`](../ics_archon/DevNote.md) 에 있다:**

| `ics_archon/DevNote.md` | 무엇 | 구 번호 |
|---|---|---|
| **1장** | `CTRLnCFG` 를 ACF 경로에서 파생 — 별개 키 둘을 하나로 | ~~11.35~~ |
| **2장** | 세 저장소 전수 검토 — 판올림이 문서를 앞질러 간 자리들 | ~~11.36~~ |
| **3장** | 실기 실측이 닫은 것 넷 — 추정으로 열려 있던 자리들 | ~~11.37~~ |

⚠️ **11.22~11.34 는 여기 그대로 둔다.**  그 절들을 가리키는 인용이 저장소
곳곳(규격·PM 문서 포함)에 있어 번호를 바꾸면 참조가 통째로 깨진다.  갈린
경계는 **날짜**다 — 2026-08-29 작업분까지가 저쪽 1~3장이고, 그 이전은 여기다.

**경계 규칙**: `ics_sim` 패키지(시퀀서·헤더 층·메시지 규약)는 여기,
Archon 제어층·실기 운용은 저쪽.  두 쪽에 걸치면 **한쪽에 쓰고 다른 쪽에서
가리킨다.**
