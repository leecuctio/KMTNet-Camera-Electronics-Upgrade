...
- [x] **`ics_sim` 연동 — 완료 (2026-08-11).** 마지막 미실증 항목이던 "9개 ID 가 한 주소를 공유해도 XIS 가 받아주는가" 가 닫혔다 — `HOSTS` 가 `NumClients=9`, 9개가 **각자 슬롯**(`Host0`~`Host8`)을 차지한 채 전부 `127.0.0.1:6600`. 이어서 재등록·라우팅·실물 TCSAgent/OBSAgent 연동·노출 사이클까지 전 구간 통과했다. 실패 신호는 `ERROR: No Route to Destination Host K.IC`(부록 A (10)).
  **결과 전체와 벤치 실행법은 [`../DevNote.md`](../DevNote.md) 3.7 · [`../README.md`](../README.md).**
...
| 날짜 | 내용 |
|---|---|
| 2026-08-11 | **`ics_sim` 연동 시험 완료.** 8절의 마지막 미실증 항목이 닫혔다 — 9개 노드 ID 가 한 주소를 공유해도 XIS 가 각자 슬롯으로 받아 준다(`NumClients=9`). 자기 발신 에코가 **36개**(삼각수)로 돌아온 것이 6.3절의 "운영본은 송신 슬롯 하나만 제외한다"(v2.9.1)를 **동작으로** 재확인했다 — 은퇴 분기의 포트 비교 패치였다면 0개였을 것이다. 부수로 XIS 가 **빈 노드 ID 도 등록**한다는 것이 드러났다(`updateHosts()` 에 ID 검증 없음, 6.1절 ② 의 실물 확인). 실물 TCSAgent·OBSAgent 까지 같은 허브에 물려 노출 사이클 전 구간을 돌렸고 신규 `ics` 결함 하나를 잡았다([`../DevNote.md`](../DevNote.md) 3.7·12.14) |
