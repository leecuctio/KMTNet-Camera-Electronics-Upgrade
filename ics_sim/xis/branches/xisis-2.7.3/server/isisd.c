...
main(int argc, char *argv[]) 
{
  ...
  logMessage("isisd daemon session start");
  ...
  if (isis.numSerial > 0) {
    if (initSerialPorts() < isis.numSerial) {
      logMessage("Error initializing serial port - aborting");
      exit(EXIT_FAILURE);
    }
  }
  ...
  if (openSocket(isis.sockPort) < 0) {
    logMessage("Error: server socket startup failure - aborting");
    exit(EXIT_FAILURE);
  }
  ...
  while (isis.keepGoing) {
    ...
    if (isis.doStartup != NO_STARTUP) {
      ...
      case COLD_START:
	logMessage("Server cold start initiated");
	if (isis.doHandShaking) handShake();
	logMessage("Server cold start done");
	break;

      case WARM_START:
	logMessage("Server warm start initiated");
	sprintf(logStr,"Reloading initialization file %s",isis.iniFile);
 	logMessage(logStr);
	loadConfig(isis.iniFile);
	logMessage("Pinging clients");
	broadcastMessage(ISIS_SERVER,"PING");
	logMessage("Warm restart completed");
	break;
    ...
    if (numReady < 0) { 
      ...
      logMessage("WARNING: server select() timeout");
      continue;
    }
    else {
  ...
  logMessage("Server shutdown normally");
