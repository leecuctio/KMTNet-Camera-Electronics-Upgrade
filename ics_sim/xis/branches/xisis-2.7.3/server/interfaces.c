...
serialHandler(int serialFD) 
{
  ...
  // Quick sanity check: if srcID (sender) is the same as *our*
  // ID, something is fishy.  Either (a) we are somehow in a state
  // where we are talking to ourselves or (b) someone out there is
  // using our ID incorrectly.  In either case, as protection, drop
  // the message on the floor but log it and exit. [rwp/osu - 2014 Sep 30]

  if (strcasecmp(srcID,isis.serverID)==0) {
    sprintf(reply,
	    "WARNING: Anomalous tty host srcID=%s same as serverID=%s)",
	    srcID,isis.serverID);
    if (isis.useCLI) printf("\n%s\n",reply);
    logMessage(reply);
    free(reply);
...
socketHandler(int sockFD) 
{
  ...
  if (strcasecmp(srcID,isis.serverID)==0) {
    sprintf(reply,
	    "WARNING: Anomalous socket host srcID=%s same as serverID=%s)",
	    srcID,isis.serverID);
    if (isis.useCLI) printf("\n%s\n",reply);
    logMessage(reply);
    free(reply);
  ...
  // We are good so far, update the host table with the srcID information 
