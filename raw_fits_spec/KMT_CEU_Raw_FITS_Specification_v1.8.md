...
> **제자리 정정 (2026-08-29) — 2.2절 `<SITE>` 의 사이트 판별 문장.** *"실효 사이트는 호스트 IP 로 판정하며 판정이 설정을 이긴다 (D-015)"* 가 남아 있었다. **D-015 는 D-020 이 대체했고**(2026-08-24 운영자 확정, `ics_sim/DevNote.md` 11.27) 사이트 판별은 **`[node] observatory` 한 줄**이 정본이다 — `siteid.py` 와 `site_from_ip` 는 삭제됐다. 같은 문장의 *"넷 밖의 값은 `KMTK` 로 정규화"* 도 현행과 다르다: **모르는 값은 기동을 거부한다**(`rawpair.site_of_observatory()` 가 `ValueError`). **대체 결정(D-020)이 v1.8 발행보다 앞서므로** 판을 올리지 않고 제자리에서 고쳤다 — **규범 내용의 변경이 아니라 이미 내려진 결정을 뒤늦게 반영한 것**이다. 머리말 결정 기록 표의 `D-015` 표기도 함께 고쳤다.
