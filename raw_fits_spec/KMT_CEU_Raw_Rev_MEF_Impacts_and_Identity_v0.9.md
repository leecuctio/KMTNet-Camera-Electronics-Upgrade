...
**v0.9 (Draft)** · 2026-09-04 · **HK 카드 5장 신설 + 게이지 Off + 반쪽 pair 반영** (raw spec v1.10 동반) — C-항목 **3건 신설**: HK 히터 넷·`HKUDATE`(converter 미독 + `'NC'` sentinel 어휘 확장) · `DEWPRES` 게이지 Off 구간(의도된 sentinel · Conductron 함정) · **반쪽 pair**(짝 탐색이 오류로 세면 안 된다) · 구판 v0.8 = 2026-08-30 · 구판 v0.7 = 2026-08-29 · **Part 1** = 헤더 카드 개정이 MEF 쪽 3자(ICD v4.1 · MEF keyword 정의서 v1.0 · converter v2.2.0)에 요구하는 개정 목록 · **Part 2** = raw 파일 번호 · 정체성 · 충돌 처리 재설계와 그 MEF 파급
...
| **C-재개: `OBSERVAT`·`<SITE>` 사이트 코드 (D-017, 2026-08-25)** | v0.2 에 등재됐다가 2026-08-21 에 **철회·종결**됐던 항목이 **되살아났다.** 운영자가 `TESTBED`/`KMTT` 를 폐지하고 **`KASI`/`KMTK`** 로 확정했다 — `OBSERVAT` ∈ {`CTIO`,`SSO`,`SAAO`,`KASI`}, 파일명 접두어 ∈ {`KMTC`,`KMTA`,`KMTS`,`KMTK`}. `ics_sim` 반영 완료(`rawpair.OBSERVAT`·`ORIGIN_OF`·`KASI_SITE`·보정표·`config._SITE_TELID`·~~`siteid.BENCH_SITE`~~ — ⚠️ **`siteid.py` 는 그 뒤 삭제됐다**(D-015 IP 판정 폐기, 2026-08-24. 사이트 판별은 `[node] observatory` 한 줄이다)). | ✅ **converter 반영 완료 (2026-09-04, v2.4.0 · LEECU)** — 파일명 정규식 넷째 대안 `KMTK`, `SITE_PREFIX`/`OBS_PREFIX` `KMTK`/`KASI`, L0 MEF prefix `kmtk`. **ICD 도 v4.2 로 판올림**(§2.1 표·개정 이력, 구 v4.1 은 `archive/`). ⚠️ 이 표시는 원래 **v0.8 에 제자리로** 달렸는데 (커밋 `f4c7e5e`) v0.9 판올림이 v0.8 을 `archive/` 로 보내 고아가 된 것을 옮겨 온 것이다 |
...
| **C-신설: HK 히터 넷 · `HKUDATE`** (2026-09-04, raw spec v1.10) | raw HK 블록에 **5장이 늘었다** — `HKUDATE`(블록 취득 시각, 19자) 와 듀어 히터 넷 `HTREN`·`HTRSET`·`HTROUT`·`HTRFORCE`. 출처 계통 **`ICG heater`** 신설로 HK 공급 계통이 둘 → 셋이 됐다. 값은 층이 갈린다 — `RCONFIG` **되읽기**(`HTREN`·`HTRSET`·`HTRFORCE`, *명령이 아니라 컨트롤러가 받아들인 값*)와 `STATUS` **실측**(`HTROUT`). ⚠️ `HTROUT`(0~25 V)는 guide 헤더의 `HEATER` 레일(+28 V)과 **다른 것**이다 | ① converter 는 5장을 **읽지 않는다** — `DMPTEMP`/`WALLBRD`/`HEBOX` 와 같은 처지(위 HK 온도 항목 ①)라 **MEF 로 보낼지 LEECU 판단**이 필요하다 ② ⚠️ **sentinel 어휘가 늘었다** — HK **상태 낱말**(`HTREN`·`HTRFORCE`)과 HK **전압**(`HTROUT`)의 측정불가는 **`'NC'`** 다(온도의 `'-999.99'` · `DEWPRES` 의 `'9.99e-9'` 와 별개). 하류 파서가 `'NC'` 를 모르면 숫자 변환에서 깨진다 ③ ⏳ `HTROUT` 의 원천이 `ics_archon` 에 아직 없어 **구현 전에는 항상 `'NC'`** 다 |
| **C-신설: `DEWPRES` 게이지 Off 구간** (2026-09-04, raw spec v1.10) | 진공 이온게이지 필라멘트가 science 영상에 영향을 주므로 **science 노출 중에는 게이지를 끈다**(ICS 가 노출 앞에 `VACGAUGE OFF`, 독출 완료 10분 뒤 `ON`). 그 구간의 `DEWPRES` 는 **`'9.99e-9'`** 이고 이는 **결측이 아니라 의도된 상태**다 | ⚠️ **하류가 이 값을 고장으로 세면 안 된다** — 정상 관측분의 상당수가 그렇게 나온다. ⛔ **Conductron 함정**: 이온게이지를 꺼도 같은 모듈의 열손실 센서가 값을 계속 내보내고 그 값이 인정 범위 `[1e-8, 1e+3]` 를 **경고 없이 통과한다** — 끈 동안 오는 값을 실으면 *"정상으로 보이는 틀린 값"* 이 남는다. guide FITS 도 같은 게이지라 그 구간에는 **양쪽이 동시에 sentinel** 이다 |
