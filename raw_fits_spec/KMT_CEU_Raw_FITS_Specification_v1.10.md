...
| OI-25 | `HTROUT` 원천 미구현 | 히터 **실측 출력**(`STATUS` 층)을 읽는 코드가 `ics_archon` 에 **아직 0줄**이다 — 구현 전까지 이 카드는 항상 sentinel `'NC'` 로 실린다(5.6.2절). 구현되면 값 범위(0~25 V)·갱신 주기 확인 |
...
1. **`EXPTIME` = 연속한 두 프레임 독출 개시 시각의 간격** [seconds]. 취득 SW 가 프레임 주기를 이 값으로 만든다 (형 규칙은 5.4절과 동일 — 정수형 기본).
   ⛔ **하한이 있다 (v1.10)** — 독출 자체가 store 1033행을 훑는 시간보다 짧은 간격은 만들 수 없다. 현행 ACF `R2610` 계산으로 **≈ 1.25 s**(`Pixels=540` 기준 디지타이징 1.2439 s + 트랜스퍼). 그보다 작은 값을 지시하면 실제 주기는 하한에 붙어 **헤더의 `EXPTIME` 과 실제 간격이 어긋나므로**, 취득 SW 는 하한 미만 지시를 **거절한다**(`icg_archon`, `acftiming.frame_timing()['floor']`). ⚠️ 하한 값은 **ACF 판에 매인다**(구 `R2609` 는 ≈ 1.37 s) — `CTRL1CFG` 와 함께 읽어야 한다.
