...
* ⛔ **`ENS1`–`ENS7` 은 예외다** — 5.8절이 *"중계 그대로"* 로 규정한 값이라
  ...
  규약을 적용하지 않는다.
* ⏳ **`Cn_TEMP` 는 이번 판에서 제외한다** — 파이프 나열이라 자리마다 한 자씩
  늘면 인용 필드 폭(science 51 · guide 49)을 넘겨 comment 가 잘린다.  ⚠️ 부호를
  주려면 필드 폭 개정이 먼저다 (8장 미결).
...
| `HTROUT` | 히터 **실제 출력 전압** `'3.512'` [V] — 부호 없음. sentinel `'NC'` | ICG heater |
...
설정 메모리를 **되읽은**(`RCONFIG`) 값이고, `HTROUT` 은 모듈이 **실제로 내는**
값(`STATUS`)이다.  ⛔ 호스트가 *"보냈다"* 고 기억하는 캐시는 원천이 아니다 --
...
⛔ **`HTROUT`(히터 채널 출력, 0~25 V)과 guide 전원 레일 `HEATER`(+28 V, 10.4절
...
⏳ **`HTROUT` 의 원천 구현은 대기 중이다** — 그 값은 HK 취득이 이미 주기마다 뜨는
`STATUS` 스냅샷 안에 있고(`C1_TEMP` 를 채우는 그 응답), 거기서 꺼내면 왕복이 늘지
...
| OI-25 | HK 신설 5장의 원천 배선 | ⛔ **`HKUDATE`·`HTREN`·`HTRSET`·`HTROUT`·`HTRFORCE` 다섯 다 실기 원천이 아직 안 닿았다.** 카드·형·자리·sentinel 은 서 있고 시뮬은 실값을 내지만, 실기 경로(`icg_archon` HK 스냅샷 → `ics_archon.sensors()`)가 **이 다섯 키를 아직 담지 않아** 실기에서는 전부 sentinel 로 실린다. 되읽기 함수 자체는 있다(`icg_archon/heater.py` 가 `HTREN`·`HTRSET` 을 `RCONFIG` 로 읽는다) — 빠진 것은 **HK 표본기가 그것을 주기적으로 담는 일**이고, `HTRFORCE` 되읽기와 `HTROUT`(`STATUS` 실측)은 코드가 **아직 0줄**이다. ⭐ `HKUDATE` 는 통과 경로가 섰다 — 스냅샷에서 **가장 낡은 표본시각**을 싣는다(실제보다 신선하다고 말하지 않으려는 것) |
...
| 적용 ACF | `../ics_archon/acf/KMTK_GUI_162_STA0201_R2610.acf` (KASI 벤치 유닛 실물 — `Pixels=540` · `PIXELCOUNT=528` · `LINECOUNT=1033` · 실탭 8 · `BIGBUF=0`), 분석은 `../ics_archon/acf/README.md`. ⚠️ `../ics_archon/` 은 `ics-archon-v1.0-build` 브랜치에 있다 — `main` 에서는 링크가 아직 안 열린다 |
...
1. **`EXPTIME` = 연속한 두 프레임 독출 개시 시각의 간격** [seconds]. 취득 SW 가 프레임 주기를 이 값으로 만든다 (형 규칙은 5.4절과 동일 — 정수형 기본).
   ⛔ **하한이 있다 (v1.10)** — 독출 자체가 store 1033행을 훑는 시간보다 짧은 간격은 만들 수 없다. 현행 ACF `R2610` 계산으로 **≈ 1.25 s**(`Pixels=540` 기준 디지타이징 1.2439 s + 트랜스퍼). ⭐ **하한 미만 지시는 거절하지 않고 하한으로 접는다** (운영자 확정 2026-08-31: *"더 작게 설정해도 최소 노출시간으로"*). ⛔ 그러면 요청값과 실제 주기가 갈리므로 **헤더에는 요청값이 아니라 실현된 간격을 싣는다** — `EXPTIME` 의 정의가 *"연속한 두 독출 개시의 간격"* 이므로 실현값이 곧 정의값이다 (`icg_archon` `frame_floor()`·`realized_period()`). ⚠️ 하한 값은 **ACF 판에 매인다**(구 `R2609` 는 ≈ 1.37 s) — `CTRL1CFG` 와 함께 읽어야 한다.
2. **첫 프레임은 항상 폐기한다** — 노출 개시 시점이 정의되지 않은 전하이기 때문이다. **저장은 둘째 프레임부터**다.
   ⭐ **그 프레임은 디지타이즈하지 않는다 — flush 다 (v1.10, 운영자 확정 2026-09-04).** 버릴 전하라 읽을 이유가 없으므로 **`EXPTIME` 을 주지 않고**(0) `SkipLine` 반복으로 비운다. ⚠️ 다만 flush 가 본 독출보다 **빨리 끝나면 첫 저장 프레임의 실적분이 `EXPTIME` 보다 짧아지므로**, `SkipLine` 횟수를 **본 독출 소요와 같아지도록** 맞춘다 — ACF 파라미터 **`FlushLines`** = **2448**(현행 `Pixels=540` 기준). ⏳ **`R2611` 로 도입 예정** — 현행 `R2610` 에는 아직 없다. **첫 저장 프레임의 실적분 기준 시각은 flush `FrameShift` 의 *개시* 시각**이다(종료가 아니다) — 4번의 `DATE-OBS` 정의와 같은 시점이다.
...
3. **`go n` = 프레임 `n`+1개 독출 · 첫째 폐기 · `n`장 저장** (운영자 확정 2026-08-30). `go` = `go 1` — 프레임 2개 독출 · 첫째 폐기 · 둘째 1장 저장. 독출 간격 = `EXPTIME`.
4. **`DATE-OBS` = 직전 프레임의 독출 개시 시각** — 저장 프레임의 노출이 시작된 순간이다. 첫 저장 프레임(둘째 독출분)의 `DATE-OBS` 는 **첫 프레임의 독출 개시 시각**이다(운영자 원문 그대로). 밀리초 필수, sentinel 금지 (5.4절과 동일). `go n` 다중 저장에서도 같은 규칙이다 — 각 저장 프레임의 `DATE-OBS` = 그 직전 독출 개시 시각 (프레임마다 파일이 갈린다, 아래 6).
5. **독출 중에도 image 구간은 계속 노출된다** — 그래서 경계가 "독출 개시"다. 실체로는 프레임 트랜스퍼(1033 사이클, ms 오더)가 노출을 끊고 store 독출이 뒤따르므로, 트랜스퍼 개시와 독출 개시는 실무상 같은 시점이다. ⚠️ 셔터 없는 frame-transfer 운용의 구조적 특성으로 **수직 스미어**가 상존한다 — gmon 설계가 이를 감안한다 (`gmon/DESIGN.md`).
6. **저장 프레임마다 파일 1개, 노출 번호 증가** — `go n` 이면 FITS `n`개가 저장된다 (운영자 확정 2026-08-30).
7. ⛔ **`GUIEXPCTRL` 이 끊으면 거기서 끝이다 — 이어가지 않는다 (v1.10, 운영자 확정 2026-09-04).** ICS 는 science **독출 구간**을 지키려고 guide 에 `GUIEXPCTRL`(`EXPENABLE=FALSE`)을 보낸다 — science **노출 중에는 guide 가 노출·독출을 계속해도 무방하고**, 막는 것은 독출 구간뿐이다. `EXPENABLE=FALSE` 가 들어오면 그 시점이 시퀀스의 어디든 **guide 노출 시퀀스는 완전히 종료**되고, **그때 노출 중이던 프레임은 저장하지 않는다.** ⛔ `EXPENABLE=TRUE` 로 풀려도 **다음 노출을 이어가지 않는다** — 재개가 아니라 **새 `GO`** 가 있어야 다시 돈다. ⚠️ 그러면 새 블록의 첫 프레임은 다시 2번대로 **폐기**된다. ⚠️ `DONE: EXPENABLE` 은 *"시퀀스를 세웠다"* 이지 *"guide 가 조용하다"* 가 아니다 — 진행 중이던 프레임은 끝까지 클록되므로 **최악 한 주기(≈ 1.25 s)** 가 더 걸린다.
...
| `CTRL1CFG` | guide ACF 이름 `<SITE>_GUI_<유닛>_<시리얼>_<ACF판>` — **검출기조 접미사가 없다** (science 와의 차이). 현행 실물은 `'KMTK_GUI_162_STA0201_R2610'` | ✅ `acf/README.md` 이름 규칙 |
...
**`C1_VOLT` · `C1_CURR` — guide 8자리** (운영자 확정 2026-08-30, 견본 v0.0): science 의 전원 레일 7자리 뒤에 guide 전용 **`HEATER`(+28 V)** 가 붙는다.
...
| 6 | 노출 | 첫 프레임 미저장(= flush, `EXPTIME` 없음) · `DATE-OBS` + `EXPTIME` ≈ 자기 프레임 독출 개시 시각 — **연속 블록 안에서만**(10.1-7) · `EXPTIME` ≥ 하한(10.1-1) |
...
| OI-26 | `FlushLines` 실측 | flush 프레임의 `SkipLine` 횟수 **2448**(⏳ `R2611` 도입 예정)은 `acftiming` **계산값**이지 실측이 아니다 (10.1-2절). 첫 guide 구동에서 flush 소요와 본 독출 소요를 재어 어긋나면 값을 고친다. ⛔ `Pixels`·`Lines`·`AT`·`ST` 중 하나라도 바뀌면 **반드시 다시 계산**한다 — 안 고치면 오류 없이 첫 저장 프레임의 실적분만 틀린다 |
...
| OI-23 | 노출 규약 잔여 | ~~장수 대응~~·~~`DATE-OBS` 배정~~·~~파일 배정~~ **종결 (2026-08-30)** — `n`+1 독출 `n` 저장 · 프레임당 파일 1개 (운영자 확정). **잔여: TC 질의 시점 · 노출 번호 카운터 독립(9.2절 제안)** — 레거시 계승안(프레임 사이클 개시 전 1회 스냅샷)과 함께 `icg_archon` 설계 때 확정 |
...
| guide 적용 ACF · 분석 | `../ics_archon/acf/KMTK_GUI_162_STA0201_R2610.acf` · `../ics_archon/acf/README.md` (⚠️ `ics-archon-v1.0-build` 브랜치) |
