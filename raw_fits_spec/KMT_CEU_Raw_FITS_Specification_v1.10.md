...
| OI-18 | NT 파일 `CCDTEMP` 귀속 | 대표 센서(M chip — comment 의 `M` 표기는 2026-08-30 제거, 귀속은 규격 기술로만 남는다)가 NT 파일에서도 그대로인지 확인 (5.6절). ⚠️ **폐기 예정** — 이월 대기 항목 |
...
| 적용 ACF | `../ics_archon/acf/KMTK_GUI_162_STA0201_R2608.acf` (KASI 벤치 유닛 실물 — `PIXELCOUNT=528` · `LINECOUNT=1033` · 실탭 8 · `BIGBUF=0`), 분석은 `../ics_archon/acf/README.md`. ⚠️ `../ics_archon/` 은 `ics-archon-v1.0-build` 브랜치에 있다 — `main` 에서는 링크가 아직 안 열린다 |
...
| 취득 주체 | `icg_archon` (별개 프로그램, 착수 미정 — `../ics_archon/SMC_CLAUDE.md` 작업 D). guide 계통 코드는 이 장과 10장이 선 뒤에 착수한다 |
...
프레임이 작고 주기가 짧아 **컨트롤러 프레임 카운터 되감김이 science 보다 훨씬 빨리 온다**(16비트라면 1초 주기 약 18시간) — 대응은 취득 SW 소관이다 (`../ics_archon/DevNote.md` 8.3절 — ⚠️ 브랜치).
...
- **물리 X overscan 은 없다** (저장 구간이 전부 실컬럼이다 — 헤더 카드로는 다크 기준열 16 을 `OVRSCNX` 에 귀속한다, 10.3절). 시퀀서는 채널당 레지스터 blank 8개를 건너뛰고(`PreSkipPixels=8`) **601 픽셀을 디지타이즈**(`Pixels=600` + 인자 없는 flush 1)하지만 **앞 528 만 저장**하고 73 개를 버린다 (ACF `PIXELCOUNT=528`, 타이밍 LINE43~47).
...
검증 불변식: **`DATE-OBS` + `EXPTIME` ≈ 자기 프레임의 독출 개시 시각** (10.5절 6번).
...
| `CTRL1CFG` | guide ACF 이름 `<SITE>_GUI_<유닛>_<시리얼>_<ACF판>` — **검출기조 접미사가 없다** (science 와의 차이). 현행 실물은 `'KMTK_GUI_162_STA0201_R2608'` | ✅ `acf/README.md` 이름 규칙 |
...
| 3 | 카드 전량 | 견본 v0.0 구성 — **값 카드 123장**(10.2절) · 카드 형 일치 · **144 레코드 = 값 123 + COMMENT 8 + END 1 + 공백 12**, 정확히 4×2880 = 11,520 B |
...
| 6 | 노출 | 첫 프레임 미저장 · `DATE-OBS` + `EXPTIME` ≈ 자기 프레임 독출 개시 시각 (10.1절) |
...
| OI-20 | 저장 528 의 X 구간 성격 | 데이터시트 대응은 **다크 기준열 15+1**(9.4절) — 실측 확정: `Pixels` 600→528 트림 무손실 검증(인자 없는 flush +1 유지, `ics_archon` P-k 항목)으로 **확정된 `OVRSCNX=16` 귀속(10.3절)을 실측으로 뒷받침** |
...
| guide 적용 ACF · 분석 | `../ics_archon/acf/KMTK_GUI_162_STA0201_R2608.acf` · `../ics_archon/acf/README.md` (⚠️ `ics-archon-v1.0-build` 브랜치) |
