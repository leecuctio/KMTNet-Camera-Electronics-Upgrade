...
## ✅ 현행 규격 — raw spec **v1.16** (2026-09-24 판올림 · `OBSTYPE` 사용자 입력 카드 · `EXPTIME` 실수형 · 견본 문안 정정)

> ▶ **이어서 시작하는 자리는 이 절이다.**
> ⭐ **v1.16 은 운영자 결정(2026-09-24) 여섯을 싣는다** — 견본 6장에 먼저 반영했고 규격 · 원장 · 통합이 따라갔다.  규격 쪽 전량은 12장 v1.16 행 ①~④.  코드가 따라올 것은 아래 「브랜치 후속」의 v1.16 항목이고, 이 라운드가 새로 남긴 판단은 「운영자 판단 대기」의 `v1.16:` 행이다.
>
> ⭐ **운영자 결정** (2026-09-24 — 이 절 밖에서는 *"v1.16 결정 n"* 으로 부른다):
>
> 1. **`OBSTYPE` 은 사용자 입력으로만 바뀌는 카드다 — `PROJID` 와 같은 부류.**  기본값은 **무조건** science ICS `'SCIENCE'` · guide ICG `'GUIDE'` 이고, 명령으로 정하면 그 값이 실린다(대문자로 접는 규칙은 그대로).  ⛔ 빈 값이 와도 `IMAGETYP` 을 복사하지 않고 기본값을 싣는다 — 종전 빈 값 폴백을 폐지했다(판단 대기 `v1.15:R2-8 ②` 가 이것으로 닫혔다).  comment 는 `User-defined observation type`.  ⚠️ v1.13 이 적은 뜻 *"어느 계통이 찍었나"* 는 기본값의 성질로 내려갔다 — 사람이 바꿀 수 있으므로 계통 식별은 `DATASRC`(`ARCHON_SCIENCE`/`ARCHON_GUIDE`) · `DETID`(MK/NT/G)가 맡는다(규격 5.4절 · 5.5절 `DATASRC` 행 · 10.3절).
> 2. **`PRESCNX`/`PRESCNY`/`OVRSCNX`/`OVRSCNY` comment 의 괄호를 모두 뺐다** — science *"side varies"* · *"frame-edge side"* · *"frame-center side"*, guide *"(none)"* · *"(frame-edge side)"* · *"(frame-center side)"*.  guide `PRESCNX` 는 `Dark reference columns per amplifier` 그대로다.  자리의 뜻은 규격 4장 · 9.4절이 정한다.
> 3. **채널 맵 소제목 `COMMENT`** — *"Map of CCD output channels, raw X ascending within each card"* → *"…, raw X ascending"* (견본 셋 모두).
> 4. science **`ICSBUILD`** comment *"ICS/ICG software version and build Info"* → *"ICS software version and build Info"* · **`DSTELALT`/`DSTELAZ`** comment 에 *"in degrees"*.
> 5. **guide 견본을 기동 기본값으로** — `IMAGETYP` `'BIAS'` → `'OBJECT'` · `OBJECT` `'bias'` → **`'guide'`**(guide 기본값 — 운영자 2026-09-24.  ICG 의 지금 기동값인 공통 기반 `IcsState.objname='test'` 를 규범으로 올리지 않았다) · `EXPTIME` `0` → `2.0`(`[icg] exptime_default` 2.0 s — 형은 결정 6).  science 견본은 `BIAS` 그대로다.
> 6. **`EXPTIME` 은 science·guide 모두 실수형이다** — 소수점 아래 최소 한 자리: 정수 값에도 `.0` 을 붙이고(`0.0` · `2.0`) 소수부가 있으면 그대로다(`0.25` · `1.3` — 끝자리 `0` 을 채우지 않는다).  종전 *"정수형 기본, 소수점 아래 값이 있을 때만 실수형"*(원장 확인 요망 4 종결, 2026-08-22)을 고친다.  guide 의 ms 반올림(규격 10.1-1)은 해상도 규칙으로 남아 소수는 셋째 자리까지다 — 최소 노출 요청은 실제 주기 1.2996… s 지만 헤더는 `1.3` 이다.  견본은 MK·NT `0.0` · G `2.0`.
>
> **문서별 한 줄**:
>
> - **규격 v1.16** — 5.4절 `OBSTYPE` 행 재작성(폴백 서술을 걷고 레거시 호환 구절 하나만 남겼다) · 5.5절 `DATASRC` 행에 계통 식별 · 5.2절 geometry 두 행과 표 아래 한 문장(comment 는 자리를 적지 않는다) · 5.3절 `OBSERVER` 행의 `OBSTYPE` 비교 괄호 · 5.7절 `DSTELALT` [deg] · 5.7.1절 참고 블록(science 견본만 BIAS · guide 견본은 셔터 행 해당 없음) · `EXPTIME` 실수형(5.4절 행 · 5.0절 금지 목록 `0.0` · 7장·10.5절 3번의 형 예외 삭제 · 10.1-1 ms 반올림 문면에서 형 규칙을 걷음 · 10.3절 `BIAS`·기동 행) · 10.3절 `IMAGETYP`/`OBJECT` 행(guide `OBJECT` 기본 `'guide'` — 적지 않으면 10.2절 규칙으로 5.4절 `'bias'`* 가 guide 에도 걸린다) · `OBSTYPE` 행 · geometry 행 · 8장 OI-32(science 기본값을 맞출 때 guide 기본값은 ICG 가 기동 때 따로 설정한다 · *"별표 넷"* → *"다른 카드"* · `OBSTYPE` 구절) · 머리말 연동 표와 10.2절의 *"guide 견본 내용이 바뀐 판"* 목록에 v1.16.  **절 번호는 그대로**이고, ICD 가 인용한 절(2.2 · 2.3 · 4.3 · 5.6 · 5.6.1)과 5장 카드 집합도 안 바뀌었다.
> - **원장 v1.22** (구판 `archive/…v1.21.md`) — 3.1절 `OBSTYPE` 행 · 3.1절 `EXPTIME` 행 Integer → **Real** · 13장 *"개칭 없이 뜻이 바뀐 카드"* 의 `OBSTYPE` · 7장 geometry 네 행의 괄호 표기를 걷고 규격 4.1 · 4.2절 포인터로 · 8장 `OVERSCNY` · 8.1절 `OVERSCNX` 행의 *"대신 보는 것"* 칸도 같은 표기 정리 · 3.7절 `CCDTEMP` 행·단락의 comment 인용을 `CCD temperature [deg C]` 로(견본 바이트와 맞춘 정정) · 11.1절 `RAILREF` 행.  판정 불변 — converter v2.5.0 은 `OBSTYPE` 을 raw 값 그대로 옮기고 raw comment 는 읽지 않는다.
> - **통합 v1.2** (구판 `archive/…v1.1.md`) — §1 **(기록, v1.2)** 행 신설(LEECU 요청 둘 — 계통 판별에 `OBSTYPE` 대신 `DATASRC` · 정의서 `OBSTYPE` 정의 갱신) · (기록, v0.10) 행과 §1.1 대비표 3행에 뜻 재정 표지 · §1.1 뒤 상자와 §4 정의서 갱신 요청을 새 뜻으로 · §3 과 머리말의 ICD 인용 갱신 요청을 v1.16 경로로 · §3 `OVERSCNY` 항목의 *"frame-center side"*(comment 에서 걷힌 괄호)를 raw spec 4.2절 포인터로 · §1 HK 온도 행의 `CCDTEMP` comment 인용을 `CCD temperature [deg C]` 로 · §1 `VOLTINFO` 공급원 행의 `RAILREF` 근거 판 목록에 v1.16 · 구판 v1.1 줄에 옮겨 적은 v1.0 날짜를 09-23 → 09-24 로(판 날짜 = 발행일).  나머지 comment 정정과 guide 견본 값, `EXPTIME` 실수형은 MEF 파급이 없다(converter 는 raw comment · `COMMENT` 를 읽지 않고 `EXPTIME` 은 `fnum()` 으로 늘 실수로 쓰며, guide 는 MEF 경로가 없다).
> - **견본 6장** — 이름을 `v1.16` 으로 바꾸고 결정 1~6 을 실었다(레코드 수·크기 불변 — 14,400 · 14,585 · 11,520 · 11,669 B).  guide 견본 대사 목록의 셋(`OVRSCNY` comment · `COMMENT` *"each card"* · `EXPTIME=0` `BIAS` 시나리오)이 이것으로 닫혔다(아래 (구) v1.9 절).
> - **README** — 현재 기준선 · 판 표 · 디렉토리 표 · `archive/` 잔류 범위를 v1.16 · v1.22 · v1.2 로, 「규격을 구현하는 곳」 시뮬 행에 v1.16 미반영 한 줄.
>
> ✅ 태그 — `raw-spec-v1.15`(`ddb6e94`)를 로컬·원격에서 지우고 **`raw-spec-v1.16`** 을 이 판의 마지막 커밋에 붙였다(2026-09-24).

### ⏳ 브랜치 후속 (`ics-archon-v1.0-build`)

> 결정 번호는 아래 「(구) v1.14」 절의 운영자 결정 번호다 — *"v1.16 결정 n"* 만 위 현행 절의 결정이다.  기준은 브랜치 로컬 HEAD **`e9e8f46`**(2026-09-24 — 워크트리 깨끗함, 원격은 `32b002d`)이다.  ⭐ 코드 자리는 줄 번호 대신 **함수 이름과 인용문**(grep 닻)으로 적는다 — 다른 세션이 계속 고치는 파일이라 줄 번호는 곧 밀린다.
...
- **`EQUINOX` 후속** — 규격이 따라잡은 뒤 할 것(DevNote 11.95 *"규격이 따라잡을 때 할 것"*): 두 `SPEC_PENDING` 비우기 · `ics_sim/tests/test_raw_draft.py` 의 `_ahead_of_sample()` 에서 `EQUINOX` 지우기(`DAZERR` 는 결정 9 항목에서 함께 지운다 — 아래) · 같은 파일 `test_ahead_of_sample_matches_the_pending_list` 의 `== ['EQUINOX']` 단언 · `test_icg_cards.test_only_equinox_runs_ahead_of_the_sample` 을 `== ()` 로 · `tools/gen_guidecards.py` 의 `SAMPLE` 갱신과 재실행(방식은 아래 v1.16 `SAMPLE` 항목이 정한다 — 판 무관 글롭 또는 현행 판 파일명.  지금은 `v1.13` 을 박아 두었다 — ⚠️ 합류 전 v1.13 견본으로 돌리면 guide `EQUINOX` 가 `'S'` 로 돌아간다) · labtest 다섯 사본의 `RAWCARDS` 와 TCS 결측값(`'NC'` → `-999.0`) · 거짓이 된 주석 넷 — `telemetry.py` 의 *"규격 5.7절·견본은 아직 문자열이다"* · *"규격은 아직 문자열이다"*, `rawcards.py` 의 *"견본은 아직 문자열 `'2000.000'` 이다"*, `guidecards.py` 의 *"견본은 아직 문자열이라"*.  `SPEC_PENDING` 을 비우는 같은 커밋에서 한다.
  ⚠️ **합류하면 빨개지는 시험이 셋이다** (v1.16 문안 정정분은 빼고 센 것) — `test_icg_cards.test_template_matches_the_sample_generator`(`gen_guidecards.SAMPLE` 이 박아 둔 v1.13 견본이 `main` 에서는 v1.14 부터 판마다 개명돼(지금 `v1.16`) 없어서 도구가 `SystemExit` 로 멈춘다) · `test_raw_draft.test_the_sample_has_not_caught_up_yet`(MK·NT) · 그 guide 짝 `test_icg_cards.test_the_guide_sample_has_not_caught_up_yet`(G — 견본 `EQUINOX = 2000.0` 을 `gen.parse()` 가 `'R'` 로 읽어 `SPEC_PENDING` 의 현행 카드와 같아진다.  종전 이 줄은 science 쪽만 적었다).  뒤의 둘은 견본이 따라잡았다고 알리는 **의도된 신호**다.  위 일감을 처리하면 셋 다 걷힌다.  나머지 바이트 대사는 v1.15 견본 기준으로는 초록일 것으로 봤다(`_ahead_of_sample()` 이 견본 값으로 현행 이미지를 만든다) — v1.16 견본의 comment 정정이 science 대사 둘을 더 빨갛게 만든다(아래 v1.16 항목).  합류 뒤 전수로 확인한다(`main` 에는 견본 대사 시험이 없다).
- **v1.16 견본 따라가기** (v1.16 결정 1~6 — 다른 세션 몫):
  - **`EXPTIME` 실수형** (v1.16 결정 6) — `ics_sim/ics_sim/rawcards.py` `CARDS` 와 `ics_archon/icg_archon/guidecards.py` `CARDS` 의 `('EXPTIME', 'I', 0, 'Exposure time [seconds]')` 를 `'R'` 로 — `'R'` 갈래는 `float()` 로 싣고 astropy 가 `2.0` · `0.25` 꼴로 쓰므로 *"최소 한 자리"* 가 저절로 된다(guide 는 생성기로 — 아래 `SAMPLE` 항목).  `rawcards.py` `kind == 'I'` 갈래의 *"EXPTIME 조건부 형"* 분기는 `EXPTIME` 만을 위한 것이었다 — 다른 `'I'` 카드에 소수 값이 오는 경로가 없는지 본 뒤 걷는다.  동반 문면 — `rawcards.py` 의 *"`EXPTIME` 은 정수형 기본, 소수점 아래 값이 있을 때만 실수형이다"* · `rawhdr.py` `exposure_header()` docstring 의 같은 구절 · `guidecards.py` 의 *"`EXPTIME` 조건부 형, 5.4절"* · `icg_archon/backend.py` `effective_exptime()` docstring(*"5.4 조건부 형 규칙으로 카드가 실수형이 된다"* — ms 반올림은 해상도 규칙(규격 10.1-1)으로 남기고 형 근거만 걷는다 · *"`guideexp 2` -> `2`"* → `2.0`) · labtest 다섯 사본의 `RAWCARDS` · `_vendor` 재동기(grep 닻 `조건부 형` · `정수형 기본`).  동반 시험 — `ics_sim/tests/test_raw_header.py` 의 *"`EXPTIME` 정수형 기본 …"* 시험(`isinstance(…, int)` 단언을 실수로 뒤집는다) · `test_raw_draft.py` 의 *"EXPTIME 은 조건부 형(I/R)이라 …"* 주석과 `exptime=int(sample['EXPTIME'])` · `ics_archon/tests/test_icg_app.py` docstring *"EXPTIME(정수 2)"*.  (`test_backend.py` 의 `h['EXPTIME'] == 2` 는 `2.0 == 2` 라 그대로 초록이다.)
  - **템플릿 comment 를 견본 v1.16 과 같게 — science 9장 · guide 7장**(guide 는 `PRESCNX` · `ICSBUILD` 가 빠진다) — `ics_sim/ics_sim/rawcards.py` `CARDS` 와 `ics_archon/icg_archon/guidecards.py` `CARDS` 의 `OBSTYPE`(*"Type of observation"* → `User-defined observation type`) · `PRESCNX`/`PRESCNY`/`OVRSCNX`/`OVRSCNY`(괄호 걷기 — guide `PRESCNX` 는 그대로) · 채널 맵 `COMMENT`(*"… raw X ascending within each card"* → *"… raw X ascending"* — `rawcards.py` 에서는 두 줄로 이어 쓴 문자열이라 grep 이 한 줄로 안 잡힌다) · science `ICSBUILD`(*"ICS/ICG software …"* → *"ICS software …"*) · `DSTELALT`/`DSTELAZ`(끝에 *"in degrees"*).  guide 는 손으로 고치지 않고 생성기로 만든다(아래 `SAMPLE` 항목).  `_vendor` 사본(`ics_archon/ics_archon/_vendor/ics_sim/rawcards.py`)은 `tools/sync_vendor.py` 재동기, labtest 다섯 사본(`ics_archon/scr_labtest/archon_kmtnet_labtest_v1.3.*.py`)의 `RAWCARDS` 는 손으로 고친다.
  - **`OBSTYPE` 빈 값 폴백 폐지** (v1.16 결정 1) — `ics_sim/ics_sim/rawhdr.py` `exposure_header()` 의 `'OBSTYPE': (obstype or kind).upper()` 를, 빈 값이면 계통 기본값(science `'SCIENCE'` · guide `'GUIDE'`)을 싣게 고친다.  guide 도 이 함수를 쓰므로(`icg_archon/guidehdr.py` → `rawhdr.exposure_header`) 기본값을 어디서 쥘지(호출부가 넘기기 등)는 브랜치가 정한다.  동반 시험 — `ics_sim/tests/test_raw_header.py` `test_an_empty_obstype_still_falls_back_to_imagetyp` 를 뒤집는다.  동반 문면 — 같은 함수 docstring 의 *"`OBSTYPE` 는 **어느 계통이 찍었나**"* · *"`obstype` 이 비면 종전대로 `IMAGETYP` 를 따른다"* · *"규격 5.4절의 … 는 낡았다 -- 갱신은 `main` 소관"*(아래 「낡은 코드 문면」 첫 항목과 같은 자리), `ics_sim/ics_sim/state.py` `IcsState.obstype` 주석의 첫 줄 *"`OBSERVER` 와 같은 부류다"* · *"`OBSTYPE` 는 **어느 계통이 찍었나**다"* · *"(`OBSTYPE` = 계통 식별 …)"*, `ics_sim/ics_sim/commands.py` `cmd_obstype` docstring 의 *"`OBSERVER` 와 같은 부류다"* · *"`OBSERVER` 와 다른 점이 여기 하나뿐이다"*(v1.16 은 `PROJID` 부류로 적었고, 빈 값 처리(`OBSERVER` 는 `'NC'` · `OBSTYPE` 은 기본값)와 계통별 기본값도 `OBSERVER` 와 다르다 — 규격 5.3 · 5.4절), `test_raw_draft._rebuild()` 주석 *"안 넘기면 `exposure_header()` 의 안전망이 `IMAGETYP` 값을 실어"*(같은 주석의 *"어느 계통이 찍었나라서"* 도), `test_icg_cards.test_the_guide_pool_carries_obstype_through` docstring 의 *"배선이 빠지면 guide 헤더만 조용히 `IMAGETYP` 사본으로 돌아간다"*, `ics_sim/tests/test_raw_header.py` 의 절 머리 주석 *"OBSTYPE -- OBSERVER 와 같은 부류의 헤더 전용 값"* · `test_obstype_defaults_to_science_and_no_longer_copies_imagetyp` docstring *"`OBSTYPE` 은 **어느 계통이 찍었나**다"* · `test_the_obstype_command_reaches_the_header_and_nothing_else` docstring *"`OBSERVER` 와 같은 부류"*, labtest 사본의 *"어느 계통이 찍었나다"* 주석, 현행 명령 표 `ics_archon/legacy_command_coverage.md` `OBSTYPE` 행의 *"어느 계통이 찍었나"*(`ics_archon/DevNote.md` 의 같은 표는 과정 기록이라 그대로 둔다).  grep 닻(정규식): `어느 계통이 찍었나` · `OBSERVER.? 와 같은 부류`(백틱이 없는 자리도 있다) · `OBSERVER.? 와 다른 점이`(`cmd_obstype` 은 *"… 다른 점이 / 여기 하나뿐이다"* 로 줄이 갈려 *"여기 하나뿐"* 으로는 안 잡힌다).  `_vendor` 사본(`commands.py` · `state.py` · `rawhdr.py`)은 `tools/sync_vendor.py` 재동기로 따라온다.
  - **`tools/gen_guidecards.py` 의 `SAMPLE`** — 지금 `…G.fits.header.v1.13.txt` 를 박아 두어서 판올림마다 멈춘다(v1.14 · v1.15 · v1.16).  판 무관 글롭(시험 `_samples()` 와 같은 `KMT?.*.G.fits.header.v*[0-9].txt`)으로 바꾸고 재생성해 `guidecards.CARDS` 를 갱신한다.  ⚠️ 반대 논거 — 도구 주석은 판을 박는 것이 **의도**라 한다(*"어느 판의 사본인지가 곧 이력이다"*).  글롭으로 바꾸면 그 이력은 `guidecards.py` 머리말의 판 표기(지금 *"…v1.13.txt"* — 아래 「판올림 규약」 절)가 맡아야 한다.  위 `EQUINOX` 항목의 `SAMPLE` 갱신과 한 번에 한다.
  - **바이트 대사 시험** — 합류하면 comment 갈림으로 빨개지는 것: science `ics_sim/tests/test_raw_draft.py` `test_template_matches_the_draft_structure` · `test_assembly_reproduces_the_draft_byte_for_byte`(MK·NT — `_rebuild()` 가 `rawcards` 템플릿 comment 로 조립한다), guide `ics_archon/tests/test_icg_cards.py` `test_template_matches_the_sample_generator`(`SAMPLE` 을 고쳐도 재생성 전에는 빨갛다).  초록으로 남는 것: guide `test_sample_bytes_are_reproduced` · `ics_archon/tests/test_fitswrite.py` 의 대사 둘 — 견본에서 뽑은 템플릿·comment 를 되먹인다.  (코드를 읽고 판정했고 돌려 보지는 않았다.)
  - **guide 대사 시험이 G 견본의 `IMAGETYP`/`OBJECT`/`EXPTIME` 을 되먹이는가** (v1.16 결정 5) — ✅ 되먹인다.  `test_sample_bytes_are_reproduced` 는 `gen.parse(path)` 가 견본에서 뽑은 값(`'OBJECT'` · `'guide'` · `EXPTIME` `2.0` → 실수 `'R'`)을 그대로 넣어 재현하므로, 견본 값이 바뀌어도 깨지지 않는다.  기동 기본값 자체는 이 대사 시험이 보지 않는다 — `IMAGETYP` `'OBJECT'` · `EXPTIME` 2 s 는 `ics_archon/tests/test_icg_imagetype.py` 의 `test_startup_defaults_are_object_and_two_seconds` 가 이미 못박는다(`app.state.imgtype == 'OBJECT'` · 응답 *"GuiExp=2 seconds"* · `icfg.exptime_default == 2.0`).  빠진 것은 `OBJECT` 하나다 — ICG 가 기동 때 `objname = 'guide'` 를 설정하는 커밋(아래 「넘겨받은 것」 science 기본값 항목)에서 같은 시험에 `app.state.objname == 'guide'` 단언을 더한다.
...
- **넘겨받은 것** (v1.14 ~ v1.16 라운드의 편집자와 재검토가 넘긴 것 — 겹친 것은 하나로):
  ...
  - **science 노출 상태 기본값 셋이 규격과 다르다** — `ics_sim/ics_sim/state.py` `IcsState` 의 `imgtype='OBJECT'` · `objname='test'` · `observer='none'` 인데 규격 5.3·5.4절(과 원장 · 견본)은 `BIAS` · `'bias'` · `'KMTNetOp'`(자리채움 낱말 금지)다 — OI-32 가 *"코드가 규격에 맞출 자리"* 로 짚은 셋이다.  ⚠️ 함정 — guide 는 같은 `IcsState` 를 물려받고 `icg_archon/app.py` 가 *"OBJECT 는 기반 상태의 기본값 그대로"* 라며 `IMAGETYP` 을 누르지 않는다.  기반 기본을 `BIAS` 로 바꾸면 guide 가 `BIAS`(최소 노출 고정 · `EXP`/`GUIEXP` 거부)로 떠 규격 10.3절 기동 기본 `OBJECT` 와 어긋나므로, 같은 커밋에서 `icg_archon/app.py` 가 `imgtype = 'OBJECT'` 를 눌러 두고 그 주석을 고친다.  ⭐ **v1.16 부터 `OBJECT` 는 기반을 고치기 전에도 어긋난다** — 규격 10.3절 guide 기본이 **`'guide'`**(v1.16 결정 5, 운영자 2026-09-24)라 기반의 `'test'` 도 5.4절의 `'bias'` 도 아니다.  그래서 `icg_archon/app.py` 가 기동 때 `objname = 'guide'` 를 설정하고(`obstype = 'GUIDE'` 옆) 위 주석 *"OBJECT 는 기반 상태의 기본값 그대로"* 를 고친다 — 기반을 고치는 커밋을 기다리지 않아도 된다.  동반 시험: `tests/test_icg_cards.py` 의 `IcsState().observer == 'none'` 단언과 guide 누름 단언.
  - 낡은 코드 문면:
...
> 번호 공간이 둘이다 — 맨 번호는 2026-09-23 전반 재검토(v1.14 라운드)의 발견 번호이고, `v1.15:` 를 붙인 것은 2026-09-24 발행본 재검토(v1.15 라운드)의 번호다.  두 목록이 모두 `C-n` 을 쓰고, 어느 쪽 `C-n` 도 통합 문서의 C-항목과는 다른 번호 공간이다.  `v1.16:` 을 붙인 행은 v1.16 라운드 편집자가 남긴 것이라 번호 없이 주제를 붙였다.  목록 원본은 세션 임시 폴더에 있어 저장소에 없으므로 행마다 내용을 적는다.
...
| D2-3 · D7-9 | 견본의 `OBSTYPE` comment 가 `IMAGETYP` 과 같은 레거시 *"Type of observation"* 이다(뜻은 v1.13 에서 계통 식별로 바뀌었다) | 카드를 지목하면 견본 6장 · 템플릿 · 대사 시험을 한 커밋에 · 6장에 뜻 변경 행(선택)  ✅ **해소 (운영자 2026-09-24, v1.16 결정 1)**: comment 를 `User-defined observation type` 으로 — 견본 6장에 실었고, 템플릿 · 대사 시험은 위 「브랜치 후속」 v1.16 항목이다.  뜻도 사용자 정의 관측 유형으로 다시 정해졌다 — 계통 식별은 `DATASRC` · `DETID`(v1.16 결정 1). |
| D7-13 | `DSTELALT`/`DSTELAZ` comment 에 단위가 없고, science `ICSBUILD` comment 에 *"ICS/ICG"* 가 있다 | 카드를 지목할 때만 고친다  ✅ **해소 (운영자 2026-09-24, v1.16 결정 4)**: *"… in degrees"* · *"ICS software version and build Info"* — 견본 6장에 실었다(템플릿은 위 「브랜치 후속」). |
...
| `v1.15:R2-8` ② | `OBSTYPE` 빈 값 폴백이 `IMAGETYP` 을 복사한다 — 5.4절이 *"`IMAGETYP` 의 사본이 아니다"* 라 하고 5.0절이 결측을 실제로 쓰이는 값으로 적지 말라는 것과 긴장이 있다.  v1.15 는 현행을 두고 *"운용 경로로는 빈 값이 오지 않는다(명령의 빈 인자는 조회 · INI 키 없음) — 내부 호출부가 인자를 빠뜨렸을 때만 타는 안전망"* 만 5.4절에 적었다 | 폴백을 `'NC'` 나 계통 기본값(`SCIENCE`/`GUIDE`)으로 바꿀지 — 규범 변경이고 브랜치 `rawhdr.py` 가 동반한다  → **ics-archon 브랜치 세션에서 검토·업데이트** (운영자 2026-09-24) — 규격은 그 결정을 따라간다.  ✅ **해소 (운영자 2026-09-24, v1.16 결정 1)**: 폴백을 폐지했다 — 빈 값이 와도 계통 기본값이다(규격 5.4절 · 10.3절).  `rawhdr.py` 는 위 「브랜치 후속」 v1.16 항목이다. |
...
| `v1.16:` guide `OBJECT` 기본 | 규격 10.3절 `IMAGETYP`/`OBJECT` 행에 guide `OBJECT` 기본 `'test'` 를 규범으로 적고 *"운영자 확정 2026-09-24"* 를 달았다.  v1.16 결정 5 는 이것을 **견본 값**으로 적었는데, 규격에 적지 않으면 10.2절 규칙(*"표에 없는 카드는 5장 그대로다"*)으로 5.4절 `'bias'`* 가 guide 에도 걸려 새 G 견본이 규격과 어긋난다.  ⚠️ 그 확정 표기는 결정 5 의 *"ICG 기동 기본"* 을 확정으로 읽은 것이고, 8장 OI-32 · 12장 v1.16 ③ 도 이것을 규범으로 쓴다.  위 「넘겨받은 것」 science 기본값 항목도 이 확정을 전제한다 | ✅ **해소 (운영자 2026-09-24)**: guide `OBJECT` 기본은 **`'guide'`** 다 — ICG 기동값 `'test'` 를 규범으로 올리지 않았다.  규격 10.3절 · OI-32 · 12장 v1.16 ③ · G 견본을 `'guide'` 로 고쳤다.  ICG 는 아직 공통 기반의 `'test'` 를 싣는다 → 「넘겨받은 것」 science 기본값 항목(코드 몫) |
| `v1.16:` OI-32 문면 | 규격 v1.16 편집에서 8장 OI-32 의 *"같은 부류의 별표 넷"* 을 *"같은 부류의 다른 카드"* 로 고치고(`OBSTYPE` 이 v1.16 부터 `PROJID` 부류라 넷이 아니다) `OBSTYPE` 한 구절을 더했다 — 기본값은 규격 · 견본 · 취득 SW(`IcsState.obstype='SCIENCE'` · ICG 가 누르는 `'GUIDE'`)가 일치하고, 빈 값 처리만 취득 SW 가 옛 규칙(`IMAGETYP` 복사 — 5.4절이 폐지)이라 코드 몫이다(위 「브랜치 후속」 v1.16 폐지 항목) | ✅ **해소 (2026-09-24 — 편입 유지)**: 규범을 바꾸지 않는 문면 보충이고, v1.16 부터 `OBSTYPE` 이 `PROJID` 부류라 OI-32 를 읽는 사람이 *"`OBSTYPE` 은?"* 을 물을 자리다.  코드 몫은 「브랜치 후속」 v1.16 폐지 항목에도 있으니, 코드가 따라오면 OI-32 의 그 구절을 걷는다 |
| `v1.16:` converter comment | 통합 v1.2 §1 (기록, v1.2) 행은 converter 의 MEF `OBSTYPE` comment(*"type of observation"*)를 raw 새 comment 에 맞출지를 **LEECU 선택**으로 적었다.  ⚠️ MEF 경로는 science 전용이라 MEF `OBSTYPE` 은 기본값이 늘 `'SCIENCE'` 인 사용자 입력 칸이 되고, 계통 판별 카드(`DATASRC`)는 정의서에 이미 있다 — LEECU 요청 ① 의 실무 뜻은 *"하류가 `OBSTYPE=='GUIDE'` 같은 분기를 만들지 말라"* 에 가깝다 | comment 정렬을 요청으로 올릴지 · LEECU 선택으로 둘지 |
...
| D3-16 | G 견본 `PRESCNY` comment `(frame-edge side)` 도 science 문구 그대로다 — 아래 대사 목록의 `OVRSCNY` 항목과 함께 정리  ✅ **해소 (운영자 2026-09-24, v1.16 결정 2)**: geometry 네 장의 괄호를 science · guide 모두 뺐다 |
...
### (구) v1.15 (2026-09-24 판올림 · 정리판 · 발행 커밋 `ddb6e94`)

> ⭐ **v1.15 는 정리판이다** — v1.14 발행본 재검토(2026-09-24)에서 반증을 통과한 발견 100건을 **전부** 이 라운드에서 다뤘고 **규범은 바꾸지 않았다**(운영자 확정 2026-09-24).  사실 정정·빈틈 채우기와 층위 정리는 이 판 · 동반 판 · README · 인수인계에 실었고, 규범을 바꿔야만 풀리는 것은 위 현행 절의 「운영자 판단 대기」 표로, 코드가 따라올 것은 「브랜치 후속」으로 넘겼다.  규격 쪽 전량은 12장 v1.15 행 ①·②.
>
> **문서별 한 줄**:
>
...
>
> ✅ 태그 — `raw-spec-v1.14`(`ad7e0a5`)를 로컬·원격에서 지우고 **`raw-spec-v1.15`** 를 이 판의 마지막 커밋에 붙였다(2026-09-24).  v1.14 발행 뒤에 쓴 README 태그 절도 이 판의 커밋에 실었고 `raw-spec-v1.14` 는 옮기지 않았다(위 판단 대기 `v1.15:R1-8`).  팀 안내는 README 「버전 / 관리 정책」의 태그 소절 한 곳이다.  `raw-spec-v1.15` 는 v1.16 발행으로 지웠다(2026-09-24).
...
- 📋 **guide 견본 ↔ 10장 대사 목록** (오기·오타는 운영자 지시로 정정 완료 — 잔여는 이 목록을 해소하는 라운드에서.  ⭐ 견본은 v1.10 부터 규격과 같은 판 번호라 따로 '승격'은 없다):
  ...
  5. ⏳ **최종 검토(2026-08-30, 커밋 후 전수 재검)에서 추가된 확인 항목** — 목록 해소 라운드에서 함께:
     ...
     - ✅ ~~`OVRSCNY` comment `(frame-center side)` — science 문구가 그대로 왔는데 guide 의 추가 9행은 **중앙이 아니고 위치도 미정**(OI-21)이다 — 목록 해소 때 문구 정정.~~ — **해소 (2026-09-24, v1.16 결정 2)**: geometry 네 장의 괄호를 모두 뺐다(`Overscan rows per amplifier`) — 자리의 뜻은 규격 9.4절이 정한다.
     - ✅ ~~`OVRSCNX` comment `(side varies)`~~ — **해소 (v1.13)**: guide 의 X 16 은 `OVRSCNX` 가 아니라 **`PRESCNX`** 로 귀속이 바뀌었다(운영자 확정 2026-09-08, CU 협의 완료 — CCD 의 **dark reference columns** 를 읽은 값).  G 견본의 두 카드 값을 `PRESCNX=16` · `OVRSCNX=0` 으로 고치고 comment 를 `Dark reference columns per amplifier`(`PRESCNX`) · `Overscan columns per amplifier (none)`(`OVRSCNX`)로 바꿨다(괄호는 v1.16 결정 2 로 걷혔다 — 지금은 `Overscan columns per amplifier`).
     - ✅ **해소 (2026-09-24, v1.16 결정 5)** — 운영자가 카드를 지목했다: G 견본을 기동 기본값(`IMAGETYP` `'OBJECT'` · `OBJECT` `'guide'` · `EXPTIME` `2.0` — 형은 v1.16 결정 6)으로 바꿨다.  그 앞 기록: **규범은 v1.14 가 정했다** — guide `BIAS` 는 최소 노출이라 `EXPTIME` 이 0 이 아니다(10.3절 · ~~OI-24~~).  견본의 `EXPTIME=0` 은 임의 샘플값이라 운영자가 카드를 지목할 때만 고친다.  종전 기록: `EXPTIME=0` · `IMAGETYP='BIAS'` 시나리오 — guide 의미론상 `EXPTIME=0`(독출 간격 0)은 실현 불가한 견본값 — 목록 해소 때 현실 시나리오(예: 1초) 검토.
     ...
     - ✅ ~~COMMENT 2번("Map of CCD output channels, raw X ascending within each card")이 science 문구 그대로 — 골격 규칙(10.2)상 유지 가능하나 guide 는 카드가 하나라 "each card" 가 안 맞음, 문구 조정 선택.~~ — **해소 (2026-09-24, v1.16 결정 3)**: 견본 셋 모두 *"Map of CCD output channels, raw X ascending"* 으로.
...
- **절 구성이 구판과 다르다** — 구판 절 번호를 인용한 문서·코드 주석(`규격 5.7절` 등)은 현행 기준으로 재확인. ⚠️ **v1.4 에서 2.5절(Wrote 통보)이 삭제돼 절 번호가 또 바뀌었다**(2장은 2.1~2.4). `ics_sim` 쪽 참조 정리는 **완료**(2026-08-22, v1.3 정렬과 함께 — 아래 "다음 사람이 할 일" 3). ICD **v4.2** §12 의 위임 대상 갱신은 LEECU 몫으로 남아 있었다 — v4.2(2026-09-04)에서도 §2·§7·§12 가 구명 `KMT_CEU_Raw_FITS_Pair_Spec_v1.2.md` 를 가리켰다(세 곳).  ✅ **ICD v4.3(2026-09-23)이 그 참조를 걷고** `KMT_CEU_Raw_FITS_Specification_v1.13.md` 를 인용한다 — ⏳ v1.14 발행으로 그 파일이 `archive/` 로 가서 현행 경로로 다시 갱신할 것을 요청했다(통합 §3 — 요청 속 경로는 판마다 현행으로 고친다, 지금 v1.2 는 v1.16 경로다).
- 헤더 5장의 바이트 단위 정본은 **science 헤더 견본 pair**(`header_samples/KMTA.20260821.123456.{MK,NT}.fits.header.v1.16.txt`)다 — 구 "초안 헤더 v1.0 pair".  ⭐ **판 번호는 규격을 따라간다**(v1.10 에서 `header_samples/` 로 모으고 규격과 맞췄다) — 따로 '견본 승격' 판올림은 없다.

## 먼저 읽을 것

| 문서 | 지위 |
|---|---|
| `KMT_CEU_Raw_FITS_Specification_v1.16.md` | ✅ **현행 raw spec** — 최종 정의·규격 (science 1~8장 + guide 9·10장). 배경은 아래 원장·통합 문서로 링크 |
| `KMT_CEU_Raw_FITS_Header_and_Refs_in_MEF_Converter_v1.22.md` | **카드 판정 원장** (v1.22 = `OBSTYPE` 사용자 입력 카드 · geometry comment 괄호 표기 정리 · v1.21 = 발행본 재검토 정정 — 7장 `RDMODE` 결측 `'NC'` 등 · v1.20 = converter v2.5.0 재대조 — raw 에서 읽는 카드 · geometry 선언 대조 · 카드 값 하드 실패 둘.  판정 불변). **0장이 판정 준거다**(준거 순위 · converter 3상태 × ICD 규정/침묵 · 준거 공백 크기). converter 가 읽는 것 · 읽지 않는 것 · 도입 후보·확정 · 폐지된 것을 13장으로 정리했다 — 최근 구판은 `archive/` |
| `KMT_CEU_Raw_Rev_MEF_Impacts_and_Identity_v1.2.md` | **통합 문서** (v1.2 = `OBSTYPE` 뜻 개정 — MEF 쪽 계통 판별은 `DATASRC` · v1.1 = LEECU 요청 정정·보강 · §7-10 신설 · v1.0 = converter v2.5.0 재대조 · *(Draft)* 를 뗐다) — Part 1: LEECU 전달용 — C-항목 · 이름 대응 · ICD/정의서 개정 후보 · **§7 「MEF converter 및 PIPELINE 판단 필요 항목」**(7-1~7-10 — 판단은 여기, 나머지 절은 수정 요청) / Part 2: 번호·충돌·정체성 **파급 요약**(정본 = raw spec 2.3절 + D-016). 전신 v0.5~v1.1 은 `archive/`, v0.4·v0.2 는 git 이력·외부 백업 |
...
- ⛔ **발행 전 예고 문면을 발행된 문서에 남기지 않는다.** 12장 행에 *"이 판을 발행할 때 … 붙인다"* 같은 예고를 적어 두면 발행 뒤에도 그대로 남아 **발행본이 자기 상태를 거짓으로 말한다**(v1.12 행에서 실제로 났다). 발행 커밋에서 과거형(*"… 에 붙였다"*)으로 바꾸고 **발행 커밋 해시를 그 자리에 적는다.**
  ...
  | v1.15 | `ddb6e94` (`Raw FITS Spec v1.15 -- 정리판 (발행본 재검토 정오 + 문서 층위 정리)`) | **삭제됨 (2026-09-24, v1.16 발행)** |
  | **v1.16** | (이 판의 마지막 커밋) | **`raw-spec-v1.16` (현행)** |
