...
**v1.11** · 2026-09-06 · **현행** — ⚠️ **v1.10 발행본(2026-09-04)과 내용이 갈려 판을 끊은 것**이고 v1.10 최종 상태에서 **내용 변경은 없다** (12장).  v1.10 이 이틀 동안 받은 제자리 개정이 작지 않았다 — 10.1절(guide 노출 의미론) 전면 개정 · 5.6.2 HK 원천 · 용어 개명(기본 노출시간 / 설정 가능한 최소 노출시간) · OI-25/26/28.  ⭐ ACF 에 세운 *"같은 이름 다른 내용은 안 된다"* 를 규격 자신에게도 적용한다.  ⚠️ **v1.10 판올림 때 이 줄이 `v1.9 · 현행` 인 채로 남아 있었다** — 함께 바로잡는다.  이하는 이전 판들의 경위다.
...
| 연동 | 값 |
| --- | --- |
| 정본 헤더 견본 | [`header_samples/KMTA.20260821.123456.MK.fits.header.v1.11.txt`](header_samples/KMTA.20260821.123456.MK.fits.header.v1.11.txt) · [`…NT.…v1.11.txt`](header_samples/KMTA.20260821.123456.NT.fits.header.v1.11.txt) — 5장 카드 전량의 **바이트 단위 견본**(각 **180 레코드 = 값 카드 136 + COMMENT 8 + END 1 + 공백 35**, 정확히 5×2880 = 14,400 바이트 — `END` 는 145번째이고 그 뒤 35장은 블록을 채우는 공백 레코드다. ⚠️ **v1.10 에서 4블록 → 5블록이 됐다**: HK 카드 5장 신설로 값이 131 → 136 이 되어 144 레코드를 넘겼다). 메모장용 사본 **`…v1.11+LF.txt`**(14,585 B)는 카드마다 LF 를 넣고 끝에 `#EOF` 를 붙인 것으로, **LF 와 꼬리 `#EOF` 를 함께 걷어내면 정본과 바이트 동일**하다.  ⭐ **v1.10 에서 견본 6장을 `header_samples/` 로 모으고 이름을 통일했다** — 판 번호를 규격과 같은 `v1.10` 으로, 메모장 사본의 꼬리를 `_REFTEXT` 에서 **`+LF`** 로 (운영자 지시 2026-09-04) |
| guide 헤더 견본 | [`header_samples/KMTA.20260821.123456.G.fits.header.v1.11.txt`](header_samples/KMTA.20260821.123456.G.fits.header.v1.11.txt) — **v1.10 정본** (구 v0.0 확정 초안, 운영자 2026-08-30 — v1.10 에서 판 번호를 규격과 통일했다. **144 레코드 = 값 카드 128 + COMMENT 8 + END 1 + 공백 7**, 정확히 4×2880 = 11,520 바이트 — 패딩 2026-08-30, v1.10 에서 공백 12 → 7). ⭐ guide 는 패딩 여유가 있어 **4블록 그대로**다. 메모장용 사본 **`…v1.11+LF.txt`**(11,669 B — LF 와 꼬리 `#EOF` 를 함께 걷어내면 정본과 바이트 동일). ⏳ 10장과의 **카드 단위 대사 확인 목록은 [`SMC_CLAUDE.md`](SMC_CLAUDE.md)** 에 남아 있다 — 판 번호를 통일했다고 그 확인이 닫힌 것은 아니다 |
| 카드 판정 원장 (배경·경위) | [`KMT_CEU_Raw_FITS_Header_and_Refs_in_MEF_Converter_v1.17.md`](KMT_CEU_Raw_FITS_Header_and_Refs_in_MEF_Converter_v1.17.md) — 이하 **원장**. 카드별 계승/개칭/폐지 근거, converter 대조, 레거시 123개 전량 귀속 |
...
| `CTRL1CFG` | guide ACF 이름 `<SITE>_GUI_<유닛>_<시리얼>_<ACF판>` — **검출기조 접미사가 없다** (science 와의 차이). 현행 실물은 **9장 연동 표의 파일명**이다(`ACF판` 은 판올림마다 오른다 — 여기에 판 번호를 박지 않는다) | ✅ `acf/README.md` 이름 규칙 |
...
| 문서 | 위치 |
| --- | --- |
| 카드 판정 원장 | [`KMT_CEU_Raw_FITS_Header_and_Refs_in_MEF_Converter_v1.17.md`](KMT_CEU_Raw_FITS_Header_and_Refs_in_MEF_Converter_v1.17.md) |
