...
| guide 헤더 견본 (초안) | [`KMTA.20260821.123456.G.fits.header.v0.0.txt`](KMTA.20260821.123456.G.fits.header.v0.0.txt) — **v0.0 확정 초안** (운영자, 2026-08-30. **144 레코드 = 값 카드 123 + COMMENT 8 + END 1 + 공백 12**, 정확히 4×2880 = 11,520 바이트 — 패딩 2026-08-30). 메모장용 사본 `…_REFTEXT.txt`(11,669 B — LF 를 걷어내면 정본과 바이트 동일). 10장과의 **카드 단위 대사 정정 목록은 [`SMC_CLAUDE.md`](SMC_CLAUDE.md)** 에 있고, 해소 후 science 견본과 함께 **v1.1 로 정본 승격 예정**이다 |
...
전부 **문자열** (5.0절 형 규칙). 공급 계통 — `ICG RTD`(Archon 쪽) · `Radionode`(환경 센서 — **구칭 `Tapaculo`**, 운영자 지시로 2026-08-30 개명. 구판·아카이브 문서의 `Tapaculo` 표기는 같은 장치다. 5.8절 FSA). ⚠️ 셋째 계통이던 `standalone RTD`(별도 판독 장치)는 **v1.5 의 4장 폐지로 현행 수록 카드가 없다** (5.10절).
...
| 헤더 | 5장, 값 카드 131장 | **골격 동일, 값 카드 123장** — `CTRL2*`·`C2_*` 6장 미수록 · `CHMAP_*` 4장 → `CHMAP` 1장 · `IMGROT` 신설 (10.2절) |
...
- **카드 골격은 science 5장을 따르되 값 카드는 123장이다** (운영자 확정 견본 v0.0, 2026-08-30): 131장에서 **`CTRL2ID` `CTRL2SN` `CTRL2CFG` · `C2_TEMP` `C2_VOLT` `C2_CURR` 6장을 뺀다**(컨트롤러가 하나다 — 검토 중의 "`NC` 로 채우기" 안은 채택되지 않았다), **`CHMAP_LT/LB/RT/RB` 4장을 `CHMAP` 1장으로** 갈음하고, **`IMGROT` 1장을 신설**한다 — 123 = 131 − 6 − 3 + 1. 블록 순서·COMMENT 8장 골격과 형·sentinel·카드 폭 규칙(5.0절)은 그대로다.
