...
**v1.7** · 개정 2026-08-21 · v1.6 까지는 원천에서 기계 추출했다 — **v1.7 은 검토 확정분을 손으로 반영한 개정판이다** (생성기 부재 상태의 수기 개정 — 생성기를 재작성할 때 이 판의 손질을 승계해야 한다).
...
| **v1.7 개정 근거** | `__review/KMT_CEU_Raw_FITS_Header_and_Refs_in_MEF_Converter_v1.6_revision.docx`(운영자 개정) · `__review/KMTA.20260818.012345.MK.fits.header.txt`(확정 초안) · `__review/AMPCARD.txt` |
...
| **Archon ICS 도입 후보·확정** — 규격 v1.2 / `ics_sim` / 확정 초안이 새로 들였고 converter 는 읽지 않는다 | **51** (후보 37 + v1.7 확정 신규 14) | **7장** |
...
| **Source (`*` default)** | 값의 출처 — `ICS INI`(설정) · `ICS code`(취득 SW 산출) · `ICS generating`(취득 SW 생성) · `user input/selection` · `TCS relay` / `AUX relay`(TC 중계) · `Archon`(컨트롤러). `*` 는 기본 출처 |
...
| `BUNIT` | O | O | `BUNIT` `+amp` | `ADU` | ICS generating |
| `DETID` | O | O | ((TBD)) | `MK` / `NT` | ICS generating |
| `DETECTOR` | O | O | `DETECTOR` | `'e2v CCD290-99'` | ICS INI |
| `CCDXBIN` | O | O | `CCDXBIN` | `1` (2 & 3 reserved) | ICS / user selection |
| `CCDYBIN` | O | O | `CCDYBIN` | `1` (2 & 3 reserved) | ICS / user selection |
...
| Raw Keywords | Raw Legacy | Raw Archon | Use in MEF | Value (`*` default) | Source (`*` default) |
| --- | :---: | :---: | --- | --- | --- |
| `EXPTIME` | O |  | `EXPTIME` | seconds | ICS code / user input |
| `DARKTIME` | O |  | `DARKTIME` | seconds | ICS code |
| `TSHOPEN` | O |  | `TSHOPEN` | `'HH:MM:SS.mmm'` | ICS code |
| `TSHSHUT` | O |  | `TSHSHUT` | `'HH:MM:SS.mmm'` | ICS code |
| `TIMESYS` | O |  | `TIMESYS` | `UTC`* | ICS code |
...
| Raw Keywords | Raw Legacy | Raw Archon | Use in MEF | Value (`*` default) | Source (`*` default) |
| --- | :---: | :---: | --- | --- | --- |
| `CTRL1ID` | **X** |  | `CTRL1ID` | 예) `'ARCHON-SSO-1'` | ICS INI |
| `CTRL1SN` | **X** |  | `CTRL1SN` |  | ICS INI |
| `CTRL1FW` | **X** |  | `CTRL1FW` |  | Archon |
| `CTRL2ID` | **X** |  | `CTRL2ID` | 예) `'ARCHON-SSO-2'` | ICS INI |
| `CTRL2SN` | **X** |  | `CTRL2SN` |  | ICS INI |
| `CTRL2FW` | **X** |  | `CTRL2FW` |  | Archon |
| `CTRLVER` | **X** |  | `CTRLVER` |  |  |
| `TIMVER` | **X** |  | `TIMVER` |  |  |
| `BIASVER` | **X** |  | `BIASVER` |  |  |
| `CLKVER` | **X** |  | `CLKVER` |  |  |
| `XTALKVER` | **X** |  | `XTALKVER` |  | caldb (C-14, raw 미기재) |
| `REFVER` | **X** |  | `REFVER` |  | caldb (C-14, raw 미기재) |
| `CATVER` | **X** |  | `CATVER` |  | caldb (C-14, raw 미기재) |

> 구 `없을 때`(converter 기본값): `CTRL1*`·`CTRL2*` `"UNKNOWN"` · `CTRLVER "ARCHON-v1.0"` · `TIMVER "TIM-v1.0"` · `BIASVER "BIAS-v1.0"` · `CLKVER "CLK-v1.0"` · `XTALKVER "UNMEASURED"` · `REFVER`/`CATVER` `"N/A"`. 버전 문자열 4종의 Value/Source 빈칸은 **근거 순환 미해소**(키워드맵 검토 항목 4) 때문이다 — 추적 대상·부여 주체·증가 조건이 정해져야 채울 수 있다.
...
| Raw Keywords | Raw Legacy | Raw Archon | Use in MEF | Value (`*` default) | Source (`*` default) |
| --- | :---: | :---: | --- | --- | --- |
| `TCSLINK` | O |  | `TCSLINK` |  | TCS relay |
| `TCSARC` | O |  | `TCSARC` |  | TCS relay |
| `TCSQDATE` | O |  | `TCSQDATE` |  | TCS relay |
| `TCSUDATE` | O |  | `TCSUDATE` |  | TCS relay |
| `RADECSYS` | O |  | `RADECSYS` | `ICRS`* | ICS code |
| `RA` | O |  | `RA` `+amp` | `'hh:mm:ss.ss'` | TCS relay |
| `DEC` | O |  | `DEC` `+amp` | `'±dd:mm:ss.s'` | TCS relay |
| `EQUINOX` | O |  | `EQUINOX` | `2000.0`* | ICS code |
| `HA` | O |  | `HA` `+amp` | `'±hh:mm:ss'` | TCS relay |
| `ST` | O |  | `ST` `+amp` | `'hh:mm:ss'` | TCS relay |
| `SECZ` | O |  | `SECZ` `+amp` |  | TCS relay |
| `ALT` | O |  | `ALT` `+amp` |  | TCS relay |
| `AZ` | O |  | `AZ` `+amp` |  | TCS relay |
| `TCSDRIVE` | O |  | `TCSDRIV` — `v("TCSDRIV", "")` ← fallback |  | TCS relay |
| `TELMOVE` | O |  | `TELMOVE` |  | TCS relay |
...
| Raw Keywords | Raw Legacy | Raw Archon | Use in MEF | Value (`*` default) | Source (`*` default) |
| --- | :---: | :---: | --- | --- | --- |
| `AUXLINK` | O |  | `AUXLINK` |  | AUX relay |
| `AUXARC` | O |  | `AUXARC` |  | AUX relay |
| `AUXQDATE` | O |  | `AUXQDATE` |  | AUX relay |
| `AUXUDATE` | O |  | `AUXUDATE` |  | AUX relay |
| `FSSTAT` | O |  | `FSSTAT` |  | AUX relay |
| `FILTOP` | O |  | `FILTOP` |  | AUX relay |
| `FILNUM` | O |  | `FILNUM` |  | AUX relay |
| `FILTER` | O |  | `FILTER` `+amp` |  | AUX relay |
| `SHUTOP` | O |  | `SHUTOP` | `NC` / `STANDBY` / `OPENING` / `OPENED` / `CLOSING` / `RELOADING` / `ERROR` | AUX relay |
| `SHUTTER` | O |  | `SHUTTER` | `OPEN` / `CLOSED` / `UNKNOWN` | AUX relay |
| `FSATEMP` | **X** |  | `FSATEMP` |  | AUX relay (도입 미결 — 검토 항목 9) |
| `FSAHUM` | **X** |  | `FSAHUM` |  | AUX relay (도입 미결 — 검토 항목 9) |
| `FSADEW` | **X** |  | `FSADEW` |  | AUX relay (도입 미결 — 검토 항목 9) |
| `FSAALRM` | **X** |  | `FSAALRM` |  | AUX relay (도입 미결 — 검토 항목 9) |
| `FASTAT` | O |  | `FASTAT` |  | AUX relay |
| `FAFOCUS` | O |  | `FAFOCUS` |  | AUX relay |
| `FATILTNS` | O |  | `FATILTNS` |  | AUX relay |
| `FATILTEW` | O |  | `FATILTEW` |  | AUX relay |
| `FAPOSS` | O |  | `FAPOSS` |  | AUX relay |
| `FALIMS` | O |  | `FALIMS` |  | AUX relay |
| `FAPOSE` | O |  | `FAPOSE` |  | AUX relay |
| `FALIME` | O |  | `FALIME` |  | AUX relay |
| `FAPOSW` | O |  | `FAPOSW` |  | AUX relay |
| `FALIMW` | O |  | `FALIMW` |  | AUX relay |
...
| Raw Keywords | Raw Legacy | Raw Archon | Use in MEF | Value (`*` default) | Source (`*` default) |
| --- | :---: | :---: | --- | --- | --- |
| `DSSTAT` | O |  | `DSSTAT` |  | AUX relay |
| `DSUP` | O |  | `DSUP` |  | AUX relay |
| `DSLW` | O |  | `DSLW` |  | AUX relay |
| `DSSAF` | O |  | `DSSAF` |  | AUX relay |
| `DSAUTO` | O |  | `DSAUTO` |  | AUX relay |
| `DSALT` | O |  | `DSALT` |  | AUX relay |
| `DSAZ` | **X** |  | `DSAZ` |  | AUX relay (신설) |
| `DSTELALT` | **X** |  | `DSTELALT` |  | ICS 재기록 (AUX 실선 `DSTEL` → `DSTELALT`) |
| `DSTELAZ` | **X** |  | `DSTELAZ` |  | AUX relay (신설) |
| `DALTERR` | **X** |  | `DALTERR` |  | AUX relay (신설) |
| `DAZERR` | **X** |  | `DAZERR` |  | AUX relay (신설) |
| `MCSTAT` | O |  | `MCSTAT` |  | AUX relay |
| `MCPOS` | O |  | `MCPOS` |  | AUX relay |
...
| Raw Keywords | Raw Legacy | Raw Archon | Use in MEF | Value (`*` default) | Source (`*` default) |
| --- | :---: | :---: | --- | --- | --- |
| `CHSTAT` | O |  | `CHSTAT` |  | AUX relay |
| `ENSTAT` | O |  | `ENSTAT` |  | AUX relay |
| `ENFAN` | O |  | `ENFAN` |  | AUX relay |
| `CCDTEMP` | O |  | `CCDTEMP` | degC | ICS generating (`CCDTEMP1`·`CCDTEMP2` 평균) |
| `DEWPRES` | O |  | `DEWPRES` |  | AUX relay |
| `PT30N1` | O |  | `PT30N1` |  | AUX relay |
| `PT30N2` | O |  | `PT30N2` |  | AUX relay |
| `CHARCOAL` | O |  | `CHARCOAL` |  | AUX relay |
| `AIR_IN` | O |  | `AIR_IN` |  | AUX relay |
| `AIR_OUT` | O |  | `AIR_OUT` |  | AUX relay |
| `GLYC_IN` | O |  | `GLYC_IN` |  | AUX relay |
| `GLYC_OUT` | O |  | `GLYC_OUT` |  | AUX relay |
...
>
> **v1.7 에서 바뀐 것**: 확정 초안(`__review/AMPCARD.txt` · 헤더 초안)이 들인 **신규 14장을 추가**하고(`AMPNAX1/2` `IMAGEX/Y` `PRESCNX/Y` `OVRSCNX/Y` `CHMAP_*` `FPAID` `ORIGNAME` — 전부 `O`), 기존 후보의 도입 여부를 검토 결과대로 판정했다. 파생 가능해진 카드(`NXTILE` `CCDCOLS` `CCDROWS`)와 대체된 카드(`AMPMAP`), 규격 조항으로 이관하는 카드(`ROWORDR`)는 `X` 다.
...
| --- | :---: | --- | --- |
| `ACFFILE` · `CTR_CFG` |  | 적용된 Archon 설정 파일 | **설정 provenance 의 유일한 포인터**인데<br>MEF 목적지가 없다(ACT-011). 이름 단일화 미결 — 규격·`ics_sim` 은 `ACFFILE` |
| `READMODE` |  |  | ⚠️ **값 충돌** — 초안은 `'FAST'`(독출 속도), 규격·MEF 는 `'64AMP'`(구조 선언).<br>같은 이름 다른 뜻 — **이름 분리 필요**(예: 속도는 `READSPD` 류) |
...
| `OVERSCNX` | 레거시 실측 `32`, converter 상수 `48` — **이름은 같은데 값이 다르다.** 게다가 `X` 만 있고 중앙 Y overscan 을 담을 자리가 없어, 양방향 overscan 을 한 이름으로 표현하지 못한다(11.3 · 12.3) | **`OVRSCNX`/`OVRSCNY` 두 장 — v1.7 에서 확정** (X = amp 당 48 · Y = amp 당 84, frame-center side). 같은 이유로 `PRESCANX`(레거시 27 ↔ 신규 0)도 **`PRESCNX`** 로 개칭하고 `PRESCNY` 를 짝으로 신설했다(7장) |
...
레거시에 없던 카드는 이 표 밖이다 — converter 가 읽는 26개는 3장에 `X` 로, 도입 후보·확정 51장은 7장에 있다.
...
- **이름은 같은데 뜻이 달라진 카드 문제는 v1.7 개칭으로 닫혔다** — `OVERSCNX`→`OVRSCNX` · `PRESCANX`→`PRESCNX` · `OVERSCNY`→`OVRSCNY`(뜻 재정의 겸 개명) · `NAMPS` 폐지(8.1절). 남은 동명이의는 둘이다: **`DETID`**(뜻 재정의 유지 — 3.1 comment 가 새 뜻을 명시) · **`READMODE`**(값 충돌 `FAST` vs `64AMP` — 이름 분리 미결, 7장).
...
- **7장 51장은 어긋나도 드러나지 않는다** — converter 가 읽지 않으므로 MEF 에 흔적이 남지 않는다. 확정(`O`) 카드도 converter 쪽 대조(C-5/C-11/C-13)가 붙기 전까지는 같은 처지다.
