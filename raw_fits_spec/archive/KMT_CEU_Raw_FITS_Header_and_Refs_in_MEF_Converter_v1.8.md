...
| **Archon ICS 도입 후보·확정** — 규격 v1.2 / `ics_sim` / 확정 초안이 새로 들였고 converter 는 읽지 않는다 | **57** (후보 39 + v1.7 확정 신규 14 + v1.8 신설 4). 구 원장 "37" 은 v1.3 `READMODE` · v1.4 `NAMPDET` 추가분을 재산정하지 않은 값이라 v1.8 에서 실측 재산정했다 | **7장** |
...
## 7. Archon ICS 도입 후보·확정 카드 (카드 57장 · 표 54행)
...
레거시에 없던 카드는 이 표 밖이다 — converter 가 읽는 26개는 3장에 `X` 로, 도입 후보·확정 57장은 7장에 있다.
...
- **7장 57장은 어긋나도 드러나지 않는다** — converter 가 읽지 않으므로 MEF 에 흔적이 남지 않는다. 확정(`O`) 카드도 converter 쪽 대조(C-5/C-11/C-13)가 붙기 전까지는 같은 처지다.
