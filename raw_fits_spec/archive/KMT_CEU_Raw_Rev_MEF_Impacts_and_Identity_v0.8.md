...
- ✅ **파일명 `<SITE>` 넷째 코드 개정 (D-017, 2026-08-25) — 반영 완료 (2026-09-04).** `KMTT`(TESTBED) → **`KMTK`(KASI)**. converter v2.4.0 이 정규식 `^(KMTC\|KMTS\|KMTA\|KMTK)\.`·`SITE_PREFIX`/`OBS_PREFIX`·L0 MEF prefix `kmtk` 를 반영했고 **ICD 는 v4.2 로 판올림**됐다(§2.1 표·개정 이력). **형식(필드 폭·구분자·6자리 zero-padding)은 불변**이고 바뀐 것은 코드 하나다.
