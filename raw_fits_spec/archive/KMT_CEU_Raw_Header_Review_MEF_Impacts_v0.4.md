...
**v0.4 (Draft)** · 2026-08-22 · raw 헤더 검토(ACT-011)가 MEF 쪽 3자 — ICD v4.1(1위 준거) · MEF keyword 정의서 v1.0 · converter v2.2.0 — 에 미치는 개정 필요 사항의 전달 목록

> **v0.4 에서 바뀐 것 — 운영자 3차 개정(Header_and_Refs v1.9) 반영**: ① **`READMODE` 값 충돌 종결** — raw 쪽은 **`RDMODE` 로 개명 도입**(독출 속도 모드 선언), MEF `READMODE`(`'64AMP'`)는 그대로: 3장 ICD 개정 후보의 "이름 분리 필요" 항목을 해소 표시. ② **raw 신설 카드 `CAMVER` · `RDMODE` · `C1_`/`C2_` 계열(`TEMP`·`VOLT`·`CURR`)** — converter 미독이라 당장 동작 불변, 도입 시 pass-through 후보로 2장 대응표에 추가. ③ **C-후보 신설** — MEF `VOLTINFO`/`TELEMETRY` 를 raw `Cn_VOLT`·`Cn_CURR`·`Cn_TEMP` 에서 채우는 안(현행 placeholder·C-18 대체 경로). ④ **raw 미도입 확정 반영** — `CTRLTAG`·`PAIRFILE`·`OSCNPATT`·`RDDIRT`/`RDDIRB`·`MIDOSC*`·전압 색인 계열: C-5/C-12 문구를 "규격 조항 + 표본 검증" 기반으로 조정. raw 쪽 근거는 Header_and_Refs **v1.9**.
...
> `mef_converter/`는 읽기 전용(LEECU 소관)이므로 이 문서는 **변경 요청 목록**이지 변경 자체가 아니다. raw 쪽 근거는 `KMT_CEU_Raw_Numbering_and_Identity_v0.2.md`(번호 · 정체성 · 충돌)와 검토 세션 기록([`SMC_CLAUDE.md`](SMC_CLAUDE.md) ▶절)이다. raw 쪽 카드 이름 · 값은 아직 Draft이며, 확정 시점(D-등재 · 규격 재작성판)에 이 문서도 판을 올린다.
...
| `CTRL1CFG` / `CTRL2CFG` (신설) | (없음 — raw 전용 설정 포인터) | 버전 문자열 6장을 귀속 (v1.9 3.3절) |
| `CAMVER` (신설, v1.9) | (없음 — 도입 시 pass-through 후보) | 카메라 시스템 버전 선언. converter 미독 |
| `RDMODE` (신설, v1.9) | MEF `READMODE`(`'64AMP'`) 와 **별개 — 대응 없음** | raw 독출 속도 모드 선언. 구 `READMODE` 이름 충돌의 해소형(3장 참조). converter 미독 |
| `C1_TEMP`·`C1_VOLT`·`C1_CURR` / `C2_*` (신설, v1.9) | MEF `VOLTINFO`/`TELEMETRY` (C-후보, 1장) | 컨트롤러별 텔레메트리(구 `BCKTEMP` 확장). converter 미독 — 도입 시 pass-through 후보 |
...
| 문서 | 위치 |
| --- | --- |
| raw 번호 · 정체성 · 충돌 처리 (Draft) | [`KMT_CEU_Raw_Numbering_and_Identity_v0.2.md`](KMT_CEU_Raw_Numbering_and_Identity_v0.2.md) |
...
| converter가 읽는 것 · 읽지 않는 것 | [`KMT_CEU_Raw_FITS_Header_and_Refs_in_MEF_Converter_v1.9.md`](KMT_CEU_Raw_FITS_Header_and_Refs_in_MEF_Converter_v1.9.md) |
