...
**v1.5** · 2026-08-25 · **현행** — 2026-08-18~22 전면 재검토(확인 요망 11건 전량 종결 · D-016 등재)를 반영한 재작성판. v1.4 가 운영자 1~4장 검토 반영판이고, **v1.5 부터 5장(헤더 keyword) 검토분을 싣는다**. 구 문서명 "KMT-CEU Raw FITS Pair 규격"(v1.2, `archive/`)을 개명·대체한다. 이 문서를 **raw spec(로우 스펙)** 이라 부른다.
...
`Cn_TEMP` · `Cn_VOLT` · `Cn_CURR` 은 **공백으로 구분한 나열**이고 **자리(토큰 위치) 자체가 항목을 뜻한다** — 값에 이름표가 없으므로 읽는 쪽은 아래 표로만 해석한다. 자리 순서는 규격 사항이며, 모듈 구성이 바뀌면 `CAMVER`(HW) · `CTRLnCFG`(설정) 범프로 드러난다 (4.3절 포장 규범 조항과 같은 원리).
