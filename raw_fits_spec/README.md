...
| `KMT_CEU_Converter_Raw_Reads_v1.1.md` | — | **사본이 아니라 생성물이다** (2026-08-18 추가, v1.1). 세 원천에서 기계 추출해 **레거시 raw 123개를 전량 귀속**시킨다 — converter 가 **읽는 카드 104개**(MEF 목적지·기본값·레거시 유무), raw 에만 있어 **읽지 않는 53장**, D-013 이 **폐지한 17장**(`DETID` 등). 원천이 바뀌면 다시 생성한다 |
...
**국문 ICD와 `KMT_CEU_Converter_Raw_Reads_v1.1.md` 를 뺀 나머지는 사본이며 기준본은 원본 위치의 것이다.** 원본이 개정되면 이 사본도 함께 갱신하거나 삭제한다. 국문 ICD 는 유일본이고, `KMT_CEU_Converter_Raw_Reads_v1.1.md` 는 converter 소스·검토 문서·D-013 에서 뽑은 **생성물**이라 둘 다 대조 원본이 없다.
