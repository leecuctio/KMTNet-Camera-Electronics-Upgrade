...
> ✅ **현행 규격: [`KMT_CEU_Raw_FITS_Specification_v1.12.md`](KMT_CEU_Raw_FITS_Specification_v1.12.md) — "raw spec"** (2026-09-06, v1.12).
...
> ⏭️ **판올림 이월 대기 2건** (구 "v1.9 대기 5건" — ~~`CCDTEMP` comment 의 chip 귀속(`M`) 제거~~ 는 **2026-08-30 운영자 지시로 조기 실행**, 견본 3장 제자리 반영): `CAMVER` 범프 규범 명시(듀어 RTD 배치 변경 포함) · 바이어스 측정값의 헤더 카드 배치(D3).  ⭐ ~~`OI-18` 폐기~~ 는 **v1.10 에서 이행**됐고, ~~`RDMODE` 결측값 등재~~ 는 **v1.12 에서 닫혔다**(INI 전용 · 결측 `'UNKNOWN'` · ACF 유도 제거, 5.5절). **헤더 견본 v1.0 → v1.1 승격 라운드**(guide 견본 v0.0 확정 대사와 함께)에서 처리 예정. 상세는 [`../ics_archon/SMC_CLAUDE.md`](../ics_archon/SMC_CLAUDE.md) "규격 쪽 후속".
>
...
| 구분 | 문서 | 버전 | 상태 |
| --- | --- | --- | --- |
| **Raw spec (현행)** | [`KMT_CEU_Raw_FITS_Specification_v1.12.md`](KMT_CEU_Raw_FITS_Specification_v1.12.md) | **v1.12** | ✅ 현행 (v1.10 발행본과 내용이 갈려 판을 끊었다 — 내용 변경 없음) |
| 카드 판정 원장 | [`KMT_CEU_Raw_FITS_Header_and_Refs_in_MEF_Converter_v1.18.md`](KMT_CEU_Raw_FITS_Header_and_Refs_in_MEF_Converter_v1.18.md) | **v1.18** | ✅ 현행 (raw spec v1.10 동반) |
| MEF·파이프라인 파급 (통합 문서) | [`KMT_CEU_Raw_Rev_MEF_Impacts_and_Identity_v0.9.md`](KMT_CEU_Raw_Rev_MEF_Impacts_and_Identity_v0.9.md) | **v0.9** | ✅ 현행 (raw spec v1.10 동반) |
...
| **Guide raw** (v1.9 신설, 규격 9·10장) | guide controller 1대 → 노출 1회 = **파일 1개** `<SITE>.<YYYYMMDD>.<NNNNNN>.G.fits`, **4224 × 1033** (CCD47-20 × 4, 채널 8), 셔터 무관 노출(`EXPTIME` = 독출 개시 간격, 첫 프레임 폐기), 헤더는 science 골격에 **값 카드 128장**(`CTRL2*`·`C2_*` 미수록 · `CHMAP` 1장 · `IMGROT` 신설). 소비자는 [`../gmon/`](../gmon/) (MEF 경로 없음) |
