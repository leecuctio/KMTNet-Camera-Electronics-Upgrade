...
> ✅ **현행 규격: [`KMT_CEU_Raw_FITS_Specification_v1.3.md`](KMT_CEU_Raw_FITS_Specification_v1.3.md) — "raw spec"** (2026-08-22 발행). 2026-08-18~22 전면 재검토(확인 요망 11건 전량 종결 · D-016 등재)의 재작성판이며, 구 "Raw FITS Pair 규격" v1.2 를 개명·대체한다(구판은 `archive/`).
>
> ⚠️ **절 구성이 구판과 다르다** — 구판 절 번호(`규격 5.7절` 등)를 인용한 문서·코드 주석은 v1.3 기준으로 재확인할 것. ICD v4.1 §12 와 `ics_sim` 구현의 참조 정리는 구현 일감이다.

| 구분 | 문서 | 버전 | 상태 |
| --- | --- | --- | --- |
| **Raw spec (현행)** | [`KMT_CEU_Raw_FITS_Specification_v1.3.md`](KMT_CEU_Raw_FITS_Specification_v1.3.md) | v1.3 | ✅ 현행 |
...
| 경로 | 내용 |
| --- | --- |
| `KMT_CEU_Raw_FITS_Specification_v1.3.md` | ✅ **현행 raw spec (2026-08-22)** — 파일 구조 · 파일명(D-011/D-014) · 충돌·정체성(D-016) · geometry(포장 규범 조항 + amp 전수 표 64행) · 헤더 keyword 135장(초안 v1.0 pair 기준) · MEF/파이프라인 연동 요점 · 검증 체크리스트 · OI. 배경·경위는 원장(Header_and_Refs)과 통합 문서로 링크 |
