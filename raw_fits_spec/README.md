...
> ✅ **현행 규격: [`KMT_CEU_Raw_FITS_Specification_v1.8.md`](KMT_CEU_Raw_FITS_Specification_v1.8.md) — "raw spec"** (2026-08-26). 2026-08-18~22 전면 재검토(확인 요망 11건 전량 종결 · D-016 등재)의 재작성판(v1.3) → 운영자 1~4장 검토 반영(v1.4) → 5장(헤더 keyword) 검토 개시분(v1.5) → **v1.6 이 노출 정체성 카드를 개정한다** (`ORIGNAME` → **`EXPID`** · `FILENAME` comment). 구 "Raw FITS Pair 규격" v1.2 를 개명·대체한다(구판은 `archive/`).
>
...
| 구분 | 문서 | 버전 | 상태 |
| --- | --- | --- | --- |
| **Raw spec (현행)** | [`KMT_CEU_Raw_FITS_Specification_v1.8.md`](KMT_CEU_Raw_FITS_Specification_v1.8.md) | v1.7 | ✅ 현행 (1~4장 검토 반영 + 5장 검토 개시분 + 정체성 카드 개정) |
...
| 주체 | 파일 | 상태 |
| --- | --- | --- |
| **신규 ICS (시뮬)** | [`../ics_sim/ics_sim/rawpair.py`](../ics_sim/ics_sim/rawpair.py)(이름) + [`rawhdr.py`](../ics_sim/ics_sim/rawhdr.py)(카드) + [`config.py`](../ics_sim/ics_sim/config.py)(사이트 — `[node] observatory` 한 줄, D-017. 구 `siteid.py` 는 IP 판정 폐지와 함께 삭제됐다) + `sequencer._store()` + `hardware/sim.py`의 `write_frame()` | **동작 중** — 파일명(D-014)·저장/통보 분리·sentinel·헤더 카드를 구판 기준으로 구현했고, v1.13 확정분 일부(ICSBUILD 신형식·LEDFLASH [ms]·EXPTIME 조건부 형·HK 문자열·ICS INI 편집성·ORIGIN 유도)를 반영했다(테스트 327). **v1.3 정렬 잔여 = 구현 일감**: D-016 충돌 처리(rawpair), 정체성 카드 재편(UNIQNAME/PAIRFILE/CHIP* 제거), 컨트롤러 블록(BCKTEMP→Cn_*, 버전 카드 제거), 신설 HK·돔 카드, 절 번호 참조 정리 |
...
남은 open item 은 **raw spec 8장**에 있다 — 포장 조항 준수 검증(OI-3) · 중앙 overscan 분배(OI-4) · binning(OI-5) · checksum(OI-7) · 셔터 반영 지연(OI-13) · **X overscan 4:4 vs 5:3(OI-15)** · Tapaculo 원값 포맷(OI-16) · e2v 데이터시트 대응(OI-17) · NT `CCDTEMP` 귀속(OI-18). **전부 실기 실측·자료 확보·협의가 있어야 닫힌다.**
