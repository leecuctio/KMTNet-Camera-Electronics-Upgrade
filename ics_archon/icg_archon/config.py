...
class IcgCfg:
    ...
    #: 이 타이밍 스크립트에서 계산한 하한(R2610~R2615: 1.251 s -- `NoIntMS` 항은 10.3 실측
    #: 1% 적중, 트랜스퍼·독출 항은 ⏳ 첫 guide 구동 실측).  하한 아래 값을 두면
    #: 클램프가 무력해지므로 ini(2.0)와 같이 하한 위로 둔다 (종전 1.0 은 근거 없는
    #: 잠정값이었다 -- DevNote 9.10·9.15).
    #: **운영 하한** [s] -- 이보다 짧은 `EXPTIME` 요청은 이 값으로 접는다 (운영자 확정
    #: 2026-09-05: 하드웨어 하한 1.2506 s 위에 여유를 두어 1.3).  ⚠️ 하드웨어 하한
    #: (`GuideBackend.frame_floor()`, ACF 계산값)과 **다른 물건**이다 -- `IntMS =
    #: EXPTIME - 하드웨어 하한` 의 하한은 계산값이어야 헤더가 참이고, 이 값은 그 위에
    #: 얹는 정책이다.  하드웨어 하한보다 작게 두면 하드웨어 하한이 이긴다.  ACF 계산이
    #: 없을 때(스크립트 없는 시험 ACF · 대역)는 이 값이 하한 노릇을 겸한다.
