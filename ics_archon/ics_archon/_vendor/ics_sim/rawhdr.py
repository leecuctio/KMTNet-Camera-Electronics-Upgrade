...
근거는 현행 raw spec(`raw_fits_spec/KMT_CEU_Raw_FITS_Specification_v<판>.md`) 4·5장이고
...
CAMVER = 'CEU-v2.1'      # **HW·성능상 변경이 있을 때만 올린다**.  ⭐ 범프 사유는
                         # **셋**이다 (5.2절, normative): ① 포장(4.3절 조항 · 축별 합이
                         # 바뀌는 AMPNAX1/2) ② `Cn_*` 모듈 나열 자리(5.6.1절) ③ **듀어
                         # RTD 의 배치·귀속**(10.4절 채널 표) -- ③은 헤더 어디에도 자취를
                         # 안 남기므로 센서를 옮긴 사람이 여기를 안 올리면 아무도 모른다.
                         # ⚠️ science·guide 두 INI 에 함께 적는다.  FW·ACF 만 바뀐 것은
                         # CTRLxCFG 범프다.  값↔구성 대장은 아직 없다 (OI-29).
...
#: `NC` 가 아니다.
#:
#: ✅ **규격이 등재했다** (v1.12, 5.5절 `RDMODE` 행 -- *"정의돼 있지 않으면 `'UNKNOWN'`"*,
#: 운영자 확정 2026-09-06 · 10.3절은 *"5.5절과 같다"*).  한때 코드가 먼저 간 예외였고,
#: 그 이유는 값이 `NORMAL` 로 실리면 **거짓말이 조용히 아카이브에 들어가기** 때문이다.
RDMODE = 'UNKNOWN'
