...
def cfg_name_from_acf(path: str) -> str:
    ...
    맡긴다.  ⛔ **여기서 값을 지어내지 않는다** -- 지어내면 "유도 실패" 와
    "정말 그 값" 이 헤더에서 구별되지 않는다.
    """
...
class ArchonCfg:
    ...
    #: -- 종전에는 확인이 대기 안에 있어 0 으로 두면 확인까지 사라졌다.
    poweron_wait: float = 0.0
    ...
    #:
    #: ⭐ **기본이 `true` 인 이유는 값이 있어서**다 -- 주기 12.78초 · fetch 3.4초면
    #: fetch 중 프레임 경계가 걸릴 확률 ≈27% 이고, 그때 잠금이 없으면 엔진이
    ...
    #: 프레임 주기(12.78초) 안에 풀어야 한다 -- 넘으면 다음 장이 덮인다.
    ...
    #: -- 주기 12.78초에 경계가 걸릴 확률 ≈27% 다 (DevNote 10.6).
    ...
    #: 프레임 주기(`MIN_FRAME_PERIOD` 12.78초)를 넘으면 다음 장이 앞 장을 덮는다
    #: (2026-09-01 `--hold 20` 실측, DevNote 10.4·10.6).  그래서 **주기 아래**여야
    #: 하고 `validate()` 가 본다.  실측 FETCH 는 99~107 MiB/s (344 MiB 에 3.2~3.5초,
    #: 두 유닛) -- 기본 10초면 2.9배 여유다.  ⚠️ **배포 ini 는 11** 이다
    #: (여유 1.78초, 2026-09-13 바닥이 12.78 로 내려가며 같이 내렸다).
    #:
    ...
    #: `_cross_checks()` 가 기동에서 본다.  현행 바닥(주기 12.78초) 기준
    #: 25초 창에는 **2개**면 충분하다 -- 그때 "창이 터진 뒤에도 자료가 남는"
    #: 구간이 약 3.96초다 (N x 12.78 - 25 + 3.4).  ⚠️ 주기가 13.27 이던 때는
    #: 4.9초였다 -- 사강이 빠지며 그만큼 얇아졌다.
    fetch_buffers: int = 2
...
def validate(cfg: ArchonCfg, ccds: tuple[str, ...],
    ...
    if cfg.lock_buffer and fetch_cap >= MIN_FRAME_PERIOD:
        notes.append(
            ...
            '주기 아래(예 11)로 적을 것' % (fetch_cap, MIN_FRAME_PERIOD))
...
#:
#: ⭐ **2026-09-13 에 13.27 -> 12.78 로 내렸다** -- 셔터 재설계로 **BIAS 의 사강이
#: 0 이 되었기 때문**이다 (DevNote 11.85-(3)).  `trigger(noint_ms=...)` 가 이제
#: `NoIntMS` 를 **노출마다** 쓰고 BIAS 에는 **0** 을 실으므로, 바닥은 **독출 시간
#: 그 자체**다.
#:
#: 종전 13.27 은 실측 2026-09-01 (두 유닛 101·113, `tools/ics_archon_buftest.py`:
#: **368 행/초** x 4700 행 = 독출 12.77초 + 그때 ACF 상수였던 `NoIntMS` 0.5초,
#: `IntMS=0`, DevNote 10.4)이고 **그 조건에서는 참이다** -- 실측을 버리는 것이
#: 아니라 조건이 바뀐 것이다.  더 종전 12.0 은 운영자 labtest 자료(2026-08-29)의
#: "순수 readout 11.3초" 에서 왔다.
#:
#: ⚠️ **12.78 은 계산값이다** -- 한 행 271,814틱 x 4700 행 = 12.7753초
#: (DevNote 11.85-(10)).  같은 틱 모형이 옛 실측을 13.2753 대 13.27 로 **5 ms
#: 안에서** 재현했으므로 믿을 만하지만, ⛔ **벤치에서 BIAS 주기를 다시 재서
#: 확정할 것.**
#:
...
#: 가 붙는다(5200 ms 면 주기가 18초 대).  이 상수는 **바닥**이지 전형값이 아니다.
#:
...
#: 이보다 길면 기동 검사가 알린다.  ⭐ 바닥이 내려가면 이 검사가 **조여진다** --
#: 그래서 배포 ini 의 `fetch_timeout` 도 12 -> **11** 로 같이 내렸다(운영자
#: 2026-09-13, 여유 1.78초 · 실측 FETCH 3.2~3.5초의 3.1배).
#:
#: ⭐ **이제 기계가 대사한다** (2026-09-14, `archon/acftiming.py`) -- 백엔드가
#: 기동에서 ACF 의 타이밍 스크립트를 틱 단위로 돌려 주기 바닥을 셈하고
#: (R2613: **12.7762 s** = 독출 12.7753 + 쓸기 0.93 ms) 이 상수와 ±0.05 s 안인지
#: 본다.  ⛔ **바닥이 상수보다 짧으면 `error`** (잠금·버퍼 검사가 헐거워진다),
#: 길면 `warning`(상수가 낡았다).  사다리 T2/T3 를 채택하면 여기가 12.82/13.06
#: 으로 움직여야 하고, 안 고치면 기동이 알린다.  ⚠️ 12.78 이 바닥 12.7762 보다
#: 4 ms 길다 -- 두 자리 반올림이고 허용 차 안이라 그대로 둔다(운영자 결정
#: 2026-09-13 의 값).  `tests/test_acftiming_science.py` 가 여덟 장 전부 대사한다.
MIN_FRAME_PERIOD = 12.78
