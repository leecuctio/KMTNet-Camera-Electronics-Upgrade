...
### ⏳ guide 유닛 **설정값**은 아직 미정이다 (운영자 2026-09-09)

⭐ **갈리는 것은 왕복 시간이 아니라 "어떤 값이냐" 이다** (운영자: *"히터나 trigger
출력의 실시간 반영이 문제로구나"*).

| | 60초에 얼마나 변하나 | 폴링값으로 되나 |
|---|---|---|
| 측정값 (RTD 4·진공·`HTROUT`·Radionode 3) | 서서히 | ✅ 된다 — 낡음은 `HKUDATE`·`HKSTALE` 이 알린다 |
| 설정값 (`HTREN`·`HTRSET`·`HTRFORCE`, `TRIGOUT*`) | ⚠️ **운영자가 방금 바꾼다** | ⛔ 안 된다 — `htrset 1 -95` 바로 뒤 `hkdata` 가 옛 값을 낸다 |

⭐ **그런데 확인하는 길이 따로 있다** — `HTRSET`·`HTRFORCE`·`TRIGOUTFORCE`·
`TRIGOUTLEVEL` 은 **인자 없이 치면 조회**고, 그 갈래는 `RCONFIG` 즉시 되읽기다.
방금 바꾼 값을 확인할 자리는 그쪽이라, `HKDATA` 가 폴링값을 내도 확인 경로는 안 막힌다.

⏳ 세 가지를 놓고 **벤치 실측 뒤에** 정한다.

| | 왕복 | 방금 바꾼 값 | 바이패스·외부 변경 |
|---|---|---|---|
| (갑) 지금대로 무조건 `RCONFIG` 3회 | 3회 (한가할 때 6 ms) | ✅ | ✅ |
| (을) 폴링값(`_sample`) | 0 | ⛔ 최대 60초 늦다 | ⚠️ 60초 |
| (병) `config_value()` — 캐시 + `config_dirty` 때만 되읽기 | 평시 0 | ✅ **즉시** | ✅ (바이패스는 표시된다) |

⚠️ (병)의 빈틈: `set_config` 가 **왕복 실패에도 캐시를 먼저** 갈아끼운다.  다만 그때는
명령 자체가 실패로 보고되고, `WCONFIG` 는 앉았는데 `APPLY` 가 실패한 경우는
**`RCONFIG` 도 새 값을 낸다** — 그 구멍은 `STATUS` 만 메운다.

## ⏳ 연속 노출 중 명령 지연을 재는 법 (2026-09-09)

운영자 물음: *"guide 유닛 timing script 가 돌고 연속 촬영 중일 때 `trigout <초>`
명령 실행 지연 여부"*.  ⛔ **막히지는 않는다** — 거부도 없고 큐에 선다.  다만
`_locked_thread` 가 모든 왕복을 한 줄로 세우므로 **진행 중인 FETCH 뒤에 선다**
(guide 8.3 MiB ≈ 0.08 s, 잠금 상한 `fetch_timeout` = 1 s).

**재는 법** — `~/AIC/Config/icg_archon.ini` 에서 임계를 0 으로 두면 전부 남는다.
⭐ **이건 실측용 설정이다** — 끝나면 운용값(기본 50)으로 되돌린다.
...
⛔ `TRIGOUT` 은 **이 눈금을 안 탄다** — 운영자가 칠 때만 나가는 드문 명령이라 늘 남긴다.
임계를 타는 것은 `HKDATA`/`HK` 뿐이고, 그 이유는 **바깥 감시 계통이 자주 물어 올 수
있어서**다 (우리 프로그램에는 주기 발신자가 없다).

```
guiexp 1.3
go 20
trigout 2      ← 연속 노출 중에 여러 번
hkdata         ← 연속 노출 중에 여러 번
```

로그에 이런 줄이 남는다 (`INFO`, `icg_archon.cmd`):

```
TRIGOUT 올림 지연 -- 수신→완료 92.4 ms (취득중) Sec=2
TRIGOUT 내림 지연 -- 수신→완료 2095.1 ms (취득중) 폭오차 +2.7 ms (요청 2s)
HKDATA 지연 -- 수신→완료 88.1 ms (취득중)
```

⭐ **가장 중요한 값은 폭오차다** — 시작이 밀려도 폭이 맞으면 광원 노출량은 맞는다.
시한을 **선이 올라간 뒤부터** 재므로 구조상 밀림이 폭에 안 섞이고, 남는 오차는
내림 쪽 락 대기뿐이다 (시험이 이를 못박는다).
⚠️ 한가할 때 기준선은 `RCONFIG` 3회 = **6 ms** 다 — 취득 중 값을 그것과 견준다.
⛔ 이 지연은 **락 대기 + 왕복 처리**를 합친 값이라 둘을 가르지 않는다.
⏳ ⚠️ **실측 뒤에 운용 임계를 다시 정한다** — 취득 중 정상 지연이 50 ms 를 늘 넘으면
그 기본값은 "이상"이 아니라 소음이 된다.  임계는 **실측 최악값 위**로 잡는 것이 맞다.
