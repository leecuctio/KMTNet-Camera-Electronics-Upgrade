...
```bash
python3 -u tools/probe_archon.py --unit guide --host 10.0.0.162 --acf acf/KMTK_GUI_162_STA0201_R2622.acf 2>&1 | tee probe1_guide.log
```
...
> ⚠️ **판 번호가 낡았다 (2026-09-14 주석).**  이 부록은 guide `R2617` 이 현행일 때 썼다.  지금
> 저장소 현행은 **guide `R2622`** 이고(`FlushFrame:` 119행 `DGHIGH` 가 이미 들어갔다), 시험 ACF 는
> **현행 사본**에서 만든다.  `FRAME6` keep 후속 판의 예약 번호는 아래 본문의 R2620/R2622 가 아니라
> **guide `R2624`** 다 (`acf/README.md` "예약 번호를 또 밀었다" · `bench_test_plan.md` 같은 절).
> 그리고 이 문서의 판 번호는 전부 **guide** 계열이다 — 규칙대로 계열을 붙여 읽을 것.
...
| D FrameShift 중 DG 높음 (본 물음) | ⛔ **`FRAME6` 에만** DG 항을 **`,1,1`(keep)** 으로 + `LINE13="DGLOW; X(1)"` | Δ_D — **Δ_D ≈ Δ₀ 면 도착하는 전하가 시프트 없이 덤프된다** → FRAME6 수정 정당 |
| E (선택) 부작용 | D 의 상태표 + `LINE13` 원상 | 배경·첫 행·노이즈가 A 와 같은가 |

> ### ⛔ 변형 D 를 **정정했다** (2026-09-13, 다각 검증)
>
> 종전 문면은 *"`FRAME6`/`IMAGE6` 의 DG 항을 **둘 다** `DG_HIGH,1,0` 으로"* 였다.
> **그대로 하면 시험이 자기모순이 된다** — 본 독출 첫 행 끝의 `IMAGE6` 이 DG 를 12 V 로
> 올려놓아 **나머지 1032 행이 열린 덤프 게이트 위를 지난다.**  덤프가 실제로 된다면
> **재려던 영상이 비어 버린다.**
>
> ⭐ 바른 변형: **`FRAME6`(STATE31) 에만**, 값은 **`,1,1`(keep)**.
> `FrameShift` 는 두 호출 자리(정상 독출 13행 · flush 119행)가 **모두 `DGHIGH;` 를
> 앞세우므로**, keep 이면 전송 내내 12 V 가 유지된다 — `DG_HIGH,1,0` 으로 set 할 필요가 없다.
>
> ⛔ **`IMAGE6`(STATE25) 은 건드리지 않는다.**  그 DG←0 V 는 잡음이 아니라 `Line:`·`SkipLine:`
> 양쪽이 쓰는 **매 행 덤프 게이트 자물쇠**다 (DevNote 11.84-(2)).

각 단계 **3 회**, 첫 행/다음 9 행의 중앙값 차를 `GAIN` 으로 e⁻ 환산해 표에 적는다.

### 판정
* Δ_D ≈ Δ₀ (Δ_B 의 10 % 이하): FrameShift 중 DG=HIGH 가 레지스터를 비운다 → **`FRAME6` 만** keep 으로 바꾸는 후속 판(**R2622**) 정당.  ⚠️ 예약 번호가 R2618→R2620→**R2622** 로 두 번 밀렸다 (그 번호들을 다른 판이 가져갔다).
