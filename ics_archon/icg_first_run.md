...
| ACF 경로 | `[icg] acf` | `acf/KMTK_GUI_162_STA0201_R2618.acf` (현행 유일본) |
...
```bash
python3 -u tools/probe_archon.py --unit guide --host 10.0.0.162 --acf acf/KMTK_GUI_162_STA0201_R2618.acf 2>&1 | tee probe1_guide.log
```
...
| `trigout 2` | 5~10회 | 올림 지연 · 내림 지연 · **폭오차** |
...
| 항목 | 기대 | 실측 |
|---|---|---|
| `HKDATA` 지연 — 한가 | ≈6 ms (`RCONFIG` 3회) | |
| ⭐ `HKDATA` 지연 — 취득중 **최댓값** | ⏳ 미지.  FETCH 0.08 s 뒤에 서면 ≈90 ms | |
| `TRIGOUT 올림` — 한가 | ⏳ 미지 — **`APPLYSYSTEM` 하나가 몇 ms 인지 아직 안 쟀다** | |
| ⭐ `TRIGOUT 올림` — 취득중 **최댓값** | ⏳ 미지 | |
| ⭐⭐ **폭오차** | ⭐ **밀림을 안 물려받아야 한다** — 시한을 선이 올라간 뒤부터 재므로, 남는 오차는 내림 쪽 락 대기뿐 | |
| ⛔ **취득을 해쳤나** | `go 30` 이 **30장을 다 찍었나** — `APPLYSYSTEM` 이 `Exposures` 를 되돌리면 거기서 끊긴다.  ⚠️ 이것이 지연보다 중한 관측이다 | |
| 프레임 주기 | `trigout` 을 친 프레임의 `DATE-OBS` 차가 튀지 않나 | |
...
로 고치는 후속 판(**R2618** — R2617 은 빈 줄 판이 썼다)이 정당하다.
