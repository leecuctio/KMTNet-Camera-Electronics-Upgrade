...
⏳ **남은 확인 하나**: 보정을 넣은 뒤 `trigout 2` 를 쳐서 **폭오차가 0 근처로 떨어지는지**
로그로 본다 (`폭오차 +x.x ms (요청 2s, 보정 -234 ms)`).

| 상황 | 기대 | 실측 |
|---|---|---|
| **한가** — `trigout 2` 5회 | 폭오차 ≈ **0** (±수 ms).  보정값은 233 ms 언저리 | |
| **취득중** — `go 60` 중 `trigout 2` 5회 | ⚠️ ±100 ms 로 흔들릴 수 있다 — **음수도 정상**이다 (아래) | |
| ⚠️ 짧은 펄스 — `trigout 0.1` | 만들 수 없다는 **경고**가 뜨고 폭은 ≈235 ms | |
