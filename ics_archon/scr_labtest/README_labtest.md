...
## ⛔ 본편(`ics_archon`)의 프로토콜 개선은 **역이식하지 않는다** (운영자 확정 2026-08-29)

본편이 Archon 매뉴얼을 다시 읽으며 찾아낸 것 셋이 **labtest 에는 없다.**  그것을
되돌려 넣을지 열려 있었는데, **넣지 않기로 확정됐다** — *"labtest 로 역이식은
하지 않을거야.  ics_archon 에만 적용해줘"*.

| 항목 | labtest (그대로 둔다) | `ics_archon` |
|---|---|---|
| 참조번호 (결정 7) | **보낸 뒤** 올린다 | **보내기 전에** 올린다 — 시한 초과 뒤 늦은 응답이 다음 명령 번호와 맞아떨어지는 일이 원리상 없어진다 (`protocol.py:181`) |
| 오류 응답 (결정 8) | `<xx` 만 대조 — 컨트롤러의 **거부(`?xx`)를 프레이밍 오류로 뭉갠다** | `ArchonError.reply_error` 로 구분, 읽기 버퍼도 하나 |
| 독출 진행률 | 프레임 번호가 **바뀌기만** 기다린다 | `FRAME` 의 `BUFnLINES`/`BUFnHEIGHT` 로 **실제 진행률** (`parse.py:176`) |

⚠️ **이것은 결함이 아니라 결정이다.**  labtest 는 **1년 실사용으로 검증된 별개
도구**이고 실기 투입 직전이다 — 검증된 왕복을 건드리는 위험이 얻는 것보다 크다.
그래서 **두 프로그램의 프로토콜 층은 앞으로 갈린다.**

⭐ **갈리면 안 되는 것은 규격 사본뿐이다** — 내장 `RAWCARDS` 는 raw spec 5장의
기계 사본이라 본편·`_vendor`·labtest 셋이 **반드시 같아야** 하고,
`../tests/test_labtest_spec_copy.py` 가 그것을 지킨다.  ⚠️ 그 시험은 **상수 표만**
대조하고 **동작은 안 본다** — 함수 본문의 규칙(예: ACF 이름 자르기)은 사람이
맞춰야 한다.
...
- `DATA_STORAGE` 자리가 **이미 있는지** (54행). **v1.1.3 부터 스크립트가 데이터셋 시작에 직접 확인하고, 없거나 못 쓰거나 좁으면 접속·전원 전에 멈춘다** — 그리고 **만들어 주지 않는다**(마운트가 안 붙은 것을 폴더 생성으로 덮으면 자료가 엉뚱한 곳에 쌓인다). 자리는 운영자가 먼저 만든다 — `mkdir -p ~/AIC/data`, 다른 디스크로 보낼 거면 심볼릭 링크 (INSTALL.md "2. 자리 만들기"). — 그리고 **여유
  용량**(아래 "첫 실행은 작게" 의 표)
- `TEMP_MODS` — 지금은 **열 자리**다: `BACKPLANE_TEMP` + `MOD1~5`·`MOD8~11`
  (규격 5.6.1절).  카드 폭(51자)을 넘으면 잘리고 경고가 난다.
  ✅ **모듈 나열 순서는 규격 5.6.1절 신설(v1.5)로 이미 닫혔다** — 종전의
  *"규격 수록 예정"* 은 낡은 문면이다.  ⛔ 줄 번호를 적지 않는다 (598행이라
  적혀 있었는데 실제는 700행이었다) — `grep -n '^TEMP_MODS'` 로 찾는다.
  정본은 `ics_sim.rawhdr.TEMP_MODS` 이고 이것은 그 사본이다
...
헤더·규격 정합·데이터셋 구성·검사는 **글자까지 같다** — 견본 대조 **144 항목**
(`RAWCARDS` 템플릿 행 = 값 카드 136 + COMMENT 8) 불일치 0.
`test_labtest_spec_copy.py` 가 그 밖의 자리에서 갈라지는 것을 막는다 — ⛔ 시험
수를 여기 적지 않는다 (*"두 시험"* 이라 적혀 있었는데 실제는 스물여섯이었다).
...
- `Cn_TEMP` **8자리** (Backplane · Mod3 · Mod4 · Mod5 · Mod6 · Mod7 · Mod9 ·
  Mod10) — **규격 10.4절이 정본이다** (v1.9 에서 **OI-19 종결**, 첫 guide 구동
  때 STATUS 재확인만 남는다).  전원 레일도 guide 는 8자리다 (7레일 + `HEATER`)
