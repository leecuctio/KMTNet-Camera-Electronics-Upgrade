#### archon_kmtnet_guide_modtm_powon_v0.3.kasi.STA0291.py / 2026-05-31
#### - original version: archon_kmtnet_guide_modtm_imgacq_v0.3.kasi.STA0291.py / 2026-05-29
...
#### powon version
####  - Disable image acquisition by commenting out the Exposure() call in the main loop,  
####    while keeping CCD power monitoring and power-on functionality active
...
def GetModTemps(IpAddr):
    ...
    T_BPT = T_M03 = T_M04 = T_M05 = 0.0
    T_M06 = T_M07 = T_M09 = T_M10 = 0.0
    STATUS_POWER = -1
    ...
    #tStatus = (timeacq, T_BP, T_M03, T_M04, T_M05, T_M06, T_M07, T_M09, T_M10, STATUS_POWER)
    ...
    return (timeacq, T_BP, T_M03, T_M04, T_M05, T_M06, T_M07, T_M09, T_M10, STATUS_POWER)
...
tStatusPrev = ('', 0, 0, 0, 0, 0, 0, 0, 0, 0)  # 10 elements (index: 0-9)
...
PathLog = "%s/modtm.gui.log" % DIR_LOG
DirFits = "%s/fits.gui" % DIR_LOG
DATA_PREFIX = "gui.103"
...
while running:
    ...
    if tStatus == None:
        ...
        strLog = '%s / %+6.1f / %+6.1f / %+6.1f / %+6.1f / %+6.1f / %+6.1f / %+6.1f / %+6.1f / %d' % tStatus
        print('> Log: ' + strLog + '\n' )
        ...
        if ElapsedSms > INTERVAL_SMS:
            ...
            if abs( tStatus[1] -tStatusPrev[1] )>TH_TM or \
               ...
               abs( tStatus[8] -tStatusPrev[8] )>TH_TM :
                tStatusPrev = tStatus
