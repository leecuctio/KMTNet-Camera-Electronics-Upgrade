...
- ⚠️ **잰 판은 `1.0.1261`(Rev H) 과 `1.0.1252`(Rev F) 다** — 그 뒤 **관측소
  상자가 `1.0.1271` 로 올랐다**(2026-09-11 초기화 시험, ACF `BACKPLANE_VERSION`).
  ⭐ 아래 결론 대부분이 *"두 유닛 동일"* 로 **판이 다른 둘에서 같은 값**을
  냈고, 운영자도 *"기능 차이는 없을 것 — 디버깅만 한 걸로 안다"* 로 본다
  (2026-09-12).  ⏳ **그래도 다시 잰다** — 절차는 벤치 계획서 3단계
  *"FW `1.0.1271` 재실측"*.  이 값들이 `MIN_FRAME_PERIOD`·`fetch_timeout`·
  `lock_buffer` 판단의 근거라, 틀렸을 때 드러나는 자리가 **프레임이 덮인
  뒤**이기 때문이다.  ⛔ 보드 판마다
  올릴 수 있는 FW 최종판이 다르다 — Rev F 는 `1.0.1252`, Rev H 는 `1.0.1271`
  (운영자 2026-09-12, [`acf/README.md`](acf/README.md)).
