...
### ⭐⭐ 2026-09-11 마감 (DevNote 11.69~11.74) — **이것이 최신 · 새 세션은 여기부터**
...
깨지면 **몇 회차**인지 + **`어긋난 자리의 원문`** 줄을 챙긴다.  ⚠️ 사이클마다 `hkdata`
...
* ⏳ `DSAZ`/`DSTELAZ` -- TC 가 `TCSSTATUS` 에 추가 대기.  ⭐ **우리가 할 일은 없다**.
...
실패한다 — **HEAD(이 세션 변경 전)에서도 같은 서명**(`MK: 링크를 다시 세운다 (FRAME 왕복이 깨졌다)` →
