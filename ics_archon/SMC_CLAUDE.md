...
#### ⛔⛔ 재실행이 컨트롤러에 못 붙는 것 — 원인은 **FIN** 이었다 (11.61)

⭐ **증상을 자리까지 짚었다**: ACF 폭주의 **870번째** 명령(`WCONFIG0365STATE26/MOD9`)
응답 **하나만** 없고 다음 것이 당겨져 읽힌다 (`기대 <66, 받음 <67` — 참조번호 셈이
정확히 맞는다).  ⭐ **두 실행이 같은 자리**라 타이밍 우연이 아니다.

⭐ **재접속이 `timed out` 되는 이유**: `FIN` 은 *"나는 더 안 보낸다"* 일 뿐이라 컨트롤러가
자리를 즉시 안 비운다 — 폭주 중에 끊으면 수백 개가 남아 있고, 다 소화할 때까지
**새 SYN 에 응답하지 않는다** (로그상 **약 10초**).  새 연결에 **옛 응답**이 흘러든 것도
잡혔다 (`기대 <01, 받음 <00`).

**처방 셋** (labtest = 실기에서 도는 원본을 따랐다):
1. ⭐ **RST 로 끊는다** (`SO_LINGER(1,0)`) — `resync()` 전용.  ⚠️ 정상 종료는 FIN 이 맞다.
2. **진정 시간** `settle_before=0.8` · `settle_after=2.0` (ini 로 조절).
3. ⛔ **guide `connect_retry` 가 2 였다** (science 4) — 11.54 의 `acf_retry` 와 같은 부류.
   → **4**.  ⭐ `acf_retry` 는 **4 그대로** (운영자 확정).

⏳ **아직 못 가린 것**: *왜 하필 870번째 응답이 없나*.  어긋난 순간 **읽은 줄 80B +
버퍼 잔여 200B** 를 남기게 했다 — 한 번만 재현하면 갈린다.
⛔ **리셋 없이 복구**: `ping` 으로 망/앱을 가르고, `sudo ss -K dst 10.0.0.162` 로 남은
소켓을 RST.  ⚠️ 프로세스가 이미 죽었으면 끊을 것이 없어 **기다리는 것**뿐이다.
