...
#### ⭐ `CCDTEMP` 에서 chip 귀속을 없앤다 (운영자 확정 2026-08-27) — **v1.9 대기**

**결정**: `CCDTEMP` 를 **특정 chip 으로 규정하지 않고 "CCD 대표 온도"** 로 한다.
카드 comment 의 `M` 도 없앤다.  **대표 센서가 어느 chip 근처인지는 정보가 지금
없다** — 그래서 있는 척하지 않는다.

**시점**: ⚠️ **v1.8 에는 안 들어갔다** — v1.8 은 `OI-9` 폐기와 `CTRLnCFG` 정합
둘만 실었다.  이 항목은 **v1.9 대기**다.  `main` 으로 넘어가 헤더 정본을 고치고
판올림하면서 함께 반영한다 (운영자 2026-08-27).  ⚠️ **이 브랜치에서 손대지 말 것**
— 견본 pair 바이트가 정본이라 규격 판올림과 함께 움직여야 한다.
...
바뀌었는지 반드시 적을 것.**
#### ⭐ `RDMODE` 결측값을 `UNKNOWN` 으로 등재 (운영자 확정 2026-08-29) — **v1.9 대기**
...
#### ⭐ `ICG` RTD 구성 변경은 `CAMVER` 로 흡수한다 (운영자 확정 2026-08-27) — **v1.9 대기**
...
| ⏳ **판단 둘** — `icg_archon` 착수(작업 D) · `connect()` 에 `drop_tickets` 배선 | **운영자** | — |
| ⏳ **`main` 소관 둘** — 규격 v1.9 등재 · 충욱 브랜치 머지 | `main` | `raw_fits_spec/` · PM |
...
전에** 못박아 두었다 — 나중에 정하면 눈이 원하는 답을 고른다).
...
1. **규격 v1.9 등재** — `RDMODE = UNKNOWN`(5.5절이 아직 `'NORMAL' 등` 이다) 포함
   대기 4건.  ⚠️ **`raw_fits_spec` 은 `main` 소관**이다
...
것**이라, 실기 없이 새 세션을 열면 할 일은 **규격 v1.9 준비**(대기 4건 —
`CCDTEMP` chip 귀속 제거 · `OI-18` 폐기 · `CAMVER` 규범 · D3 카드 배치)나
**`icg_archon` 착수 판단**(작업 D)뿐이다.  목의 지시를 먼저 받을 것.
...
| **P-k** | **guide `Pixels` 를 528 로 줄여도 영상이 같은가** (신설 2026-08-29, 운영자 예정) | 지금은 탭당 601 을 디지타이즈해 **528 만 저장**한다 -- 73개가 버려지고 픽셀 클록의 약 12% 다.  같으면 무손실 트림 | 손으로 -- 같은 조건 `Pixels=600` vs `528` 프레임 **바이트 비교**.  ⚠️ `LINE47` 의 인자 없는 `CALL PixelFirst`(+1)는 남길 것 |
...
| **F5** | ✅ **종결 (2026-08-29, 실측)** | FETCH 상한을 `[archon] fetch_timeout` 으로 뺐고, `frame_timeout` 과 어긋나면 기동에서 알린다.  ⭐ **값을 `30` 으로 확정** — 실측이 344 MiB 에 **약 5초**(72 MB/s)라 6배 여유다.  종전 유도값 344초는 실측의 **69배**여서, 링크가 죽어도 25초 창이 터진 한참 뒤까지 매달렸다 | `config._cross_checks` |
| **F6** | **결함 아님** | 아래 | -- |
...
| 5 | `erase(ccd)` 를 **두 대 다** flush — ⛔ **다만 기본값은 `false`(안 한다)** (운영자 확정 2026-08-29) | *"이제는 clock 을 개선해서 별도 erase 를 하지 않고 바로 노출을 시작한다"*. 종전 기본값 `true` 는 **clock 개선 전 전제**였다. labtest 도 1년 실사용을 `bFullFlush=False` 로 했다. ⚠️ 켜면 노출마다 **독출 1회분(11.3초)** 이 더 붙어 주기가 12.4 → 23.7초 | `full_flush_on_erase=true` 로 켜면 두 대 다 비운다 (그 규칙은 유효하다) |
...
| 15 | fetch 중 **`LOCKn` 으로 버퍼를 잠근다**(`lock_buffer=true`) + fetch 앞 프레임 번호 대조 | ⭐ **실측 여유가 ~6초다** (2026-08-29) — FETCH 는 `IDLE`+3.4~8.4초에 끝나고 그 버퍼 재사용은 ~14.7초다. `BIAS` 연속이 가장 얇다. **매뉴얼 p.71 이 `LOCK` 을 통상 경로로 적어 두었다** (2026-08-30) — *"새 프레임이 있으면 호스트는 LOCK 을 내려 그 버퍼가 덮이는 것을 막고 FETCH 한다"*. ⚠️ **다만 판정이 아니라 가설의 강도다** — 매뉴얼은 2021-02-23 판이고 **현행 FW 와 양방향으로 어긋날 수 있다**(운영자 2026-08-30, DevNote 8.7). 기본을 `true` 로 둔 진짜 이유는 **더 안전한 쪽이기 때문**이다: 켜서 문제가 나면 크게 드러나고(지연·실패), 꺼서 문제가 나면 조용하다(누더기). ⚠️ 다만 엔진은 *"다음 **잠기지 않은** 버퍼"* 를 잡으므로 **science(BIGBUF, 2개)에서는 하나를 잠그면 엔진에 하나만 남는다** — labtest 가 뺀 이유일 수 있다(가설 H3, 실기 확인) | `lock_buffer=false` 로 끄면 labtest 와 같아진다(대조는 남고, `recheck_after_fetch` 가 fetch 중 창까지 본다) |
| 18 | ⭐ **`LOCKn` 이 먹었는지를 `RBUF`/`WBUF` 로 관측한다** (왕복 0) | 매뉴얼은 판정 근거가 아니므로(DevNote 8.7) **실기가 답하게** 한다. fetch 앞 덮임 대조가 **이미 읽는 `FRAME`** 안에 `RBUF`(*"locked for reading"*, p.50)가 있었는데 버리고 있었다. 관측값은 **이미 있는 `FETCH` 로그 줄에 얹어** 정상 취득에 새 줄을 안 늘린다. ⚠️ 한 방향으로만 결정적(안 맞아도 `RBUF` 미구현일 수 있다) — ⭐ 그래서 **거동**인 `WBUF` 이동을 함께 잰다 | 관측일 뿐이라 되돌릴 것이 없다. 경고가 거슬리면 `lock_buffer=false` |
| 17 | **fetch 뒤 재대조**(`[archon] recheck_after_fetch`, 기본 **`true`**) + **`POWERON` 에서 저장 표 폐기** | fetch 앞 대조는 **직전 한 순간**만 본다 — fetch 자체가 5초라 그 사이에 덮이면 **앞뒤가 다른 누더기**가 길이·헤더 정상으로 나와 로그에 안 남는다. ⭐ **`lock_buffer=false` 인 경우 필요할 수 있다** — `LOCKn` 을 끄면 그 창을 막는 것이 없다. 잠겨 있으면 **절대 안 걸리고** 왕복 하나(ms)만 는다. ⭐ A/B 실험을 뜻있게 만든다(누더기를 조용히 쓰는 대신 크게 운다). ⚠️ **둘 다 끄면 기동 교차검사가 알린다** | `recheck_after_fetch = false`. ⚠️ `lock_buffer` 와 **함께 끄지 말 것** |
...
5. ✅ **버퍼 수 대 저장 여유 — 실측으로 판단 완료 (2026-08-29).**
   ...
   쓰는 것은 ~14.7초다.  ⚠️ **아래 본문의 "프레임 ~40초" 는 가정이었고 실제
   주기는 12초**(노출 0 + readout 11.3)라 여유가 가정의 절반이다 — 그래서
...
| 폭 | science (주기 12초) | **guide** (짧은 노출 연속) |
|---|---|---|
| 16비트 (65,536) | 약 **9일** 연속 | 1초 주기 **18시간** · 0.5초 **9시간** |
| 32비트 | 사실상 무한 | 사실상 무한 |
...
1,600년이라 사실상 닫히지만, ⚠️ **추론이지 문서가 아니다.**
...
⭐ **컨트롤러 재시작도 같은 코드로 잡힌다** (`REBOOT`·`WARMBOOT`·전원 재투입).
자세한 것은 `DevNote.md` 8.3.
...
1. ✅ `drop_tickets()` 를 **`power_on()` 에 배선**했다 -- `POWERON` 은 **새 프레임
   번호 세션의 시작**이라, 전원이 내려간 사이의 프레임은 다시 못 받는다.
...
- ⚠️ **미확인 -- 매뉴얼뿐.**  ⭐ **`LOCK` 의 효과가 실기에서 매뉴얼대로인지가
  A/B 실험의 본체다.**
  ...
  **매뉴얼 안에서 `LOCK` 은 예외 조치가 아니라 통상 경로에 있다**(결정 15).
  ...
  벤더 절차에도 없다.**
- ⚠️ **미확인 -- 매뉴얼뿐** (가설 H3 의 출처).
  **엔진은 새 프레임을 시작할 때 "다음 잠기지 않은 버퍼" 를 잡고**, 그 버퍼의
  정보(시각·번호)를 갱신한 뒤 채운다 (p.71).  -> **번호가 가만히 있으면 옛 프레임,
  바뀌면 새 프레임**이라는 판별의 근거다(되감김 처리).  ⚠️ **남는 버퍼가 없을 때
  어떻게 되는지는 안 적혀 있다** -- science(BIGBUF 2개)에서 하나를 잠그면 하나만
  남는다.
- ⚠️ **매뉴얼에 없다 -- 실측으로만 좁힐 수 있다.**
  ...
  **65535 를 넘은 적이 있으면 16비트가 아니다**(`probe_archon` 1단계가 이미 원문을
  ...
  ② **전원 재투입 직후 `BUFnFRAME` 이 0 인가** -> 재시작 경로가 실측으로 확인된다
