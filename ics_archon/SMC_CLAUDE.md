...
| 시험 | `ics_archon` **679** · `ics_sim` **395** — **전수 통과** (2026-09-10 실측, deselect 없음).  ⛔ **알려진 flake 는 없다** — 종전 표의 *"flake 1 deselect"* 는 둘 다 사실이 아니었고(회귀였다, `8664e92` 에서 고쳤다 · 오기 철회 `086bb4e`), `deselect` 장치는 저장소에 없다 |
...
#### ⛔⛔ 재실행이 컨트롤러에 못 붙는 것 — ⏳ **미해결, 원인 추적 중** (11.61)

⚠️⚠️ **내 진단이 두 번 틀렸다.**  ⛔ *"소켓을 안 닫아서"*(11.59-(2))도, ⛔ *"FIN 이 자리를
안 비워서"*(11.61 초판)도 아니다 — **`ping` 이 100 % 손실**이다.  컨트롤러가 **망에서
통째로 사라진다**.  ⭐ 기다려도 안 풀리고 `gauge_on_start=off` 여도 같다.
