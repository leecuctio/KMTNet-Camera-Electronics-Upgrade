...
`ics_archon` **214 통과** · 배치본 `-m "not repo_only"` 178.
...
`main` 이 **16커밋 앞서 있다** (2026-08-29 기준).  들어올 것:

- **raw spec v1.8 발행** (`0c821ea`, 태그 `raw-spec-v1.8`) — `OI-9` 폐기 ·
  `CTRLnCFG` 를 실제 ACF 이름으로 · **견본 pair 4장의 `CTRLnCFG` 값이 바뀌었다**
- 세 문서 판올림으로 **파일명이 바뀌었다** — 규격 `_v1.8.md` · 원장 `_v1.15.md` ·
  통합문서 `_v0.7.md` (구판 `archive/`).  이 브랜치의 **코드 주석 18곳이 구
  파일명을 가리킨다** — 작업 E 의 딸림 1
- 충욱 작업 15커밋 (PM 문서 · gmon 그래프 창·합성 별 주입 등)

```bash
git merge origin/main            # 지난번 합류는 4a1d898, 충돌 1건이었다(README 표)
python -m pytest tests -q        # ics_archon
cd ../ics_sim && python -m pytest tests -q
```

⚠️ **머지 뒤 `ics_sim/tests/test_raw_draft.py` 를 꼭 돌릴 것** — 견본 pair 가
`main` 에서 바뀌었고 **그 바이트 대사는 이 브랜치에만 있다**(`main` 에는 없어서
`main` 쪽 시험이 못 잡는다).

### 첫 일감은 **작업 E** (`CTRLnCFG` 파생)

규격 v1.8 이 *"폴더 경로와 확장자를 뗀 이름"* 을 못박았으니 코드가 뒤따를
차례다.  상세는 아래 [작업 E](#-작업-e--ctrlncfg-를-acf-경로에서-파생-이-브랜치-다음-세션-첫-일감) ·
메모리 `project_ctrlcfg_from_acf_path.md`.
...
| 2 | **운영자 ACF 정리 반영** — guide 정본을 하나로 줄이고 개명, 영향 전수 | 이 커밋 |
| 3 | **참고 자료 재검토** — `__ref_archon_control/` 전수, 옮긴 것 1 · 안 옮긴 것 3 · 새 사실 3 | 이 커밋 |

**코드 변경은 하나뿐이다** — `controller._await_power()`(`POWERON` 뒤 `POWER=4`
확인).  나머지는 시험·문서·자료 정리다.  `ics_sim` 은 **한 줄도 안 고쳤다**
(그래서 `tools/sync_vendor.py` 를 돌릴 일도 없었다).
...
1. **머지** (위) → **작업 E**(`CTRLnCFG` 파생) → 그다음이 **작업 B~D** 다.
...
`main` 에서 할 일이다.  ⚠️ **작업 B 는 전부 실기가 있어야 하는 것**이라, 실기
없이 새 세션을 열면 할 일은 **작업 C 준비**(영향 전수 목록 확인)나
...
- **`CTRLnCFG` 예시를 실제 ACF 이름 규칙으로** + 규격 5장에 **"폴더 경로와
  확장자(`.acf`/`.cfg`)를 뗀 이름"** 명시.  ⏳ **그 파생을 코드로 넣는 것이 이
  브랜치 몫이다** -- 아래 "작업 E".
...
### ⭐ 작업 E — `CTRLnCFG` 를 ACF 경로에서 파생 (이 브랜치, **다음 세션 첫 일감**)

**규격이 먼저 섰고(v1.8) 코드가 뒤따를 차례다.**  전체 지시·주의는 메모리
`project_ctrlcfg_from_acf_path.md` 에 있고, 여기는 이 브랜치에서 손댈 자리다.
