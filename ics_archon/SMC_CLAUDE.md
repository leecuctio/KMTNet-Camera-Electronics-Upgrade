...
| **나열 결측 sentinel** | `-999.99` → **`NC`** | `archon/parse.SLOT_NC` · labtest `SLOT_NC` 신설.  ⚠️ **단일 HK 카드(`CCDTEMP` 등)는 `-999.99` 그대로다** |
...
**커밋은 다 됐고 `origin` 에는 아직 안 올라갔다.**

| 어디 | 상태 |
|---|---|
| `main` | `6d9c137` raw spec **v1.6** 발행 (규격·견본·원장·통합문서·D-019). **origin 미푸시** |
| `ics-archon-v1.0-build` | main 머지 + v1.5·v1.6 코드 반영 완료. **origin 미푸시** (앞선 5커밋 포함) |

⚠️ **바로 처리해야 할 것 넷**

1. **`origin` 푸시** — `main` 과 브랜치 둘 다. 목이 "푸시 전에 수정할 게 있다" 며 멈춰 둔 상태에서 v1.6 작업이 들어왔고, 그 뒤로 다시 안 올렸다.
2. **`raw-spec-v1.6` 태그** — 이 폴더의 규칙은 "그 판의 **마지막 커밋**에 붙인다"(`raw_fits_spec/SMC_CLAUDE.md`). v1.6 의 마지막 커밋이 확정되면 붙이고, 구판 `raw-spec-v1.5` 태그는 보존 방침대로 지운다. **아직 아무 태그도 안 붙였다.**
...
**규격 v1.6 이 코드에 다 내려왔는지 다시 보고 싶으면** — 아래 "raw spec v1.6 반영" 절의 표가 자리별 대조표다. 기계 검증은 `ics_sim/tests/test_raw_draft.py`(견본 바이트 재현) · `ics_archon/tests/test_fitswrite.py`(카드 이미지) · `ics_archon/tests/test_labtest_spec_copy.py`(사본 셋 표류)가 한다.
...
| **1.68** | ⏳ **`origin` 푸시 + `raw-spec-v1.6` 태그** — 위 "다음 세션이 먼저 알아야 할 것". 둘 다 아직이다 |
