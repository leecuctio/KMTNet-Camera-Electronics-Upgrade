...
⏳ **목의 판단이 필요한 것 하나** — `RDMODE` 의 실제 값.  현행 ACF 여섯이 어느
독출 모드인지(`NORMAL` 이 맞는지)를 정해 `ics_archon.ini` 에 적어야 한다.
labtest 가 그 파일을 `UNIT_ACF_SCI_NORMAL` 이라 부르므로 `NORMAL` 일 공산이 크다.
