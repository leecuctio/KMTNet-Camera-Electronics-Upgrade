...
### ⚠️ 프레임 번호 **되감김**이 코드를 깬다 (2026-08-30 발견, 판단 대기)
...
**얼마나 자주인가** (⚠️ `BUFnFRAME` 폭을 **우리가 모른다** — 매뉴얼 미확인):
...
1. ⭐ **매뉴얼 p.49-50 에서 `BUFnFRAME` 폭을 먼저 확인한다** — `__ref_archon_control/`
   에 있어 **실기 없이 지금 할 수 있다.**  32비트면 이 항목은 사실상 닫힌다.
2. 폭을 알면 **모듈러 비교**로 고친다 (`frame > prev` → 창 기반 비교).
3. ⚠️ **폭을 모르는 채 휴리스틱을 넣지 말 것** — *"정상인데 되감김으로 오해"*
   하는 새 결함을 만든다.  `mine.frame < prev` 를 되감김으로 해석하면, 실제로는
   "버퍼가 덮여 옛 번호가 보이는" 정상 경보와 구별되지 않는다.

### ⏳ `drop_tickets()` 가 죽어 있다 (2026-08-30 발견, 판단 대기)
...
1. `drop_tickets()` 를 **`power_on()` 에 배선** -- 전원 재투입 뒤 첫 노출 전에
   비운다.  간단하고 부작용이 작다.
2. **fetch 뒤 재대조** 추가 -- 위 "결과를 볼 때 함께 볼 것" 의 누더기 파일을
   잡는다.  ⚠️ fetch 를 마친 뒤라 *"버렸어야 했다"* 를 뒤늦게 아는 것뿐이지만,
   **누더기가 아카이브에 들어가는 것보다는 낫다.**
   ⏳ **`LOCK` 실험 결과를 보고 정하는 것이 낫다** -- 켜기로 하면 이 보완책의
   값이 줄고, 못 켜면 **유일한 방어**가 된다.
...
grep    '버퍼 .* 로 덮였다'                 → 덮임 감지 = 저장 거부 (파일 손실)
...
#### ⚠️ 결과를 볼 때 함께 볼 것 -- **대조가 못 지키는 구간이 있다**

카운터 대조는 fetch **직전 한 순간**만 본다.  **FETCH 자체가 5초**이고 그 뒤에
재대조가 없으므로, 그 5초에 덮이면 **앞부분은 내 프레임 · 뒷부분은 남의
프레임인 누더기 파일**이 나온다 -- 길이도 헤더도 정상이라 **로그에 아무것도
안 남는다.**

→ 그래서 2단계에서 **로그만 보지 말고 영상도 볼 것.**  누더기는 이미지 중간에
**가로로 끊긴 자국**(밝기·패턴이 한 줄 경계에서 불연속)으로 나타난다.
`--no-lock` 쪽에서만 나오면 그것이 `LOCK` 의 존재 이유다.
...
|---|---|---|
| **`LOCKn` 을 켜도 되나** (구 A-5 판단 ②) | labtest 가 2026-05-28 에 뺀 근거가 *"remove to fetch debug"* 한 줄뿐이다 -- 켠 채로 정상 취득이 도는지 | `[archon] lock_buffer` · 결정 15 |
