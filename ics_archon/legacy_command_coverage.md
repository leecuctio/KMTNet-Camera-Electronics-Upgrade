...
| **FLASHNOW** | IC (ICS·ICG 주소로도 받음) | `FLASHNOW [<ms>]` | 점검용 LED 를 n ms 즉시 점등 | 구현됨 — 원안 맞아. [검증: 443-459 전문. 문서 쪽에도 `FLASHNOW` 7회] ⛔ **2026-09-09 폐지**: 실기 ICS·ICG 둘 다에서 뺐다(`UNSUPPORTED`) -- 점검용 LED 는 실기에서 구현된 적이 없고(`flash_led()` 는 `_NOT_YET`), ICG 는 눌러 둔 sim 스텁을 불러 **거짓 `DONE`** 을 냈다.  그 자리는 `SHOPEN <초>`(science) · `TRIGOUT <초>`(guide)가 대신한다.  ⚠️ 시뮬(`ics_sim`)에는 남는다. |
...
| **LEDFLASH** | ICS·ICG·IC | `LEDFLASH [<ms>]` | 점검용 LED 점등시간 조회·설정 | 구현됨 — 원안 맞아. [검증: :283 핸들러 + :443-452 `cmd_flashnow` 가 인자 없으면 `self.state.ledflash_ms` 를 쓰는 것(:450)까지 확인해서 '값만 정하고 점등은 FLASHNOW' 해석이 맞아] ⛔ **2026-09-09 폐지**: 실기 ICS·ICG 둘 다에서 뺐다(`UNSUPPORTED`) -- 점검용 LED 는 실기에서 구현된 적이 없고(`flash_led()` 는 `_NOT_YET`), ICG 는 눌러 둔 sim 스텁을 불러 **거짓 `DONE`** 을 냈다.  그 자리는 `SHOPEN <초>`(science) · `TRIGOUT <초>`(guide)가 대신한다.  ⚠️ 시뮬(`ics_sim`)에는 남는다. |
