...
⭐ **미구현 63 의 대부분은 되살릴 것이 아니다** — `+LOG`/`-LOG`·`BIOSDISKREAD`·
...
| **BIAS** | ICS·ICG | `BIAS [<objname>]` | IMAGETYP=BIAS 설정 | 구현됨 — 원안 맞아. [검증: icg_archon/commands.py:140-161 열어 확인. `keep = st.exptime` → `super()._image_type(...)` → `if st.exptime != keep: st.exptime = keep`(:154-160). docstring 이 이유도 적어 뒀어 — guide 의 EXPTIME 은 독출 개시 간격이라 0 이면 `go` 가 거부되고 `EXP` 로 되돌릴 수도 없어 가이딩이 잠긴다].  ⛔ **2026-09-15 개정 (DevNote 11.92)**: guide `BIAS` 는 이제 `EXPTIME` 을 **최소 노출**(`_min_exptime()` = `effective_exptime(0)` 실현값, 대역엔 `exptime_min`)로 둔다(`icg_archon/commands.py` `_image_type`).  응답은 `ImageType=BIAS ObjectName=… EXP=<최소>` 다.  위 keep 서술은 2026-08-31 판이다 — 하한 아래를 `GO` 가 접게 된 뒤로 그 이유가 없어졌다.  `BIAS` 중에는 `EXP`/`GUIEXP` 설정이 거부되고, 다른 국면으로 옮기면 풀린다 |
...
| **DONE:** | 공통 | `<본문>` | IMPv2 완료 응답 수신 처리 (메시지 타입) | 구현됨 — 원안 맞아. [검증: impv2.py:40 · app.py:326-352 전 구간 열람. 보고 갈래는 :349-352 `log.info('보고 수신 (조치 없음) -- …')` 로 끝나 어떤 경우에도 답을 안 보내]. ⭐ 원안에 없던 사실: **실제로 등록된 조치는 딱 둘** — ics_archon/ics_archon/app.py:572-573 `for word in ('HK','HKDATA'): self.register_report('DONE', word, self._on_hkdata)`. 워크트리 전체에서 `register_report` 호출은 이 한 자리뿐이라, ICG 쪽은 등록된 조치가 0개야.  ⭐ **2026-09-23 개정**: 같은 자리(`IcsArchon.start()`)가 이제 **여섯**을 건다 — `HK`·`HKDATA` 둘 각각에 `DONE` → `_on_hkdata`, `ERROR`·`FATAL` → `_on_hk_refused`.  `HKDATA` 거절이면 `GO` 의 HK 질의(`fetch_icg_hk`)가 시한을 안 기다리고 곧바로 풀린다(카드 sentinel · 게이지는 추적 상태 · 노출은 간다), `HK` 거절(콘솔 `hk`)은 경고만 한다.  경고 한 줄 `ICG refused HKDATA (ERROR) -- <사유>` 에 원인 본문이 실리고, 조치가 걸린 뒤로는 기반의 *"보고 수신 (조치 없음)"* 줄이 안 나온다.  ICG 쪽은 여전히 0개다 |
| **ERASE** | ICS·ICG·IC | 없음 | CCD flushing (master 채널에서만) | 구현됨 — 원안 맞아. [검증: 400-409 전문 열람. ICG 의 sim 스텁 경고도 icg_archon/app.py:52 로 확인] |
| **ERROR:** | 공통 | `<본문>` | IMPv2 에러 알림 수신 처리 (메시지 타입) | 구현됨 — 원안 맞아. [검증: 위 DONE: 과 같은 구간]. ⭐ 덧붙일 것: ics_archon 은 ICG 의 `ERROR:` 를 **_on_message 에서 따로 엿들어** 게이지·노출잠금 데드맨을 푼다(ics_archon/ics_archon/app.py 의 `IcsArchon._on_message` -- `for ctl, word in ((getattr(self, 'gauge', None), GAUGE_CMD), (getattr(self, 'expenable', None), EXPENABLE_CMD)):` … `and _is_reply_to(msg, word)): ctl.note_reply(msg)`) — 명령 처리부가 아니라 수신 훅이야.  ⭐ 답인지는 `_is_reply_to` 가 **커맨드워드 + 메시지 종류**(`DONE:`/`ERROR:`/`FATAL:`)로 가르고, 거절 판정도 받는 쪽이 `msg.mtype` 으로 한다 — `ERROR:`·`FATAL:` 둘 다 거절이다 (2026-09-23 개정).  ⭐ `HK`/`HKDATA` 의 `ERROR:` 는 `register_report` 조치 `_on_hk_refused` 가 받는다 — 경고 한 줄(`ICG refused HKDATA (ERROR) -- <사유>`)을 남기고, `HKDATA` 면 `GO` 의 HK 질의를 곧바로 끝낸다(위 `DONE:` 줄) |
...
| **EXP** | ICS·ICG | `EXP [<초>]` | 노출시간(EXPTIME) 조회·설정 | 구현됨 — 원안 맞고 ICS·ICG 갈림도 확인했어. [검증: icg_archon/commands.py:163-171 전문 열람 — BIAS 가드가 정말 없고 `if arg: st.exptime = float(arg)` 뒤 곧장 `Reply.done('EXP', 'ExpTime=%g seconds.' % st.exptime)`. docstring 이 `_image_type` 과 같은 이유라고 명시].  ⛔ **2026-09-15 개정**: guide 도 `BIAS` 가드를 쓴다 — `IcgDispatcher.cmd_exp` 는 부모(`ics_sim/ics_sim/commands.py` `cmd_exp`)에 넘겨 `ERROR: EXP Cannot change EXPTIME for ImgType=BIAS` 를 낸다 (`GUIEXP` 도 같다).  위 *"가드 없음"* 은 옛 판이다 |
...
| **FATAL:** | 공통 | `<본문>` | IMPv2 치명 오류 알림 수신 처리 (메시지 타입) | 구현됨(수신) — 원안 맞아. [검증: MSG_TYPES 에 `'FATAL:'` 있음 확인]. 발신 대응물 없다는 것도 grep 으로 확인 — UDP 전환으로 UART 버퍼 폭주 상황 자체가 사라졌어.  ⭐ **2026-09-23 개정**: 받는 쪽은 `ERROR:` 와 같다 — `HK`/`HKDATA` 의 `FATAL:` 은 `_on_hk_refused` 가 받고(경고 `ICG refused HKDATA (FATAL) -- <사유>`, `HKDATA` 면 `GO` 의 HK 질의를 곧바로 끝낸다), `VACGAUGE`/`EXPENABLE` 의 `FATAL:` 은 `note_reply` 가 `msg.mtype` 으로 거절로 친다 |
| **FILENAME** | ICS·ICG·IC | 없음 | 현재 설정된 저장 파일이름 반환 | 구현됨 — 원안 맞아. [검증: 207-214 전문 열람] |
| **FLASHNOW** | IC (ICS·ICG 주소로도 받음) | `FLASHNOW [<ms>]` | 점검용 LED 를 n ms 즉시 점등 | 구현됨 — 원안 맞아. [검증: 443-459 전문. 문서 쪽에도 `FLASHNOW` 7회] ⛔ **2026-09-09 폐지**: 실기 ICS·ICG 둘 다에서 뺐다(`UNSUPPORTED`) -- 점검용 LED 는 실기에서 구현된 적이 없고(`flash_led()` 는 `_NOT_YET`), ICG 는 눌러 둔 sim 스텁을 불러 **거짓 `DONE`** 을 냈다.  그 자리는 `SHOPEN <초>`(science) · `TRIGOUT <ms>`(guide)가 대신한다.  ⚠️ 시뮬(`ics_sim`)에는 남는다. |
| **FLAT** | ICS·ICG | `FLAT [<objname>]` | IMAGETYP=FLAT 설정 | 구현됨 — 원안 맞아. [검증: :320-321] |
| **GO** | ICS·ICG | `GO [<장수>]` | 노출 개시 | 구현됨 — 원안의 3층 구조 서술 전부 확인했어. [검증: 층1 commands.py:462-482 전문(`count = max(1, int(arg))`, 인자 없으면 1장, `self.app.seq.start(count, msg.src)`). 층2 ics_archon/app.py:207-215 — `bad = self._inflight_reply('GO')` 를 **게이지보다 먼저** 두고 그 뒤 `gauge.before_exposure()`, 그다음 `super().cmd_go(...)`. 층3 icg_archon/commands.py:311-331 — `flag.allowed` 검사(:325-326) → `_op_in_flight` 검사(:327-329) → `super()`]. ⭐ 원안이 안 적은 층2 세부: GO 가 거절됐는데 취득 중이 아니면 게이지 되켜기 타이머를 자가 치유로 건다(app.py:219-231).  ⛔ **2026-09-15·09-23 개정**: 층2 순서는 이제 `_inflight_reply('GO')` → `super().cmd_go()` → **거절(`ERROR`)이면 그대로 돌려준다**(게이지도 되켜기 타이머도 안 건드린다 — 켜짐대기 10분이 그대로 흐른다) → 수락이면 `gauge.cancel_reenable()` → `app.begin_go()`.  `begin_go` 가 `HKDATA NOW` 질의 태스크를 띄우고 그 답의 `VACGAUGE` 로 `before_exposure(word)` 가 끌지 말지 정한다(`_hk_before_exposure`).  위 *"그 뒤 `gauge.before_exposure()`"* 와 자가 치유는 옛 판이다 — 거절이 타이머를 안 지우니 치유할 것이 없다 (`IcsDispatcher.cmd_go`) |
...
| **GUIDEEXP** | ICG (ICS 테이블에도 CASE) | `GUIDEEXP [<초>]` | 가이드 노출시간(독출 개시 간격) 조회·설정 | 구현됨(ICG) / 미구현(ICS) — 원안 맞아. [검증: 174-194 전문 열람. 음수 거부(:191-192)까지 있고 응답 문구를 조회·설정 양쪽에서 똑같이 내. ICS 쪽은 핸들러 목록에 `cmd_guideexp` 가 없어 `Didn't understand` 가 맞아. 어휘도 확인 — `ICG_COMMANDS` 에만 있고 기반 KNOWN_COMMANDS 에는 없어].  ⚠️ **현행 낱말은 `GUIEXP` 다** — 2026-09-08 에 운영자가 줄였고(11.45) 응답도 `GuiExp=<n> seconds.` 다.  ⛔ 이 행 이름을 안 바꾸는 것은 **레거시 실측 낱말이 `GUIDEEXP`** 이기 때문이다 (이 표가 대조하는 것이 그 레거시다) |
...
| **LEDFLASH** | ICS·ICG·IC | `LEDFLASH [<ms>]` | 점검용 LED 점등시간 조회·설정 | 구현됨 — 원안 맞아. [검증: :283 핸들러 + :443-452 `cmd_flashnow` 가 인자 없으면 `self.state.ledflash_ms` 를 쓰는 것(:450)까지 확인해서 '값만 정하고 점등은 FLASHNOW' 해석이 맞아] ⛔ **2026-09-09 폐지**: 실기 ICS·ICG 둘 다에서 뺐다(`UNSUPPORTED`) -- 점검용 LED 는 실기에서 구현된 적이 없고(`flash_led()` 는 `_NOT_YET`), ICG 는 눌러 둔 sim 스텁을 불러 **거짓 `DONE`** 을 냈다.  그 자리는 `SHOPEN <초>`(science) · `TRIGOUT <ms>`(guide)가 대신한다.  ⚠️ 시뮬(`ics_sim`)에는 남는다. |
...
| **SHCLOSE** | ICS·ICG·IC | 없음 | 셔터 즉시 닫기 (강제 중단용) | 구현됨 — 원안 맞아. [검증: 434-441 전문 열람] ⛔ **ICG 에서는 폐지** (2026-09-09) — guide 에는 셔터가 없어(frame-transfer) `TRIGOUT <ms>`/`TRIGOUT 0` 으로 갈렸다.  ICG 로 보내면 `ERROR … Not supported on this node` 다 (DevNote 11.50).  ⚠️ science(ICS)에서는 남되 **Trigger Out 강제**로 구현이 바뀌었다 — `LEVEL=1`+`FORCE=1` 한 적용으로 열고, 내림은 `LEVEL=0`+`FORCE=0`. |
| **SHOPEN** | ICS·ICG·IC | `SHOPEN <초> [<sourceID> USESTATUS]` | 셔터를 지정 시간만큼 개방 | 구현됨 — 원안 맞고 `USESTATUS` 지적도 정확해. [검증: 412-433 전문 열람. `parts = msg.body.split()` 뒤 `parts[0]`(초)와 `parts[1]`(sourceID)만 읽고 **`parts[2]` 는 어디서도 안 봐**. 늦은 응답 경로 `_do_shopen`(:427-433)이 `emit.ic_shutter_open` → sleep → `emit.ic_shutter_closed` 를 부르는데, 그 타입은 emitter 가 스스로 정해서 `USESTATUS` 스위치가 닿을 자리가 없어. 레거시와 어긋나는지 확인이 필요하다는 원안의 유보 그대로 남겨] ⛔ **ICG 에서는 폐지** (2026-09-09) — guide 에는 셔터가 없어(frame-transfer) `TRIGOUT <ms>`/`TRIGOUT 0` 으로 갈렸다.  ICG 로 보내면 `ERROR … Not supported on this node` 다 (DevNote 11.50).  ⚠️ science(ICS)에서는 남되 **Trigger Out 강제**로 구현이 바뀌었다 — `LEVEL=1`+`FORCE=1` 한 적용으로 열고, 내림은 `LEVEL=0`+`FORCE=0`.  ⭐ 2026-09-24: 종료가 시작된 뒤의 `SHOPEN <초>` 는 `ERROR: SHOPEN Shutting down -- not raised`(`SHOPEN 0` 은 받는다, `_refuse_if_stopping`) · 올림이 실패하면 **올렸거나 올리려 한** 컨트롤러를 모두 쉬는 상태로 내린 뒤(한 대의 내림이 실패해도 나머지를 다 내린다 — `_rest_each`) `ERROR: SHOPEN Failed: …` 로 답한다 (내림 실패는 로그에만, 답은 올림 실패).  ⚠️ **남이 끊으면(`ABORT`·`SHCLOSE`·새 `SHOPEN`/`CnTRIGOUT`·종료) 제 몫의 응답이 없다** — 올림 전·도중이면 `Shutter=Open` 이, 대기 중이면 `Shutter=Closed` 가 안 나가고 끊은 명령이 답한다(종료면 답이 없다).  ICG `TRIGOUT` 의 `Cut …` 한 줄과 다른 **알려진 차이**다. |
...
| **VERBOSE** | 공통 | `VERBOSE [ON\|OFF]` | 상세 출력 켜기·끄기 | 구현됨 — ⭐ **2026-09-11 신설**이다(원안의 *"미구현"* 은 그 전 상태다). ⭐ **화면만 탄다** — 로그 파일은 이 명령과 무관하게 **언제나 전부** 적는다 (운영자 2026-09-11).  ini `[behavior] verbose` 가 기동값이고(2026-09-11 에 `[logging]` 에서 옮겼다) 이 명령이 **재기동 없이** 그것을 민다.  ⛔ 어휘 밖 인자는 기본값으로 떨어뜨리지 않고 **거부**한다.  [검증: `ics_sim/ics_sim/commands.py` `cmd_verbose` — `set_verbose()`/`verbose_state()` 왕복까지 열람] |
...
| **C1TRIGOUT / C2TRIGOUT** | ICS | `C1TRIGOUT <ms>` · `C2TRIGOUT <ms>` | 컨트롤러 1(MK)/2(NT) 의 Trigger Out 을 `<ms>` 동안 HIGH.  `0` 이면 즉시 내림 | 신설 — 운영자 2026-09-12.  ⭐ **`shutter_ctrl` 지정 여부와 무관하게** 그 컨트롤러를 움직인다 (배선 점검·예비 유닛 시험용).  `SHOPEN`/`SHCLOSE` 는 지정된 것만 움직인다.  ⚠️ 단위가 다르다 — `SHOPEN` 은 **초**, 이쪽은 **밀리초**(ICG `TRIGOUT` 과 같은 규약).  ⭐ 내려갈 쉬는 상태가 갈린다 — 셔터를 모는 쪽은 `FORCE=0`(스크립트에 반환), 안 모는 쪽은 `FORCE=1`(붙든다).  ⭐ **끊는 자리** (2026-09-23 개정): `ABORT`·종료(`release_pulse`)가 도는 펄스를 끊고 그 컨트롤러의 쉬는 상태로 내린다 · 같은 컨트롤러의 새 `CnTRIGOUT` 이 앞 펄스를 끊는다 · `SHOPEN`/`SHCLOSE` 가 **셔터를 모는 컨트롤러**의 대기 중 펄스를 끊고 선을 가져간다(`_take_from_trigout`, 경고 `… took the line from a pending CnTRIGOUT pulse`) · 거꾸로 셔터를 모는 쪽 `CnTRIGOUT` 은 도는 `SHOPEN` 을 끊는다(`shutter_ctrl = both` 면 다른 쪽도 내린다).  펄스는 올림 앞부터 내림 끝까지 핸들을 쥔다.  ⭐ 끝 응답은 **내림 뒤 하나** — `DONE: CnTRIGOUT TrigOut=Low Ctrl=<MK\|NT> Width=<ms>ms` 또는 `ERROR: CnTRIGOUT Failed: …`.  ⚠️ **남이 끊으면(올림 전·도중·대기 중 어디서든) 제 몫의 응답이 없다** — 끊은 명령이 답한다(종료면 답이 없다).  ICG `TRIGOUT` 은 올림 뒤에 끝 응답을 내고, 올림이 확인되기 전에 끊기면 `ERROR: TRIGOUT Cut before the raise was confirmed (MS=<ms>) -- …` 를 낸다(대기 중에 끊기면 끝 응답은 이미 나갔다) — **알려진 차이**로 남긴다.  ⭐ 2026-09-24: 종료가 시작된 뒤의 `CnTRIGOUT <ms>` 는 `ERROR: CnTRIGOUT Shutting down -- not raised`(`CnTRIGOUT 0` 은 받는다, `_refuse_if_stopping`) · 올림이 실패하면 그 컨트롤러를 쉬는 상태로 내려 본 뒤 `ERROR: CnTRIGOUT Failed: …` 로 답한다 (내림 실패는 로그에만, 답은 올림 실패).  [검증: `ics_archon/app.py` `cmd_c1trigout`/`_trigout_cmd`/`_do_trigout_pulse`/`release_pulse`/`_take_from_trigout` + 어휘 `ICS_OPS_COMMANDS` + 시험 `test_ics_ops_commands.py` — 이름에 `cntrigout` 이 든 것들 + `SHOPEN` 과 함께 도는 `test_shutdown_in_the_rest_window_still_rests_the_line`·`test_a_trigger_line_failure_is_ascii_on_the_wire_and_raw_in_the_log`·`test_a_raise_after_the_shutdown_released_the_pulses_is_refused`·`test_nan_and_inf_durations_never_raise_a_line` (수는 세지 않는다 — 함수 이름으로)] |
...
| **HK / HKDATA** | ICS·ICG | `HKDATA [NOW]` · `HK [NOW]` · 별칭 **`HKNOW`**(= `HK NOW`, 인자 거절) | HK(하우스키핑) — ICG 가 만들고 ICS 는 `GO` 마다 `HKDATA NOW` 로 물어 헤더에 싣는다 (2026-09-15, 파일 경로 없음) | 신설 (2026-09-15 개정).  [검증: `ics_archon/app.py` `_ask_icg(cmdword, body)` 가 `NOW` 만 받아 그대로 넘기고(다른 인자는 거절), 즉답은 `Queried … -- its … report follows separately` 다(안 기다린다).  답은 `register_report('DONE', word, self._on_hkdata)` 로 받고, `_on_hkdata` 는 `self.hk_wire` 에 담고 `log.debug` 한 줄만 남긴다(`print` 없음, 11.94-l).  ⭐ `GO` 가 부른 `fetch_icg_hk` 의 Future 를 풀어 **헤더 5.6절의 원천**이 된다.  ICG 가 `ERROR:`/`FATAL:` 로 거절하면 `register_report('ERROR'/'FATAL', word, self._on_hk_refused)` 가 받아 경고 한 줄(`ICG refused HKDATA (ERROR) -- <사유>`)을 남기고, **`HKDATA` 거절이면** 같은 Future 를 곧바로 푼다(시한 안 기다림 · 카드 sentinel · 게이지는 추적 상태).  `HK` 거절(콘솔 `hk`)은 경고만 한다 — `GO` 는 `HKDATA NOW` 로 묻기 때문이다.  ⚠️ 콘솔 `hkdata` 의 거절은 `GO` 의 질의와 못 가르므로 그때 기다리던 `GO` 몫도 함께 끝난다.  별칭 `HKNOW` 는 벤치 첫날(2026-09-15) 운영자 지시 — ICS 는 와이어로 `HK NOW` 를 보낸다(답 `DONE: HK …` 를 `register_report` 가 받는다).  ⛔ 종전 판의 *"값을 헤더로 흘리지는 아직 않는다"*·*"콘솔에 찍기만 한다"* 는 2026-09-15 전 서술이다] |
| **C1HKDATA / C2HKDATA** | ICS·ICG (ICG 는 `C1HKDATA` 만) | `CnHKDATA [NOW]` · 별칭 **`CnHK [NOW]`** · **`CnHKNOW`**(= `CnHK NOW`, 인자 거절 — ICG 는 `C1HKNOW` 만) | 컨트롤러 텔레메트리 한 줄 — 헤더 `Cn_TEMP/VOLT/CURR` 의 와이어 판, `CnSTALE`·`CTRLnID` 동반 | 신설 — 운영자 확정 2026-09-04, 구현 2026-09-15 (`hkwire.ctrl_hkdata_body` 를 ICS·ICG 가 같이 쓴다).  별칭은 벤치 첫날(2026-09-15) 운영자 지시 — 답의 커맨드워드는 받은 그대로 |
| **IMAGETYPE / IMAGETYP / IMGTYP** | 공통 (기반 `ics_sim`) | `IMAGETYPE` | 지금 이미지 종류 **조회만** — `ImageType= ObjectName= EXP=` (설정 명령들의 응답과 같은 모양).  인자는 거절 | 신설 — 벤치 첫날(2026-09-15) 운영자 지시.  설정은 종전대로 `OBJECT`/`BIAS`/…  ⭐ ICG 는 `BIAS` 뒤 조회도 `EXP=` 가 **guide 최소 노출**이다 — 부모의 `effective_exptime`(BIAS 면 0) 대신 `st.exptime` 을 싣는다(`IcgDispatcher._image_type_query`, 2026-09-23).  guide `BIAS` = 최소 노출이라 `BIAS` 응답·`EXP`/`GUIEXP` 조회·헤더 `EXPTIME` 과 같은 값이다 |
...
| **TRIGOUT / TRIGOUTFORCE / TRIGOUTLEVEL** | ICG | `TRIGOUT <ms>` \| `TRIGOUT 0` · `TRIGOUTFORCE [ON\|OFF]` · `TRIGOUTLEVEL [0\|1]` | guide Trigger Out 선 — n ms 펄스 / 강제할지 / 강제했을 때의 레벨 | 신설 — **guide 에는 셔터가 없다**(frame-transfer). `SHOPEN`·`SHCLOSE`·`FLASHNOW` 가 폐지된 자리를 이 셋이 대신한다 (2026-09-09, DevNote 11.50).  ⚠️ **`FORCE` 와 `LEVEL` 은 다른 것이다** — `TRIGOUT 0` 도 펄스의 자동 내림도 **강제를 풀지 않는다**.  ⭐ 쉬는 상태가 `TRIGOUTFORCE=1`·`TRIGOUTLEVEL=0` 이다(선을 우리가 붙들어 LOW 로 고정, 운영자 2026-09-08).  ⭐ **`TRIGOUT <ms>` 의 끝 응답은 꼭 하나다** (2026-09-23 개정 · 2026-09-24 문면) — 올림 뒤 `DONE: TRIGOUT TRIGOUTLEVEL=1 TRIGOUTFORCE=1 MS=<ms>` · 올림 실패 `ERROR: TRIGOUT Failed: …`(선을 쉬는 상태로 한 번 내려 본 뒤 — 적용이 나간 뒤의 시한 초과면 선은 이미 HIGH 다.  내림 실패는 로그에만) · 올림이 확인되기 전에 끊기면(`ABORT`·`EXPENABLE OFF`·새 `TRIGOUT`·`TRIGOUT 0`·종료) `ERROR: TRIGOUT Cut before the raise was confirmed (MS=<ms>) -- <선을 맡은 쪽>`(`_trigout_cut` — 꼬리는 `the line is being rested` / `a new TRIGOUT took the line`, 종료 막바지라 발신 길이 닫혔으면 로그에만) · 종료 중이면 `ERROR: TRIGOUT Shutting down -- not raised`(`IcgDispatcher.stopping`).  ⚠️ 올림 `DONE` 뒤 대기가 안 끊겼으면 부르지 않은 통보가 하나 더 온다 — 시한 내림 `DONE: TRIGOUT TRIGOUTLEVEL=0 TRIGOUTFORCE=1 (auto after <ms> ms)` 또는 그 내림의 실패 `ERROR: TRIGOUT Auto lower failed after <ms> ms: …`.  ⚠️ ICS `SHOPEN`·`CnTRIGOUT` 과 응답 모양이 다르다 (그쪽은 남이 끊으면 제 몫의 응답이 없다 — 알려진 차이, README "`STOP` · `ABORT` 와의 관계" 표).  [검증: `icg_archon/commands.py` `cmd_trigout`·`_do_trigout_pulse`·`_trigout_cut`·`cmd_trigoutforce`·`cmd_trigoutlevel` + 어휘 `ICG_COMMANDS` — ⛔ 줄 번호는 코드가 바뀔 때마다 밀려 함수 이름으로 적는다] |
| **VACGAUGE** | ICG (ICS 는 발신) | `VACGAUGE [ON\|OFF]` | 이온게이지 켜기·끄기·조회 | 신설 — 원안 맞아. [검증: gaugectl.py:57 + icg_archon/commands.py:473 + 어휘 :95 확인. ⭐ 원안이 안 적은 배선 하나 — ICS 는 이 응답을 **명령 처리부가 아니라 `_on_message` 훅에서 엿들어** 데드맨을 풀어(`IcsArchon._on_message` — `if (ctl is not None and ctl.enabled and msg.src.upper() == ctl.node.upper() and _is_reply_to(msg, word)): ctl.note_reply(msg)`). gaugectl.py:43 주석도 *"답이 안 와도 우리는 모른다"* 라고 그 이유를 적어 뒀어. `EXPENABLE` 어휘 미등록 주의는 이 낱말도 똑같아].  ⛔ **2026-09-15·09-23 개정**: 종전 판정 `word in raw`(원문 부분 문자열)는 걷었다 — `HKDATA` 답 본문의 `VACGAUGE=ON` 으로 게이지 데드맨이 풀리던 벤치 결함 때문이다.  지금은 `_is_reply_to(msg, word)` 가 **커맨드워드가 같고 종류가 `DONE:`/`ERROR:`/`FATAL:`** 일 때만 답으로 치고(`EXEC:` 는 답이 아니다), `note_reply` 가 `Message` 째로 받아 `ERROR:`·`FATAL:` 을 거절로 가른다.  ⭐ 2026-09-24: ICG 의 실패 응답에 **남은 상태 낱말**이 붙는다 — `ERROR: VACGAUGE Failed: … (Gauge=<낱말>)`.  `gauge.set()` 이 실패한 자리에 따라 직전 상태로 되돌리거나(다시 치면 된다) 모름(`UNKNOWN` — 게이지가 켜졌을 수 있다)으로 두므로, 할 일이 갈린다 |
