...
### P1 — `ABORT` 와 노출 번호 : ⏳ **규범 미정.  이 시험이 정할 자료를 만든다**
...
* `State.load_expnum()` 은 *"마지막으로 쓴 번호 **+1**"* 부터 시작한다 → 재시작하면 그 번호를
  **건너뛴다**(영구 구멍).

**→ 그래서 비대칭이다.**  같은 현상을 두 규칙이 다르게 다룬다.

| # | 하는 것 | 기대 (현행 코드) | 실측 |
|---|---|---|---|
| P1-1 | `expnum` 으로 현재 번호를 적어 둔다 → `go 1` → 독출 시작 뒤 `abort` | 파일이 **안 생긴다**.  ICG 는 flush 1회가 돈다 | |
| P1-2 | `expnum` 다시 | ⭐ **번호가 그대로**다 (advance 를 건너뛰었다) | |
| P1-3 | `expnum_file` 을 눈으로 (`~/AIC/icg_archon.expnum` 등) | ⛔ **그 번호가 이미 적혀 있다** — 이것이 비대칭의 씨앗이다 | |
| P1-4 | 이어서 `go 1` (재시작 없이) | 파일 이름이 **P1-1 과 같은 번호**를 쓴다 (재사용) | |
| P1-5 | 다시 `abort` → **프로그램 재시작** → `go 1` | 이번엔 그 번호를 **건너뛴다** (`load_expnum` 이 마지막+1) | |
| P1-6 | 기동 로그 | `expnum 기록을 이어받는다 -- 마지막 %06d, 이번 %06d` 가 그 점프를 보여준다 | |

**⏳ 운영자가 정할 것 — 규범 둘 중 하나**

| 안 | 뜻 | 대가 |
|---|---|---|
| **① 재사용** (같은 프로세스 거동을 정본으로) | `ABORT` 는 번호를 안 먹는다.  재시작해도 그 번호부터 | `ABORT` 때 `expnum_file` 을 **되감아야** 한다.  ⚠️ *"노출 중 죽었다"* 와 구별해야 하는데, `ABORT` 는 정상 종료라 구별이 된다.  번호 공간에 구멍이 없어 아카이브가 연속이다 |
| **② 건너뜀** (재시작 거동을 정본으로) | `ABORT` 도 번호를 먹는다.  같은 프로세스에서도 `advance()` 한다 | 번호에 **영구 구멍**이 남고, 나중에 보면 그 구멍이 `ABORT` 인지 사고인지 **구별할 수 없다** |

⭐ **판단 재료**: 이름 충돌 처리(D-016)가 이미 있어 번호가 겹쳐도 `resolve_pair_number()` 가
밀어내고 경고를 남긴다 — 그래서 ①의 위험은 이미 받쳐져 있다.  반대로 ②는 구멍의 뜻을
잃는다.  ⚠️ 다만 ①은 `ABORT` 경로에 되감기 코드를 새로 넣는 일이라 **`_record_expnum` 의
원래 목적**(노출 중 죽었을 때 겹침 방지)을 깨지 않는지 함께 봐야 한다.

⚠️ **ICG·ICS 각각 재라** — 카운터가 독립이다 (`icg_archon.expnum` / `ics_archon.expnum`,
raw spec 9.2절 "카운터 독립").
