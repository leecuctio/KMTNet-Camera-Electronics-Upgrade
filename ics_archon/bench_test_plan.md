...
- guide 유닛 `10.0.0.162` 접속 · 어느 ACF 가 적용돼 있나.  ⛔ **판은 파일명 전체로 적는다**
  (`acf/README.md` 머리의 규칙 셋) — 저장소 현행은 **`KMTK_GUI_162_STA0201_R2622.acf`**, 벤치에
  마지막으로 확인된 것은 **`KMTK_GUI_162_STA0201_R2619.acf`** (2026-09-11).  둘 사이 세 판이다.
  ⚠️ **KASI guide 상자가 둘이다** — `STA0201` 과 `STA0230` 이 같은 `.162` 를 쓴다.
  어느 쪽이 붙었는지는 `BACKPLANE_ID` 로 가린다 (`…1A99369B` / `…1A9939ED`).
...
> 전량+`APPLYALL`+`POWERON` 이 돈다** -- ⭐ **늘 그렇다**: `apply_acf` 눈금은 걷었고
> 기동마다 컴퓨터의 ACF 를 적용한다 (2026-09-12).
>
> ⭐ **그래서 1단계를 돌리는 길이 둘이다:**
>
...
- **멈출 조건**: 1 에서 부호 규약이 어긋나면 `rawhdr.format_temp`/`format_ens` 를 먼저
  본다(규격 5.0 · 5.8).  ⛔ `POWERON` 이 `?xx` 로 거절당하면 (⚠️ **첫 `go` 가 아니라 기동 로그**에 뜬다 — 기동이 이미 전원을 켠다.  `app.py` 가 그 실패를 로그로만 남기고 기동은 계속한다) **그 기동의 `APPLYALL` 이 실패했거나 그 뒤에 REBOOT·전원 재투입이 있었다는 뜻**이다 — 기동 로그의 ACF 적용 줄을 먼저 보고, `icg_first_run` 0단계로 돌아간다.
...
### DG 정적 덤프 실측 (FRAME6 후속 판 **R2624** 판단)

> ⚠️ 예약 번호를 **R2621 → R2624** 로 다시 밀었다 (2026-09-14) — R2621 은 DG 상수
> 이름 판이, R2622 는 `FlushFrame` DGHIGH 판(P3)이 가져갔다.  종전에 R2620 을 guide 의
> 파라미터 슬롯 순서 판올림이 가져갔다 (acf/README 의 `science R2612 · guide R2620` 절).
...
### ⏳ `RESETTIMING` 이 호출 스택을 지우는가 (2026-09-14 신설, science R2613 후속)

⛔ **매뉴얼이 말이 없다** — p.52 의 `RESETTIMING` 항도 p.64 의 RESET 서술도 스택을 한
마디도 안 적는다.  R2613 이 `FlushFrame:` 을 **서브루틴**으로 바꾸면서 호출 깊이가
최대 **6단**(한계 16)이 됐다.  abort 가 독출 한복판(최대 4단)에 떨어지는데 SP 가 안
지워진다면, abort 를 거듭할수록 낙진이 쌓여 언젠가 16 에 닿는다.

| 절차 | 판정 |
|---|---|
| `go 5` 를 걸고 **독출 한복판에서 ABORT 를 연속 4회** (`abort_now` → `Exposures=0` + `RESETTIMING`) | 그 뒤 `go 1` 이 **정상 프레임**을 내면 SP 가 지워지는 것이다 ✓ |
| 〃 | 5회째쯤 **코어가 멎으면** 안 지워지는 것이다 ⛔ — 그때는 `FlushFrame` 을 `GOTO` 꼴로 되돌리거나 abort 뒤 `LOADTIMING` 을 한 번 낸다 |

⚠️ **차단 조건은 아니다** — 실운용 abort 는 드물고, 종전에도 `Prep`/`Flush` 로 4단은 썼다.

---

### ⭐ FW `1.0.1271` 재실측 — `LOCK`/`FETCH` 보고서를 다시 받는다 (2026-09-12 신설)

[`archon_lock_fetch_report.md`](archon_lock_fetch_report.md) 는 **`1.0.1261`**
(Rev H) · **`1.0.1252`**(Rev F) 때 잰 것인데, 2026-09-11 초기화 시험으로
**관측소 상자가 `1.0.1271` 로 올랐다**.

⭐ **기능 차이는 없을 것으로 본다** — 운영자 2026-09-12, *"디버깅만 한 걸로
알고 있어"*.  종전 보고서도 판이 다른 두 유닛에서 **같은 값**을 냈다.
⛔ **그래도 다시 잰다** — 이 값들이 `MIN_FRAME_PERIOD`·`fetch_timeout`·
`lock_buffer` 판단의 근거라, *"아마 같을 것"* 으로 두면 틀렸을 때 드러나는
자리가 **프레임이 덮인 뒤**다.

| 항목 | 재는 법 | 판정 (종전 값) |
|---|---|---|
| 독출 라인 속도 | `tools/ics_archon_buftest.py`(2x2, 본편 무수정) | **368.0 행/초** (±0.1%) |
| 독출 시간 (4700행) | 같은 도구 | **12.77 s** |
| `LOCKn` 이 FW 에 반영되나 | `LOCK` 직후 `RBUF` 확인 | 잠근 버퍼를 가리킴 (종전 15/15) |
| `LOCKn` 이 fetch 를 늦추나 | `lock` 조건 vs `idle` 조건 | **감속 0** (20초를 쥐어도) |
| `LOCK` 없이 fetch | 프레임 경계가 fetch 에 걸릴 때 | 엔진이 읽는 중인 버퍼를 집어 간다 (종전 2/2) |

⚠️ **대상은 Rev H 상자다** (`BACKPLANE_VERSION=1.0.1271` 확인 후 시작).
Rev F(KASI, `1.0.1252`)는 판이 안 바뀌었으므로 **대조군으로만** 한 번.
⭐ 결과가 같으면 보고서 머리의 ⏳ 를 걷고 판 목록에 `1.0.1271` 을 더한다.
⛔ 다르면 그 자리가 곧 `MIN_FRAME_PERIOD`(**12.78**) 재계산 사유다.
...
`NoIntMS` 다.  ⛔ 짧으면 **셔터가 닫히는 중에 독출이 시작돼 프레임에 빛이 샌다** — 그리고
그것은 조용하다(오류도 경고도 없다).
