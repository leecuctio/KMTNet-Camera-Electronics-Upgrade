...
## ⛔ 판을 부르는 규칙 셋 (운영자 2026-09-13 · 명문화 2026-09-14)

1. **판 번호는 계열별이다** — science 와 guide 가 **각각 독립**으로 센다.  `R2608`~`R2613` 이
   두 계열에 다 있는 것은 충돌이 아니라 설계다 (파일명에 `SCI`/`GUI` 가 박혀 파일·`CTRLnCFG`
   수준에서는 애초에 안 겹친다).
2. **글·말에서는 반드시 계열을 붙인다** — *"guide R2622"* · *"science R2613"*.  번호만 쓰지 않는다.
3. **벤치·현장에 판을 지시할 때는 파일명 전체로** — `KMTK_GUI_162_STA0201_R2622.acf`.
   *"R2613 깔아 줘"* 는 갈리지 않는다.

⭐ **어느 상자에 어느 판이 깔려 있나는 [`deployment_ledger.md`](deployment_ledger.md)** (설치 대장)
— 이 README 는 *구운 판*만 알고, 깐 판은 그 대장만 안다.  ⛔ 2026-09-14 현재 현장은 저장소보다
**다섯 판 뒤**다 (science `R2611` · guide `R2619` 가 마지막 확인).

## 목록 — 계열별

`BACKPLANE_ID`·IP·REV·FW 같은 **상자 신원**은 [`deployment_ledger.md`](deployment_ledger.md) 에 있다.
아래 두 표는 **저장소 현행 파일**이다 (구판은 `archive/`).

### science — **R2613** 8장 (`LINES=143` · `PARAMETERS=23`)
...
| `KMTK_SCI_112_STA0212_R2613_MK.acf` | KASI 시험 유닛 **둘째 상자** (MK) ⭐ 2026-09-11 신규 | **32** | 1 | **1200** × 4700 | `.112` | 5 · 1.0.1252 |
| `KMTK_SCI_112_STA0212_R2613_NT.acf` | KASI 시험 유닛 둘째 상자 (NT) ⭐ 2026-09-11 신규 | 33 | 1 | **1200** × 4700 | `.112` | 5 · 1.0.1252 |
...
### guide — **R2622** 4장 (`LINES=122` · `PARAMETERS=17`)

| 파일 | 유닛 | `TAPLINES` | `BIGBUF` | **저장** 픽셀/탭 × 줄 | IP | 백플레인 REV · FW |
|---|---|---:|---:|---|---|---|
| `KMTC_GUI_161_STA0290_R2622.acf` | CTIO guide ⭐ 2026-09-11 신규 | 9 | **0** | **528** × 1033 | `.161` | 7 · 1.0.1271 |
| `KMTS_GUI_161_STA0291_R2622.acf` | SAAO guide ⭐ 2026-09-11 신규 | 9 | **0** | **528** × 1033 | `.161` | 7 · 1.0.1271 |
| `KMTK_GUI_162_STA0201_R2622.acf` | KASI guide | 9 | **0** | **528** × 1033 | `.162` | 5 · 1.0.1252 |
| `KMTK_GUI_162_STA0230_R2622.acf` | KASI guide **둘째 상자** ⭐ 2026-09-11 신규 | 9 | **0** | **528** × 1033 | `.162` | 5 · 1.0.1252 |

⭐ 열두 장의 **상자·신원·채널 offset** 은 2026-09-11 각 유닛 초기화 시험 반입분 그대로이고,
그 뒤 판올림은 **타이밍 스크립트·파라미터 슬롯**만 바꿨다.  그 왕복에서 무엇이 바뀌었는지는
아래 "2026-09-11 각 유닛 초기화 시험" 절.
...
### ⛔ 백플레인 판이 둘이고, **올릴 수 있는 FW 최종판이 서로 다르다**

운영자 확정 2026-09-12.  *"보드 버전이 달라서 올릴 수 있는 FW 최종 버전이 달라."*

| 상자 | `BACKPLANE_REV` | 보드 | **그 보드의 FW 최종판** | 쓰는 유닛 |
|---|---:|---|---|---|
| 관측소 | **7** | Rev **H** | **`1.0.1271`** | CTIO 3 · SAAO 3 |
| KASI 벤치 | **5** | Rev **F** | **`1.0.1252`** | KASI 6 (science 4 · guide 2) |

⛔ **"최신" 으로 통일하려 들지 말 것** — `1.0.1271` 은 Rev H 의 최종판이지
Rev F 보다 새 판이 아니다.  **보드에 맞는 이미지만 굽는다.**
⭐ 대응은 추론이 아니라 자료다 — FW 이미지 파일명(`…revf…`/`…revh…`)과 ACF 의
`BACKPLANE_VERSION` 이 열두 장에서 정확히 짝을 이룬다
(`REV=5`+`1.0.1252` 여섯 · `REV=7`+`1.0.1271` 여섯).

**이미지가 있는 곳**: `../__ref_archon_control/ArchonFW/<이름>/<이름>.mcs`
(Intel HEX 원본을 그대로 담는다).  **각 보드에 굽는 것은 이 둘이다**:

| 보드 | 구울 이미지 |
|---|---|
| Rev **H** (관측소) | `archonbackplanerevh_1_0_1271/` |
| Rev **F** (KASI) | `archonbackplanerevf_1_0_1252/` |

⭐ 모듈 FW 는 보드 판과 무관하게 같은 것을 쓴다 — `archonadrevk_1_0_1175` ·
`archondriverrevd_1_0_1175` · `archonhvxbiasreva_1_0_1175` ·
`archonlvdsreva_1_0_1175` · `archonheaterxreva_1_0_1192`.

⭐ **`OldVer/` 에는 진짜 구판만 둔다** — 지금은 `archonbackplanerevf_1_0_1175/`
하나다 (Rev F 의 구판 → `1.0.1252`).
⚠️ 받은 묶음에서는 **Rev H 최종판이 `OldVer` 쪽에 잘못 들어 있었다**
(운영자 정정 2026-09-12) — 관측소 여섯 대가 쓰는 판이라 제자리에 두었다.
⛔ 앞으로도 **폴더 이름만 보고 지우지 말 것** — 무엇이 최종판인지는 위 표가
정본이다.

⚠️ **실측을 옮길 때 이 차이를 먼저 볼 것** — 벤치(Rev F)에서 잰 값이 관측소
(Rev H)에서 그대로 서는지는 따로 확인해야 한다.
⭐ 지금 있는 근거: [`../archon_lock_fetch_report.md`](../archon_lock_fetch_report.md)
가 **`1.0.1261`(Rev H) 과 `1.0.1252`(Rev F) 두 판에서 같은 값**을 얻었다
(독출 368.0 행/초 · `LOCK` 반영 15/15).  ⏳ 그 뒤 관측소가 **`1.0.1271`** 로
올랐고 **그 판은 다시 잰다** — 벤치 계획서 3단계 *"FW `1.0.1271` 재실측"*.
운영자는 *"디버깅만 한 판이라 기능 차이는 없을 것"* 으로 본다 (2026-09-12).

⚠️ **IP 가 겹치는 것은 정상이다** — 사이트마다 내부망이라 CTIO 와 SAAO 의
science 1 이 둘 다 `.101` 이다.  ⛔ **그런데 KASI guide 둘(`STA0201`·`STA0230`)은
같은 망에서 둘 다 `.162` 다** — 동시에 못 띄운다.  어느 상자를 쓰는지는 ini
`[icg] acf` 가 정하고, 실제로 붙은 상자는 `BACKPLANE_ID` 로 가린다
(`STA0201` = `…1A99369B` · `STA0230` = `…1A9939ED`).

⚠️ **KASI 의 science 두 상자는 MK/NT 가 한 상자다** — `STA0212`·`STA0200` 각각
`BACKPLANE_ID` 가 MK·NT 파일에서 같다(`…1A7068D5` · `…1A708986`).  관측소는 MK·NT
가 서로 다른 상자다.

⏳ **SSO(`KMTA_*`) ACF 는 아직 없다** — 위 열둘은 CTIO·SAAO·KASI 몫이다.
반입되면 이 표와 아래 대조를 다시 돌릴 것.
...
## 2026-09-11 각 유닛 초기화 시험 — 열두 장을 다시 받았다

받은 자리는 `__ref_archon_control/acf_20260911/` 이고(그 전 것은 같은 폴더의
`acf_labtest/`), 이 폴더는 그 사본이다.
...
| 자리 | 무엇이 | 성격 |
|---|---|---|
| `BACKPLANE_VERSION` | 관측소 넉 장 `1.0.1261` → **`1.0.1271`** | 컨트롤러 FW 판 — 사람이 개정한 값이 아니다 |
| `MOD5/6/7/10_VERSION` (`STA0201`) | `1.0.1104`·`1.0.1175` → **`1.0.1175`·`1.0.1192`** | 모듈 FW 판 |
| `MOD2_ID` (`STA0284`) | `013E551EF60876F3` → **`013D87EF8EE127DC`** | ⚠️ **드라이버 모듈이 갈렸다** — 상자는 같고 모듈이 바뀐 것이다 |
| `TAPLINE0`·`TAPLINE1` (`STA0201`) | `1018`·`1032` → **`1000`** | ⭐ 아래 참조 |

⭐ **guide 의 채널 offset 이 1000 으로 정돈됐다.**  그 전 `1018`/`1032` 는
상자마다 다른 실측값처럼 보였는데, 이번에 받은 넉 장의 guide 가 **백플레인 판도
FW 도 다른데 전부 `1000`** 이고 science 도 유닛과 무관하게 MK `1005` · NT `1007`
로 같다.  ⛔ 즉 이 표는 **채널별 실측 보정값이 아니라 명목 pedestal** 로 읽어야
한다 — 하류가 `1003~1007` 의 계단을 "채널 특성" 으로 읽으면 안 된다.
⏳ 값의 출처는 운영자 확인 대기다.

⛔ **같은 판 이름으로 내용이 달라진 자리가 있다** — 이 폴더의 규범은 *"같은 이름
...
| 어긴 것 | 결과 |
|---|---|
| `IntMS` 또는 `Exposures` 가 없다 | ⛔ **노출 준비가 멈춘다**(`GO` 가 `prepare()` 에서 실패한다) — 그 ACF 로는 노출을 못 건다.  프로그램은 떠 있다 (아래 "설정에서 뺀 것" 끝 문단) |
...
⭐ 매뉴얼의 *"parameter list"* 는 **슬롯 번호 순**으로 읽는다 (운영자 확정 2026-09-13) —
검사(`_require_exposures_last`)는 **슬롯 번호로만** 보고, `Exposures` 보다 뒤 번호가 하나라도
있으면 멈춘다.  ⚠️ 종전에는 설정 메모리 줄 순까지 둘 다 봤는데, 파라미터가 열 개를 넘으면 두
순서를 함께 만족하는 "맨 마지막" 슬롯이 없어 걷었다 (근거는 그 함수 앞 주석).
...
⭐ 우회로가 없어지는 대신, 어긋나면 **노출 준비(`prepare()` 의 `_require_param_slots`)에서
분명히 멈춘다** — 종전처럼 조용히 엉뚱한 슬롯을 덮지 않는다.  ⚠️ **프로그램이 서는 것이
아니라 `GO` 가 실패한다** (`GO` 는 받아들여진 뒤 그 `prepare()` 에서 `ERROR` 로 끝난다):
* **guide** — 기동의 `prepare()` 가 실패해도 ICG 는 **안 멈추고 떠 있다** — 로그 `guide
  controller connect failed at startup -- <사유>` 한 줄이 남고, 그 뒤 **모든 `GO` 가 `prepare()`
  에서 실패한다** (`ERROR: GO Failed to initialize guide controller`.  ACF 는 이미 밀렸으므로
  되밀지 않고 판정만 되풀이한다).
* **science** — 기동은 접속만 하므로 기동에서는 안 드러난다.  **첫 `GO` 부터** 그 `GO` 의
  `prepare()` 가 ACF 를 민 뒤 판정에서 실패한다 (그 뒤의 `GO` 도 같다).
고치는 길은 규약에 맞는 ACF 로 ini 를 고친 뒤 **재기동**이다.
...
⚠️ **`Pixels` 트림은 P-k(실기 시험 항목)이다** -- 트림 전후 바이트가 같으면 저장 창이
앞 528 임이 확정된다.  ⛔ 선두 16 의 **귀속**과는 별개다 -- 규격 v1.13 이 OI-20 을 닫으며
*"이 물음과 별개"* 로 못박았다(CU 협의 완료 2026-09-08): 16 은 CCD 의 dark reference
columns 라 **`PRESCNX=16` · `OVRSCNX=0`** 이다.  (종전 이 절이 `OVRSCNX=16` 귀속을 P-k 가
뒷받침한다고 적었던 것은 인과가 아니다 -- 걷었다, 2026-09-15.)
...
소자)과 성격이 다르다.  헤더는 그 16 을 **`PRESCNX`** 에 귀속한다 (규격 v1.13 10.3절 ·
OI-20 종결: `PRESCNX=16` · `OVRSCNX=0`, comment 에 dark reference columns 병기).
⚠️ v1.12 까지는 `OVRSCNX=16` 이었다 -- 옛 guide 파일을 볼 때 그 판을 가늠할 것.  독출 순서로는 항상
...
기록하지 않는다(`rawhdr.py`).  guide 는 `AMPNAX1 = 528 = PRESCNX(16) + IMAGEX(512) +
OVRSCNX(0)` 으로 같은 불변식을 따른다 (규격 v1.13 10.3절).  ⚠️ 위 `y_trim` 가정(9행의
위치)은 여전히 실측 미확정이다 (OI-21).
...
⭐ **판 번호는 계열별이다** — `R2608`~`R2613` 이 guide·science 양쪽에 다 있고 그게 정상이다.
규칙 셋은 이 문서 **머리**에 있다 (계열을 붙인다 · 벤치 지시는 **파일명 전체**로).
⛔ science 쪽도 밀렸다 — **science R2613 은 P6(`EveryFlush`)가 가져갔고** P2(`RGHIGH`)는 번호
미정으로 물러났다 (위 "science R2613" 절 · DevNote 11.86-(11)·(12)).
...
[`../__ref_archon_control/acf_labtest/`](../__ref_archon_control/acf_labtest/) 로 옮겼다.  그
...
`../__ref_archon_control/acf_labtest/` 에 있다.
...
`R2608` · `R2601` 은 ACF 개정 번호이고 **`YYMM`** 으로 읽는다.  ⛔ **계열별로 센다** — 같은
번호가 science 와 guide 에 각각 있다.  부르는 규칙 셋은 이 문서 머리.  파일명 규칙:
