...
### 1.1 사전 점검 체크리스트 (전문가 검토 반영, 2026-09-18)

현장에서 헷갈리지 않도록 아래 항목을 체크리스트로 만들어 순차 확인한다 — 확인 방법까지 함께 지정한다.

- [ ] 모든 CCD 냉각기 Compressor 정지
- [ ] 듀어 내부 및 냉각부 온도 확인 — **도스 프로그램으로 확인**
- [ ] 내부 온도와 주변 Dew Point 비교 — **외부 온도계 이용 및 기록**
- [ ] 진공펌프, 배관 및 밸브 상태 확인 — **사진 촬영**
- [ ] 진공계 및 온도계 정상 작동 여부 확인
- [ ] 작업장 청정도 확인 — 카메라 바깥을 정전기 없는 재질로 알콜 세척
- [ ] 듀어 카트 및 장비 접지상태 확인 — **리스트스트랩 반드시 착용**
- [ ] 진공 관련 예비부품 및 안전장비 확인
...
- 가능한 경우 일반 실내 공기보다 **Dry N2**를 사용한다. 가스 공급라인에 Pressure Regulator와 Particle Filter를 적용해 압력·청정도를 관리한다.
- **단, Dry N2는 필수가 아니다** (전문가 검토, 2026-09-18) — 현장에서 준비가 어려우면 **이슬점 조건을 맞춘 뒤 천천히 개방하는 것으로 충분**하다. NASA를 비롯한 우주미션 장비 개발에서도 N2를 반드시 쓰지는 않으며, **CCD는 N2 없이도 큰 문제가 되지 않는다**. **UV 디텍터를 쓰는 경우에만 디텍터 보호를 위해 N2가 필수**다 — KMTNet 카메라는 CCD이므로 Dry N2는 "가능하면 사용" 수준의 옵션이다.
- Vent Gas Line 내부는 깨끗하고 건조하게 유지한다.
- 사용하지 않는 KF 배관과 포트는 즉시 Blank Cap으로 막는다.
- 진공 부품 작업 시 **Powder-free clean glove**를 사용하고, O-ring 및 sealing surface를 맨손으로 만지지 않는다.
- Vacuum-side 부품은 lint-free wipe와 승인된 세척제를 사용하여 청소한다.
- O-ring groove와 sealing surface에 이물질이 없는지 확인한다.
...
8. **플랜지 볼트는 토크 렌치로 규정 토크에 맞춰 체결한다** (전문가 검토, 2026-09-18).
9. **Pump-down을 어느 정도 진행한 뒤, 플랜지 체결 상태를 토크 렌치로 다시 확인·재체결한다** — 진공 하중으로 O-ring이 압착되면서 체결 토크가 변할 수 있다.
...
현재 Rev.7.1에서는 아래 값이 정량적으로 명확하지 않다. **11.2·11.4·11.5의 정량 기준은 SSO(호주) 첫 작업의 실측값을 기록해 확정하고, CTIO 적용 전에 이 문서에 반영한다** (전문가 검토, 2026-09-18).

## 11.1 Vent Gas 종류 — **확정 (2026-09-18 전문가 검토)**

- **기준: Dry N2 사용을 권장하되 필수는 아니다.** 현장에서 준비가 어려우면 이슬점 조건(듀어 내부 온도 > 주변 Dew Point)을 맞춘 뒤 천천히 개방하는 것으로 갈음한다.
- 근거: NASA 등 우주미션 장비 개발에서도 N2를 반드시 쓰지 않으며, **CCD는 N2 없이도 문제되지 않는다**. N2가 필수인 경우는 **UV 디텍터**를 쓸 때다 (KMTNet 카메라는 해당 없음).
- Dry N2를 쓰는 경우에는 공급라인에 Pressure Regulator + Particle Filter를 적용한다 (§7).
...
> **확정 방법 (2026-09-18 전문가 검토):** SSO 첫 작업에서 실제 Venting 소요시간·압력 상승 추이를 기록하고, 그 실측값을 기준으로 CTIO·SAAO에 적용할 값을 정한다.
...
> **확정 방법 (2026-09-18 전문가 검토):** SSO 첫 작업에서 위 항목을 실측·기록하고, 그 값을 Pump-down 완료 기준으로 확정해 CTIO·SAAO에 적용한다.
...
> **확정 방법 (2026-09-18 전문가 검토):** SSO 첫 작업의 실측 누설 거동을 기록해 합격 기준을 확정한다. 재펌핑 초기에는 압력 변화와 O-ring 압착 상태를 지속 관찰해 누설을 조기에 발견한다.
...
- [ ] 모든 Compressor OFF
- [ ] CCD/DMP/PT 온도 확인 (도스 프로그램)
- [ ] 모든 Dewar 내부 온도 > Dew Point (외부 온도계로 비교·기록)
- [ ] Clean Booth 정상 — 카메라 외부 정전기 없는 재질로 알콜 세척
- [ ] Dewar Cart Grounding 완료 — **리스트스트랩 착용**
- [ ] 진공펌프·배관·밸브 상태 사진 촬영
- [ ] 진공계·온도계 정상 작동 확인
- [ ] Vacuum Pump 준비
- [ ] Bellows 준비
...
- [ ] Rear Cylinder 체결 확인
- [ ] Rear Cover 체결 확인
- [ ] **플랜지 볼트 토크렌치 규정 토크 체결**
- [ ] Vacuum Pump 연결
- [ ] Pump-down 시작
- [ ] 초기 Leakage 확인
- [ ] **Pump-down 진행 후 플랜지 체결 토크 재확인(토크렌치)**
- [ ] L4 O-ring 압착 상태 확인
- [ ] Pressure 기록
- [ ] Cooler Line 6개 연결
- [ ] 목표 진공도 도달 확인
...
### 2026-09-18 전문가 검토에서 추가·확정된 사항

- 사전 점검 체크리스트의 **확인 방법 지정** — 도스 프로그램(온도), 외부 온도계 기록(Dew Point 비교), 사진 촬영(배관·밸브 상태), 알콜 세척(카메라 외부), **리스트스트랩 착용**(접지) (§1.1)
- **Dry N2는 권장이되 필수 아님** — CCD는 N2 없이도 무방, UV 디텍터에만 필수. 준비 어려우면 이슬점 조건 충족 후 저속 개방으로 갈음 (§4.1, §11.1)
- **플랜지 볼트 토크렌치 체결 + Pump-down 진행 후 토크 재확인** (§5.1)
- Venting 속도·Pump-down 완료 진공도·Leak Test 합격기준은 **SSO 첫 작업 실측값으로 확정** 후 CTIO·SAAO에 적용 (§11.2·11.4·11.5)
...
| 2026-09-21 | **2026-09-18 전문가 검토(진호 교수, 경희대) 반영** — 사전 점검 체크리스트 §1.1 신설, Dry N2 필수 여부 확정(§4.1·§11.1), 플랜지 토크렌치 체결·재확인 추가(§5.1), 정량 기준 3건은 SSO 실측 후 확정으로 명시(§11.2·11.4·11.5), 현장 체크리스트 반영(§12) |
