...
## D-015: ~~사이트는 호스트 IP 로 판정하고, 그 판정이 설정을 이긴다~~ (폐지)

> ⛔ **폐지 (운영자 확정 2026-08-24)** — 사이트 판별은 **`[node] observatory` 설정
> 한 줄**이 정본이고, **호스트 IP 판정은 쓰지 않는다.** `ics_sim/ics_sim/siteid.py`
> 와 `site_from_ip` 는 **삭제됐다** — 되살리지 말 것.
>
> **왜 뒤집혔나** — D-015 는 "잘못 배포된 ini" 를 막으려 했지만 **반대 위험**을
> 안고 있었다: NIC 이 내려가거나 낯선 대역에 붙으면 **진짜 관측 자료가 벤치 이름
> (`KMTK`)으로 저장된다.** 두 위험 중 하나를 골라야 했고 운영자가 **설정 쪽**을
> 택했다. 대신 값이 넷(`CTIO`/`SSO`/`SAAO`/`KASI`) 밖이면 **기동을 거부**한다 —
> 조용히 KASI 로 떨어뜨리지 않는다. 그래서 **배포 절차가 유일한 방어**이고,
> `operations/ICS_DEPLOYMENT_CHECKLIST.md` 의 기동 배너 확인을 건너뛰면 안 된다.
>
> 경위는 `ics_sim/DevNote.md` **11.27**. 아래 본문은 **이력으로 남긴다** — 판정을
> 다시 도입하자는 제안이 나올 때 당시 근거와 반대 근거가 함께 필요하다.

> ⚠️ D-017 (2026-08-25) 로 사이트 코드가 개정됐다 — 아래 본문의 `KMTT`(벤치)는 전부 **`KMTK`(KASI)** 로 읽는다.

날짜: 2026-08-13
관련: D-011 (사이트 코드 prefix) · D-014 (관측일) · **D-017** · `operations/ICS_DEPLOYMENT_CHECKLIST.md` · ACT-008 · ACT-009
상태: **폐지 (2026-08-24)** — 구현도 함께 삭제됐다. 종전: Accepted, `ics_sim` 구현 완료(시험 308개).
...
- **실효 사이트는 호스트 자기 IPv4 주소의 `/24` 대역으로 판정한다.**
  ...
  > ⚠️ **D-017 (2026-08-25)로 개정됨** — 떨어지는 코드가 **`KMTK`(KASI)** 다. ⛔ **판정 규칙 자체는 2026-08-24 에 폐지됐다** (위 배너).
