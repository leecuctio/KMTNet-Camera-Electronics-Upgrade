...
상태: **Accepted (Amended)** — 항목 3 의 정규화 조항은 **D-020 이 대체**했다 (2026-08-24) — raw spec v1.5 반영 완료, **구현 완료**(`rawpair.KASI_SITE` · `site_of_observatory()` — `ics-archon-v1.0-build` 브랜치, **`main` 합류 예정**).
...
- **`DAZERR` 의 성격이 계산에서 중계로 바뀐다.** ⏳ 규격 v1.12 5.7절이 아직 `ICS calculation` 으로 적고 있어 `main` 라운드에서 정정할 것 — `DSAZ`/`DSTELAZ` 행의 `TCS relay or REDIS` 에서 **`TCS relay` 를 뺄지**도 함께 (TC 가 안 보낸다는 것이 확인됐다).
