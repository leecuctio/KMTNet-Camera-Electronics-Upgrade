...
| 12a | Photometric ZP (v1.6) | WCS 성공 칩에서 aperture 측광(r=4px, annulus 8–12px, 마스크 제외) ↔ 성표 Gaia G 매칭(3px) → 2.5σ clipped median `ZPMAG` + `ZPRMS`/`ZPNSTAR`. **근사값**(Gaia G, 색항 없음): 투명도·칩간 상대 추적/QA용. GMAG 없는 성표(`make-refcat` 부트스트랩)면 no-op | refcat GMAG (로컬 Gaia 스토어/`fetch-gaia`) | 동작 |
