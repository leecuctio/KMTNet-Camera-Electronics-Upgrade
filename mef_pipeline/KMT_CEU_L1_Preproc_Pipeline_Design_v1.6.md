# KMT-CEU L0→L1 전처리 파이프라인 설계 (v1.6)
...
| 8a | Fringe 감산 (v1.6) | 필터별 master fringe(하늘 대비 비율 패턴, `calib-fringe`)를 amp별 clipped LSQ 스케일 fit 후 감산 (`SCI -= a·f`; a는 [0, 3·sky]로 제한). 템플릿 진폭 <1e-4·sky amp는 no-op. `FRNGSCL` 기록 | master fringe | 동작 |
| 8b | Illumination 보정 (v1.6) | 필터별 dark-sky 조명 response(`calib-illum`, block-median 평활·칩 중앙값=1) 나눗셈, [0.5,2] 클램프, var 전파. twilight flat과 밤하늘 조명 차 보정. `ILLUMDEV` 기록 | master illumination | 동작 |
...
| 11a | CR 플래그 (v1.6) | 단일 프레임 우주선 검출: 3×3 median Laplacian 유의도(σ≥6) + 8-이웃 ring 판정 + van Dokkum(2001) fine-structure 판정(후보 픽셀만 진짜 2D med7 gather — 분리형 근사는 언더샘플 PSF 어깨에서 오검출). 포화 주변 2px 가드, 1px 성장 → **MASK bit 64 기록만**(픽셀 불변, `CRCOUNT`; >0.2% 시 QA `cr_warning`). `--cr off` 비활성 | — | 동작 |
| 11b | Sky 모델 (v1.6) | 256px clipped-median mesh + bilinear 배경 모델. 기본 측정만(`SKYLVL`/`SKYRMS`/`SKYGRADX`/`SKYGRADY`; DIA용 하늘 보존), `--sky sub` 시 감산(`SKYSUB=T`) | — | 동작 |
| 12 | Astrometry | 5σ 별 검출(최대 800개/CCD, bloom family 정리) → 초기값: **칩별 대표 템플릿**(scale 0.3952″/px, 칩 회전, SIP3; 지향 keyword로 CRVAL 산출; 없으면 L0 근사 WCS를 0.395로 재척도) → 최다득표 구역 seed(±700px) → annulus 확장(1,500px 스텝, 12px 매칭, 2.5px 클립) → **TAN–SIP(3) fit**. 매칭·평가는 SIP 인지(`all_world2pix`). 성공: `WCSSOLVE=T`/`WCSRMS`/`WCSNMAT`; 실패: `WCSSOLVE=F`/`WCSFAIL` | refcat: **로컬 Gaia 스토어**(`--gaia-local`, 지향에서 자동 콘 추출·PM 전파) 권장, 또는 `fetch-gaia`(네트워크)/`make-refcat` + `data/astrom_template.json` | 동작 |
| 12a | Photometric ZP (v1.6, v1.7 개정) | WCS 성공 칩에서 aperture 측광(r=4px, annulus 8–12px, 마스크 제외) ↔ 성표 매칭(3px) → 2.5σ clipped median `ZPMAG` + `ZPRMS`/`ZPNSTAR`/`ZPREF`. 기준 등급(`select_zp_refs`): ① **GSPC 합성 JKC V/I**(필터 대응, 색항 불필요; RUWE≤1.4 조정형 컷 — 혼잡 시 자동 완화·기록) ② 부족/부재 시 Gaia G fallback(근사; 사유는 QA `zp_ref.note`). 등급 없는 성표(`make-refcat`)면 no-op | refcat VMAG/IMAG/RUWE (로컬 Gaia 스토어 v2 / `fetch-gaia` GSPC 조인) | 동작 |
| 13 | 헤더/출력 | provenance(L0 파일+SHA256, calib 버전, `GAINAPPL`/`XTALKAPL`/`VARINCL`/`MASKFILE`/`CRMODE`/`SKYSUB`/`WCSNSOLV`/`ZPNMEAS`), 보정 수식 COMMENT, CALHIST 17행, 임시파일→원자적 교체 | — | 동작 |
...
| master fringe (v1.6) | **필드당 1장**의 flat 보정 과학 프레임 ≥3장 (한 필터) | flat-field 후 대규모 성분(128px block-median) 제거 → 하늘 정규화 → 2.5σ clip stack. 하늘 대비 비율(`sky_fraction`) 평면. 중복 지향·필드 다양성 부족 경고 | `calib/masters.py` |
| master illumination (v1.6) | 위와 동일 입력 | flat-field 후 하늘 정규화 → 2.5σ clip stack → 128px block-median 평활 → 칩 중앙값=1 재정규화. 팽대부 항성밀도 기울기 유입 주의(여러 방향 필드 혼합 권장) | `calib/masters.py` |
...
mef_pipeline/
  ...
  kmt_ceu_preproc/
    ...
    background.py          # mesh median + bilinear (sky 모델/평활 공용)
    steps/                 # saturation, overscan, bias, dark, linearity,
                           # xtalk, gain, flat, fringe, illum, bpm, assemble,
                           # crflag, sky, photzp
    calib/                 # masters, bpm, caldb
    qa/                    # 노출별 QA JSON, batch summary markdown
    cli.py                 # calib-bias|calib-flat|calib-fringe|calib-illum|bpm|run|qa-summary
...
| 8 | **방문 간 TCS 지향 반복오차가 정밀 매칭 반경(15px≈6″)을 초과**하고, **초기 WCS 오차가 칩 내에서 ~100px 기울기**를 가짐 | 최다득표 구역 seed + annulus 확장 매칭 |
| 8b | **plate scale 실측 0.3952″/px** (명목 0.400 대비 1.3% — Gaia 16개 해에서 std 0.01 mas/px), **칩 장착 회전각** M +0.072°/K −0.358°/N −0.777°/T +0.239° (std ≤0.003°), 칩 중심 오프셋 (±0.533°, ±0.562°) | 칩별 대표 템플릿(`astrom_template.json`)으로 초기값 제공; converter `PIXSCALE`도 0.395로 갱신 (CR-002) |
| 8c | **광학 왜곡 17–54″(칩 모서리)** — 선형 TAN 불가, SIP 3차 필수. 4차는 fit 수치 불안정 | TAN–SIP(3) annulus-growth 솔버 (전 칩 rms 0.23–0.38″, median ~0.13″) |
| 8d | 011107 프레임 **T칩 readout 손상**: overscan 침수 + 별 패턴이 하늘과 비대응(translation/flip/교차 모두 배제) — 프레임 한정 이상 | `WCSFAIL=GEOM_CORRUPT_READOUT` 플래그; 타 프레임 T는 정상 solve |
...
| 2 | CR rejection 위치 | **해결(v1.6)**: L1에서 flag-only 검출(MASK bit 64), 픽셀 수정은 하지 않음 — 보간/제거는 후단 선택 |
...
| v1.4 | 2026-07-02 | Astrometry를 Gaia 실증 절차로 교체: TAN–SIP(3) annulus-growth 솔버, 칩별 대표 템플릿 초기값(scale 0.3952″/px·칩 회전·SIP, 011103–06 16개 해 평균), `fetch-gaia` 명령. plate scale 0.395 반영(CR-002) |
