...
설계 문서: [`KMT_CEU_L1_Preproc_Pipeline_Design_v1.4.md`](KMT_CEU_L1_Preproc_Pipeline_Design_v1.4.md)

## 현재 기준선

| 구분 | 값 |
| --- | --- |
| 파이프라인 | `kmt_ceu_preproc` v1.4 (순수 Python + NumPy + astropy) |
...
기준성표: `fetch-gaia`(VizieR 콘 검색, 네트워크 필요 — 사이트에서는 필드별로 미리
캐시)가 표준이며, 오프라인 대안으로 `make-refcat`(L1들에서 추출·병합; 상대 정렬)이
있다. 성표와 대상의 별 검출은 같은 마스크 정책이어야 한다(`--mask-file` pass 권장).
...
$PP fetch-gaia  --like KMTN...MK.fits -d l1_out                # Gaia DR3 기준성표(권장)
...
| 문서 | 위치 |
| --- | --- |
| 설계 문서 | `KMT_CEU_L1_Preproc_Pipeline_Design_v1.4.md` |
