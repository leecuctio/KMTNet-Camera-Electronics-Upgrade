...
설계 문서: [`KMT_CEU_L1_Preproc_Pipeline_Design_v1.3.md`](KMT_CEU_L1_Preproc_Pipeline_Design_v1.3.md)

## 현재 기준선

| 구분 | 값 |
| --- | --- |
| 파이프라인 | `kmt_ceu_preproc` v1.3 (순수 Python + NumPy + astropy) |
...
| Astrometry | `--refcat` 기준성표로 TAN 해; 성공 `WCSSOLVE=T`+`WCSRMS`, 실패 `WCSSOLVE=F`+`WCSFAIL` |
...
| 13 | Astrometry | 별 검출→기준성표 매칭→TAN fit(CD+CRPIX), WCS 키워드 갱신. 실패 시 `WCSSOLVE=F`+`WCSFAIL` 플래그 | 동작 |
...
**Astrometry**: 조립된 CCD에서 5σ 별(최대 200개/CCD, 마스크 제외)을 검출하고,
전역 지향 오프셋을 사전 추정(오프셋 히스토그램 최빈값, ±600px — TCS 반복오차 대응)한 뒤
기준성표와 매칭(15→5→3px 반복)하고 TAN 6-parameter fit `(ξ,η)=CD·(pix−CRPIX)`
(CRVAL 고정)으로 WCS를 갱신한다(`WCSSOLVE=T`, `WCSRMS`, `WCSNMAT`). 성표가 없거나
별/매칭 부족·RMS 초과 시 근사 WCS를 유지하고 `WCSSOLVE=F` + 사유(`WCSFAIL`:
NO_REFCAT/FEW_STARS/FEW_MATCHES/HIGH_RMS)를 기록한다. `make-refcat`은 여러 L1에서
별을 추출·병합(2″ 중복 제거)해 다중 필드 야간 전체를 덮는 부트스트랩 성표를 만든다
(절대 astrometry는 Gaia 등 외부 성표 필요; 부트스트랩 성표는 상대 정렬).
주의: 성표와 대상의 별 검출은 같은 마스크 정책을 써야 하므로, 부트스트랩 pass는
`--mask-file`로 돌려 `make-refcat`이 동일한(포화 제외) 검출을 하게 한다.
...
$PP make-refcat l1_out/*.ceu.l1ccd.mef.fits -d l1_out          # 부트스트랩 기준성표(병합)
...
- **Astrometry**: 야간 부트스트랩 성표(21,794개 별, 28장 병합·마스크 일관 검출)로
  28노출 × 4 CCD = **112개 해 전부 `WCSSOLVE=T`** (rms 최대 0.50″ ≈ 1.3px —
  부트스트랩 성표 기준 상대 정렬), TCS 지향 오프셋은 전역 사전 추정으로 흡수.
  성표 없이 실행 시 전 노출 `WCSSOLVE=F`/`WCSFAIL=NO_REFCAT` 플래그 확인
...
| 문서 | 위치 |
| --- | --- |
| 설계 문서 | `KMT_CEU_L1_Preproc_Pipeline_Design_v1.3.md` |
