# KMT-CEU L0→L1 전처리 파이프라인 설계 (v1.4)
...
| 12 | Astrometry | 5σ 별 검출(최대 800개/CCD, bloom family 정리) → 초기값: **칩별 대표 템플릿**(scale 0.3952″/px, 칩 회전, SIP3; 지향 keyword로 CRVAL 산출; 없으면 L0 근사 WCS를 0.395로 재척도) → 최다득표 구역 seed(±700px) → annulus 확장(1,500px 스텝, 12px 매칭, 2.5px 클립) → **TAN–SIP(3) fit**. 매칭·평가는 SIP 인지(`all_world2pix`). 성공: `WCSSOLVE=T`/`WCSRMS`/`WCSNMAT`; 실패: `WCSSOLVE=F`/`WCSFAIL` | refcat (`fetch-gaia` Gaia DR3 또는 `make-refcat`) + `data/astrom_template.json` | 동작 |
