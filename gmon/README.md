...
그래프 창은 **크기를 바꾸면 다음 렌더부터 그 크기로 꽉 차게** 그려진다.
제어부 창을 닫으면 GUI 전체가 종료되며, 이때 **세 창의 위치(그래프는
크기 포함)를 `run/gui_geometry.json`에 저장**해 다음 실행 때 같은 배치로
시작한다(데몬은 유지). GUI가 떠 있는 동안
