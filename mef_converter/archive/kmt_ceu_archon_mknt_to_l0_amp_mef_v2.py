...
def primary_cards(mk_hdr: dict, mk_path: Path, nt_path: Path, out_path: Path):
    now = dt.datetime.now(dt.UTC).replace(microsecond=0).isoformat().replace("+00:00", "")
    date_obs = str(hval(mk_hdr, "DATE-OBS", now))
    try:
        if "T" in date_obs:
            jd_obs = jd_from_datetime(dt.datetime.fromisoformat(date_obs.replace("Z", "+00:00")))
        else:
            jd_obs = jd_from_datetime(dt.datetime.now(dt.UTC))
    except Exception:
        jd_obs = jd_from_datetime(dt.datetime.now(dt.UTC))
    ...
    cards = [
        ...
        card("CREATOR", "kmt_ceu_l0amp_mknt2mef_v2.0", "MEF creation program"),
        ...
        card("PRODVER", "v2.0", "product format version"),
        ...
        card("CAMVER", "CEU-v2.0", "camera/electronics version"),
        ...
        card("PIPEVER", "kmt_ceu_l0amp_mknt2mef-v2.0", "converter version"),
...
def xtalk_rows():
    today = dt.datetime.now(dt.UTC).replace(microsecond=0).isoformat().replace("+00:00", "")
...
def convert(mk_path: Path, nt_path: Path, out_path: Path):
    ...
    with out_path.open("wb") as fout:
        ...
        fout.write(bintable_bytes("AMPINFO", amp_cols, ampinfo_rows(mk_path, nt_path), [
            ...
            card("GEOMVER", "CEU-L0AMP-v2.0", "geometry definition version"),
