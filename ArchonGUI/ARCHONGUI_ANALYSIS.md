...
| V4 | ~~**guide 시스템의 `BACKPLANE_REV`/`VERSION`**~~ | ✅ **해소(2026-09-03)** — 보유 ACF 로 닫혔다. guide 는 **두 대**이고 서로 다르다: `STA0201`/162 = REV 5(Rev F)/1.0.1252, **`STA0291`/103 = REV 7(Rev H)/1.0.1261**. ⚠️ **이 항목의 전제 자체가 틀렸다** — "guide ACF 에 `ADXCDS`/`ADXRAW` 키가 없다" 고 적었으나 **12종 전부에 `ADXCDS=0` · `ADXRAW=0` 이 있다** [확정] |
