...
| U7 | **`LOCKT` / `AUTOFETCH` / `FASTAUTOFETCH` 의 의미** | FW 이미지 문자열에만 있고 매뉴얼·GUI 어디에도 없다. `LOCKT` 은 1252 에 있다가 **1271 에서 사라졌고**, `FASTAUTOFETCH` 는 **1271 에 새로 생겼다**. ⚠️ **임의로 실행하지 말 것** | **STA 문의.** "벤더 GUI 도 안 쓴다" + 판별 이력 + ⭐ **송출 형식 `<QF%d%08X%04X%04X%08X%08X` 를 그대로 인용**해 "이것이 `FASTAUTOFETCH` 의 헤더인가, 필드 순서는 무엇인가" 로 물으면 실기에 명령을 때려 볼 이유가 없어진다(§10.1.2) | 중 |
