...
TArchonGUI::TArchonGUI(QString loadFilename, QWidget *parent) : QMainWindow(parent)
{
	...
	label = new QLabel("Netmask:");
	gl->addWidget(label, y, 0);
	leNetmask = new QLineEdit("255.255.255.0");
	gl->addWidget(leNetmask, y++, 1);
	label = new QLabel("Gateway:");
	gl->addWidget(label, y, 0);
	leGateway = new QLineEdit("10.0.0.1");
	gl->addWidget(leGateway, y++, 1);
...
//	hl->addWidget(button);
	...
	hl->addWidget(button);
	button = new QPushButton("Next Frame");
	QObject::connect(button, SIGNAL(clicked()), this, SLOT(nextFrame()));
	hl->addWidget(button);
...
void TArchonGUI::saveNiceFile()
{
	...
	QStringList sl;
	QVariantMap map;
	QVariantMap::const_iterator vit;
	QCollator collator;
	...
	for (i = 0; i < count; i++)
	{
		...
		while (vit != map.end())
		{
			// Skip the state name, as the name within each state may be stale/left over from the last configuration load
			if (vit.key() != "NAME")
				sl.append(vit.key() + "=" + vit.value().toString());
			++vit;
		}
		// Record the correct state name from the current list
		sl.append("NAME=" + lwStates->item(i)->text());
		std::sort(sl.begin(), sl.end(), collator);
		...
		if (i < mod_count)
		{
			...
			else
			{
				switch (id)
				{
					...
					case MOD_TYPE_HVYBIAS: mod_type[i]->setText("HVY Bias"); break;
				}
...
int TArchonGUI::parseUI()
{
	...
	config.insert("NETMASK", leNetmask->text());
	config.insert("GATEWAY", leGateway->text());
...
void TArchonGUI::updateUI()
{
	...
	leNetmask->setText(config.value("NETMASK", "255.255.255.0"));
	leGateway->setText(config.value("GATEWAY", "10.0.0.1"));
...
//---------------------------------------------------------------------------
void TArchonGUI::nextFrame()
{
	QFileInfo info(lastfilename);
	QDir d(info.absolutePath());
	QStringList sl(d.entryList(QStringList("*.raw")));
	int i = sl.indexOf(info.fileName());
	if (i < 0)
		return;
	i++;
	if (i >= sl.size())
		i = 0;
	openFrame(info.absolutePath() + QDir::separator() + sl.at(i));
}
//---------------------------------------------------------------------------
