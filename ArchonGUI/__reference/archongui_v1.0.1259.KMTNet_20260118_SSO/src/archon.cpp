...
int Archon::openInterface()
{
	...
	if (!socket->waitForConnected(ARCHON_TIMEOUT))
...
int Archon::closeInterface()
{
	...
	if (connected)
	{
		...
		{
			if (!socket->waitForDisconnected(ARCHON_TIMEOUT))
				socket->abort();
		}
...
int Archon::getSystem()
{
	...
	system.clear();
	interfaceFlush();
	if (interfaceCommand("SYSTEM", s, ARCHON_TIMEOUT, false))
...
int Archon::getStatus()
{
	...
	status.clear();
	interfaceFlush();
	if (interfaceCommand("STATUS", s, ARCHON_TIMEOUT, false))
	...
	for (i = 0; i < logs; i++)
	{
		if (interfaceCommand("FETCHLOG", s, ARCHON_TIMEOUT, false))
...
int Archon::getFrameStatus(bool quiet)
{
	...
	frameStatus.clear();
	interfaceFlush();
	if (interfaceCommand("FRAME", s, ARCHON_TIMEOUT, false))
...
int Archon::lockFrame(QString &param)
{
	...
	s = "LOCK" + param;
	if (interfaceCommand(s, s, ARCHON_TIMEOUT, false))
...
int Archon::flash(QStringList &params)
{
	...
	if (interfaceCommand("BIASPOLLOFF", s, ARCHON_TIMEOUT, false))
	...
	// Reenable polling
	interfaceCommand("BIASPOLLON", s, ARCHON_TIMEOUT, false);
	return 0;
...
int Archon::reboot()
{
	...
	interfaceCommand("REBOOT", s, ARCHON_TIMEOUT, false);
	return 0;
}
//---------------------------------------------------------------------------
int Archon::warmboot()
{
	...
	interfaceCommand("WARMBOOT", s, ARCHON_TIMEOUT, false);
	return 0;
}
//---------------------------------------------------------------------------
...
int Archon::flashMod(QStringList &params)
{
	...
	case MOD_TYPE_HVYBIAS:
		t = "archonhvybias"; break;
...
int Archon::fetchFrame()
{
	...
	for (chunk = 0; chunk < chunks; chunk++)
	{
		...
		if (interfaceBinaryCommand(s, p, qMin(bytes_remaining, chunk_size), ARCHON_TIMEOUT, false))
	...
	// Unlock frame buffer
	if (interfaceCommand("LOCK0", s, ARCHON_TIMEOUT, false))
...
int Archon::writeConfig()
{
	...
	interfaceFlush();
	if (interfaceCommand("POLLOFF", s, ARCHON_TIMEOUT, false))
		LOGERROR("Error disabling polling");
	if (interfaceCommand("CLEARCONFIG", s, ARCHON_TIMEOUT, false))
...
//		logMessage(s);
	...
	if (interfaceCommand("POLLON", s, ARCHON_TIMEOUT, false))
		LOGERROR("Error enabling polling");
	return 0;

error:
	interfaceCommand("POLLON", s, ARCHON_TIMEOUT, false);
...
int Archon::applyNet()
{
	...
	if (writeConfig())
		return 1;
	if (interfaceCommand("APPLYNET", s, ARCHON_TIMEOUT, false))
...
int Archon::pollOn()
{
	...
	if (interfaceCommand("BIASPOLLON", s, ARCHON_TIMEOUT, false))
...
int Archon::loadParams()
{
	...
	if (writeConfig())
		return 1;
	if (interfaceCommand("LOADPARAMS", s, ARCHON_TIMEOUT, false))
...
int Archon::loadParam(QString paramName)
{
	...
	if (writeConfig())
		return 1;
	if (interfaceCommand(QString("LOADPARAM ") + paramName, s, ARCHON_TIMEOUT, false))
...
int Archon::resetTiming()
{
	...
	if (interfaceCommand("RESETTIMING", s, ARCHON_TIMEOUT, false))
...
int Archon::holdTiming()
{
	...
	if (interfaceCommand("HOLDTIMING", s, ARCHON_TIMEOUT, false))
...
int Archon::releaseTiming()
{
	...
	if (interfaceCommand("RELEASETIMING", s, ARCHON_TIMEOUT, false))
...
int Archon::applySystem()
{
	...
	if (writeConfig())
		return 1;
	if (interfaceCommand("APPLYSYSTEM", s, ARCHON_TIMEOUT, false))
...
int Archon::applyCDS()
{
	...
	if (writeConfig())
		return 1;
	if (interfaceCommand("APPLYCDS", s, ARCHON_TIMEOUT, false))
...
int Archon::direct(QString cmd)
{
	...
	if (interfaceCommand(cmd, s, ARCHON_TIMEOUT, false))
