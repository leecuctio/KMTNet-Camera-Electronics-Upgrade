...
//
// Routines:
//   void comsoftChkSum(char *cmd) - append a checksum to a PC-TCS command
//   void parseComsoft(char *cmd) - parse a comsoft telemetry string
...
//---------------------------------------------------------------------------
//
// comsoftChkSum() - append a checksum to a PCTCS command
//
// Arguments:
//   cmd (char *): command string
//
// Description:
//   A checksum consisting of the sum of all the ascii character codes
//   in the command string needs to be added to a remote PCTCS command
//   before uploading it to the PCTCS serial port.  All characters are
//   used except \n, and a \n is added at the end as required by the
//   PCTCS.
//
//   This code is taken directly from siocln_chksum_comsoft() from the
//   CTIO pcguider code written by R. Cantarutti (CTIO staff).
//   
// Adapter: 
//   R. Pogge, OSU Astronomy Dept.
//   pogge@astronomy.ohio-state.edu
...
//---------------------------------------------------------------------------
//
// parseComsoft() - parse the ComSoft TCS telemetry string into components
//
// Arguments:
...
void
parseComsoft(pctcs_t *tcs, char *tcsstr)
{
  ...
  // what customizations were requested.  Set the #def flags in
  // pctcs.h accordingly if adding an implementation below.
  ...
  // Derive or convert data that need converting, as required by the
  // particular PC-TCS implementation we're dealing with
  ...
  // If we add parameters that need computing independent of the
  // PC-TCs system, do them here.

  // ... space for future expansion ...

  // Get the UTC time from the local clock now - don't believe the
  // PC-TCS!  True, we're taking a chance on the local clock, but
  // linux hosts are more likely to be synched to stratum-1 or
  // stratum-0 ntp time servers, whereas we know the PC-TCS being a
  // Windows 3.x system is synched to nothing.
...
/*!
  ...
  (Re)initializes the PC-TCS serial communications, clears the
  telemetry buffers, and gets the ball rolling.
...
initPCTCS(pctcs_t *tcs, char *reply)
{
  ...
  if (tcs->fd > 0) {
    ...
    // between flushes.  If we don't pause, fast modern machines can
    // hammer the port faster than it can work.
  ...
  // Set these move condition flags as "false", the next telemetry sip
  // will update them.
