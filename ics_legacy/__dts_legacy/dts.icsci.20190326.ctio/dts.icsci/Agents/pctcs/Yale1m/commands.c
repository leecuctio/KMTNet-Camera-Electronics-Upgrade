...
cmd_ports(char *args, MsgType msgtype, char *reply)
{
  ...
  if (tcs.FD < 0)
    strcpy(reply,"No PC-TCS serial port is connected.");
  else
    sprintf(reply,"TCSPort=%s",tcs.Port);
...
cmd_version(char *args, MsgType msgtype, char *reply)
{
  ...
  sprintf(reply,"PCTCSAgent Version=(%s) CompileDate=%s CompileTime=%s",
...
cmd_tcclose(char *args, MsgType msgtype, char *reply)
{
  ...
  tcs.Link = TCS_DOWN;
  sprintf(reply,"PC-TCS Link Closed");
  return CMD_OK;
}
...
cmd_setidle(char *args, MsgType msgtype, char *reply)
{
  ...
  if (strlen(args)<=0) {
    sprintf(reply,"IdleTime=%d seconds",tcs.idletime);
  }
  else {
    GetArg(args,1,argbuf);
    tcs.idletime = atoi(argbuf);
    if (tcs.idletime >= 1) {
      sprintf(reply,"IdleTimeout=%d seconds",tcs.idletime);
    }
    else {
      sprintf(reply,"Invalid comm idle timeout '%s'",argbuf);
      return CMD_ERR;
    }
  }
  return CMD_OK;
...
cmd_tcstatus(char *args, MsgType msgtype, char *reply)
{
  ...
  case TCS_UP:
...
#if defined(__Lab)
    sprintf(reply,"DATE-OBS=%s TIME-OBS=%s TIMESYS=UTC JD=%s RA=%s DEC=%s EQUINOX=%s HA=%s ST=%s SECZ=%.2f TELFOCUS=%d TCSLINK=Enabled",
    ...
    sprintf(reply,"DATE-OBS=%s TIME-OBS=%s TIMESYS=UTC JD=%s RA=%s DEC=%s EQUINOX=%s HA=%s ST=%s SECZ=%.2f TELFOCUS=%d TELTEMP=%d TCSLINK=Enabled",
            tcs.UTCDate, tcs.UTCTime, tcs.JD, tcs.RA, tcs.Dec, tcs.Equinox, tcs.HA, 
  ...
  case TCS_IDLE:
    sprintf(reply,"DATE-OBS=%s TIME-OBS=%s TIMESYS=UTC TCSLINK=Idle",
            tcs.UTCDate, tcs.UTCTime);
    break;

  default:
    sprintf(reply,"DATE-OBS=%s TIME-OBS=%s TIMESYS=UTC TCSLINK=Disabled",
            tcs.UTCDate, tcs.UTCTime);
    break;
...
cmd_tstat(char *args, MsgType msgtype, char *reply)
{
  ...
  case TCS_UP:
    sprintf(reply,"UP %s %s %s %s %s %s %s %s %s %s %s",
            tcs.UTCDate, tcs.UTCTime, tcs.JD, tcs.RA, tcs.Dec, tcs.Equinox, tcs.HA, 
            tcs.LST, tcs.SecZ, tcs.Focus, tcs.Temp);
    break;
...
cmd_sip(char *args, MsgType msgtype, char *reply)
{
  ...
  case TCS_DOWN:
    sprintf(reply,"TCSLink is DOWN, telemetry stream unavailable");
    tcs.dosip = 0;
    break;

  case TCS_IDLE:
    sprintf(reply,"TCSLink is IDLE, telemetry stream unavailable");
    tcs.dosip = 0;
    break;
...
cmd_tcscmd(char *args, MsgType msgtype, char *reply)
{
  ...
  if (tcs.Link != TCS_UP) {
    sprintf(reply,"TCSLink is IDLE/DOWN, remote commands unavailable");
    return CMD_ERR;
  }
  ...
  strcpy(tcscmd,args);
  chksum_comsoft(tcscmd);  // append checksum
  ...
  if (nsent < 0) {
    REDTEXT;
    sprintf(reply,"Cannot send command '%s' to PC-TCS port - %s",
	    args,strerror(errno));
    TXTRESET;
    return CMD_ERR;
  }
  ...
  sprintf(reply,"Sent PC-TCS Command '%s'",args);
  return CMD_OK;
...
cmd_tcsynch(char *args, MsgType msgtype, char *reply)
{
  ...
  memset(tcscmd,0,sizeof(tcscmd));
  sprintf(tcscmd,"SETDATE %.2d/%.2d/%.4d",tctime.year,tctime.month,tctime.day);
  chksum_comsoft(tcscmd);  // append checksum
