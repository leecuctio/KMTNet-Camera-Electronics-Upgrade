...
serialHandler(int serialFD) 
{
  ...
  // If we are in verbose mode, also include the interface port
  // info in the log message, otherwise just the message is passed

  if (isis.isVerbose) {
    sprintf(reply,"[%s] %s",ttyTab[iPort].devName,message);
    logMessage(reply);
    memset(reply,0,ISIS_MSGSIZE);
  }
  else
...
socketHandler(int sockFD) 
{
  ...
  // New hook: if verbose mode, also include the sender's
  // UDP socket host and port number in the log message

  if (isis.isVerbose && isis.useCLI) printf("<< %s >>\n",message);
  if (strlen(msgBody)>0) {
    if (isis.isVerbose) {
      sprintf(reply,"[%s:%d] %s",clientAddr,clientPort,message);
      logMessage(reply);
      memset(reply,0,ISIS_MSGSIZE);
    }
    else 
