/*!
  \file ttyutils.c
  \brief Generic serial port (tty) I/O handling utilties.
  ...
  servers like the Comtrol DeviceMaster RTS-x series devices.  They
  ...
  \date 2004 September 15

*/

#include "ttyutils.h" // All the header we should need
...
{ 
  ...
  if (strlen(port->Port)<=0) {
    printf("ERROR(OpenTTYPort): device name NULL!\n");
    return -1;
  }
  ...
  if (strstr(port->Port,":")>0) {
    ...
    if (!(PortHost=gethostbyname(hostID))) {
      printf("ERROR(OpenTTYPort): Cannot resolve network serial port server hostname %s - %s\n",
    ...
    if (portFD < 0) {
      printf("ERROR(OpenTTYPort): Cannot open socket for network port server %s:%d - %s\n",
    ...
    if (connect(portFD,(struct sockaddr *) &PortServer, sizeof(PortServer))<0) {
      printf("ERROR(OpenTTYPort): Cannot connect to network port server %s:%d - %s\n",
	     hostID,portID,strerror(errno));
      close(portFD);
      return -1;
    }
...
ReadTTYPort(ttyport_t *port, char *msgstr, long timeout)
{
  ...
  while (keepReading) {
	...
	sprintf(msgstr,"(ReadTTYPort) Port %s read aborted Ctrl+C",port->Port);
	return -1;
      }
	...
	// Now comes the fun bit - this could be a stream we're reading,
	// so the message could be only part of what the microstep drive
	// sent, depending on how the message was buffered.  A message
	// from the Si microstep drive is not complete unless we see \r at
	// the end.  If \r is absent, it means the message is still
	// incomplete and we have more to read.  Here we build the
	// message string sequentially using "block" reads, and test for
	// \r in the last character to tell us if we're done.  If no \r
	// appears, go back into the loop to get the rest.  In practice,
	// at most 2 reads are required, and 95% or more require only 1
	// read.  Note that if port->FD points to a line-buffered device
	// (e.g., a serial device instead of a socket stream), one read
	// does it every time.  These contortions are required because
	// Unix uses \n for line termination, while the rest of the
	// serial port world uses \r.  Not pretty...

	if (strlen(inbuf) > 0) {
	  strcat(msgstr,inbuf);  
	  lastchar = strlen(msgstr)-1;
	  // if the last char is \r or \n the string is complete, null terminate & return
