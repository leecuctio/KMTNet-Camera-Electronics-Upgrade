...
DoCommand(int port, char *cmdStr)
{
  ...
  // full list of supported commands
    ...
    XmitMsg(port, fromID, "DONE: ARCHIVE=T");
  }

  else if (strcmp(cmdWord, "-ARCHIVE")==0) {
    systab->doarchive = cb_FALSE;
    XmitMsg(port, fromID, "DONE: ARCHIVE=F");
  }
    ...
    XmitMsg(port, fromID, "DONE: AUTOLOG=T");
  }

  else if (strcmp(cmdWord, "-AUTOLOG")==0) {
    systab->doautolog = cb_FALSE;
    XmitMsg(port, fromID, "DONE: AUTOLOG=F");
  }
    ...
    XmitMsg(port, fromID, "DONE: DISPLAY=T");
  }

  else if (strcmp(cmdWord, "-DISPLAY")==0) {
    systab->dodisplay = cb_FALSE;
    XmitMsg(port, fromID, "DONE: DISPLAY=F");
  }
    ...
    XmitMsg(port, fromID, "DONE: ADDFITS=T");
  }

  else if (strcmp(cmdWord, "-ADDFITS")==0) {
    systab->addfits = cb_FALSE;
    XmitMsg(port, fromID, "DONE: ADDFITS=F");
  }
    ...
    XmitMsg(port, fromID, "DONE: SWAP=T");
    ...
    XmitMsg(port, fromID, "DONE: SWAP=F");
  }
    ...
    XmitMsg(port, fromID, "DONE: Caliban system variables restored to default values");
  }
  ...
  // n defaults to 10                                                          
    ...
    // Then we read the resulting output from the pipe - STATUS: --> DONE: [2012 Aug 2 rwp/osu]
      
    while(fgets(outStr, sizeof(outStr), logPipe) != NULL)
      XmitMsg(port, fromID, "DONE: LOG: %s", outStr);
  ...
  // we can still get data
    ...
    if (strcmp(argBuf, "MOUNT")==0)   
      ...
      XmitMsg(port, fromID, "DONE: LASTFILE=%s", systab->lastfile);
  
  }

  else if (strcmp(cmdWord, "PATH")==0)  { // what is the current path (mount point)?
      XmitMsg(port, fromID, "DONE: PATH=%s", mounttab->mount[mounttab->current]);
  ...
  // report our version and compilation info 
    ...
    XmitMsg(port, fromID, "DONE: Version=(%s) ExeName=%s UserID=%s CompilationDate=%s CompilationTime=%s", 
