...
//   2014 Apr 20 - modified for KMTNet TCS
//
//---------------------------------------------------------------------------
...
//   2014 Apr 16: modified for KMTNet PC-TCS - Telcom link [cha/kasi]
//
//---------------------------------------------------------------------------
...
parse_comsoft(pctcs_t *tcs, char *tcsstr)
{
  ...
  // actual telemetry data in the packet from Telcom of KMTNet TCS.
  ...
  tcs->UpdateFlag = 1;
      ...
      //   flag for checking execution code update after operating command
      //   set to 0 before operating command is sended in commands.c
      //   after operating command, update request command is sended in commands.c
      //   for prompt update, and then set to 1 when telemetry data is received
...
//
// Description:
//   This function is called right after parse_comsoft() was return 
//   for monitoring the telescope moving status code in the TCS telemetry data, 
//   and update moving status.
//
//   If the telescope is moving, display the telescope position on the consol
//   This function is made from original code in parse_comsoft()
//
//   the routine of this function was saparated from parse_comsoft() to 
//   handle message display and modified for KMTNet TCS Agent
//
// Author:
//   R. Pogge, OSU Astronomy Dept.
//   pogge@astronomy.ohio-state.edu
...
//
// Modification History:
//   2014 Apr 16: saparate the routine from parse_comsoft() and modified for KMTNet
//
//---------------------------------------------------------------------------
...
InitPCTCS(pctcs_t *tcs, char *reply)
{
  ...
  // Create tcp socket and connection to Telcom for the telemetry
  ...
  // Create tcp socket and connection to Telcom for command proc
...
ClearAUX(auxctrl_t *aux)
{
  ...
  // Reset AUX subsystem's telemetry data
...
ClearAuxData(auxctrl_t *aux, int subsys)
{
  if( subsys==AUX_IDX_FS || subsys==AUX_IDX_AL ) {
    ...
    aux->FS_FilterNumber               = AUX_UNKNOWN;
