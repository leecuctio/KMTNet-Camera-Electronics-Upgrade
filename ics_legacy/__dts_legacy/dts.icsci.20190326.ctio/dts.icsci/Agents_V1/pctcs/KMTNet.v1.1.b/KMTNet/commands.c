...
//   2014 May 12: modified for KMTNet TCS
//
//---------------------------------------------------------------------------
...
SocketCommand(char *buf)
{
  ...
  default:  // we don't know what we got, print for debugging purposes
    ...
    if(client.isVerbose) {
      CYATEXT;    
      if (client.useISIS) printf("\rISIS IN : Malformed message type\n");
...
cmd_help(char *args, MsgType msgtype, char *reply)
{
  if (msgtype==EXEC) {
    ...
    printf("   filter       - change filters to arg filter#, arg: <fnum>\n");
    printf("   fsastat      - query & return raw AUX Filter/Shutter status\n");
    ...
    printf("   fttstat      - query & return raw AUX Focuser/Tip-Tilt status\n");
    printf("\n");
...
cmd_tcsstatus(char *args, MsgType msgtype, char *reply)
{
  ...
  case TCS_UP:
    ...
    sprintf(reply, "DATE-OBS=%s TIME-OBS=%s TIMESYS=UTC DATE-UP=%s TIME-UP=%s"
...
//    UP DATE-OBS TIME-OBS TIMESYS DATE-UP TIME-UP 
//      RA DEC EQUINOX HA ST SECZ ALT AZ MOVESTATUS EXECODE
//
// TCS_IDLE: PC-TCS link has been idle for longer than the allowed time
//    IDLE DATE-OBS TIME-OBS TIMESYS ARCMODE
//
// TCS_DOWN: PC-TCS link is disabled ("down")
//    DOWN DATE-OBS TIME-OBS TIMESYS ARCMODE
//
...
//
//   DATE/TIME-OBS: current time recorded when this function is called
//   DATE/TIME-UP : update time recorded when the telemetry data packet was received
//   MOVESTATUS   : 0:no / 1:RA moving / 2:Dec moving / 3: Both moving
//   EXECODE      : '0' / 'e' / 'E' / '3', if cmd was executed successfully, changed e/E
//   ARCMODE      : AutoRecovery mode flag, ARC.ENABLED / ARC.DISABLED
//
...
cmd_tcmd(char *args, MsgType msgtype, char *reply)
{
  ...
  if (strlen(args)<=0) {
    ...
    strcat(reply, "  /  <tcmd>: PC-TCS commands defined in COMSOFT Native Protocol");
    return CMD_ERR;
  }
...
cmd_auxstatus(char *args, MsgType msgtype, char *reply)
{
  ...
  case AUX_UP:
    ...
    sprintf(reply, "%s FILTERSHUTTER_STATUS=%s", reply, 
                    AuxStatusArg(aux.Statuses[AUX_IDX_FS]));

    if(aux.Statuses[AUX_IDX_FS]!=AUX_STATUS_NC) {
      sprintf(reply, "%s FILTER_OPSTAT=%s FILTERNUM=%d", reply, 
                      AuxStatusArg(aux.FS_FilterOpStat), aux.FS_FilterNumber);
      sprintf(reply, "%s SHUTTER_OPSTAT=%s SHUTTER=%s", reply, 
                      AuxStatusArg(aux.FS_ShutOpStat), AuxStatusArg(aux.FS_ShutStatus));
    }

    sprintf(reply, "%s FOCUSACTUATOR_STATUS=%s", reply,
                    AuxStatusArg(aux.Statuses[AUX_IDX_FA]));

    if(aux.Statuses[AUX_IDX_FA]!=AUX_STATUS_NC) {
      sprintf(reply, "%s FA_FOCUS=%+.3f FA_TILT_NS=%+.1f FA_TILT_EW=%+.1f", reply,
                      aux.FA_Focus, aux.FA_TiltNS, aux.FA_TiltEW);
      sprintf(reply, "%s FA_SOUTH_LIMIT=%d FA_EAST_LIMIT=%d FA_WEST_LIMIT=%d", reply,
                      aux.FA_ActLims[SOUTH], aux.FA_ActLims[EAST], aux.FA_ActLims[WEST]);
      sprintf(reply, "%s FA_SOUTH_POS=%+.3f FA_EAST_POS=%+.3f FA_WEST_POS=%+.3f", reply,
    ...
    if(aux.Statuses[AUX_IDX_DS]!=AUX_STATUS_NC) {
      sprintf(reply, "%s DS_UPSHUT=%s DS_LOSHUT=%s DS_SAFSW=%s DS_ELEVATION=%.1f", 
...
//    UP DATE-OBS TIME-OBS TIMESYS DATE-UP DATA-UP 
//      FS: FSSTAT FOPSTAT FNUMBER SOPSTAT SSTATUS
//      FA: FASTAT FOCUS TILTNS TILTEW SOUTHLIM EASTLIM WESTLIM SOUTHPOS EASTPOS WESTPOS
//      DS: DSSTAT UPSHUT LOSHUT SAFSW ELEVATION
//      MC: MCSTAT MCPOS
//      CH: CHSTAT SETPOINT PROCTEMP
//      EN: ENSTAT S1 S2 S3 S4 S5 S6 S7
//
// TCS_DOWN: AUX link is disabled ("down")
//    DOWN DATE-OBS TIME-OBS TIMESYS ARCMODE
//
// Time system for Time/Date in all cases is UTC.  In the Idle/Down cases,
// the time/date returned are from the system time clock, which hopefully is
...
//   FA:      : Focuser Acutaor data start
//   FASTAT   : FA subsystem operation status (NC/STANDBY/RUNNING/ERROR)
//   FOCUS    : focus position at the center of PFI(on axis), averaged of 3 actuator pos
//   TILTNS   : North-South PFI tilt angle in arcsec, positive when N is higher than S
//   TILTEW   : East-West PFI tilt angle in arcsec, positive when E is higher than W
//   SOUTHLIM : limit status of south actuator (no:0/outer:1/inner:2/both:3)
//   EASTLIM  : limit status of east actuator
//   WESTLIM  : limit status of west actuator
//   SOUTHPOS : position of south actuator in mm
//   EASTPOS  : position of east actuator in mm
//   WESTPOS  : position of west actuator in mm
//   DS:      : Dome Shutter data start
//   DSSTAT   : DS subsystem operation status
//   UPSHUT   : upper dome shutter status (OPEN/MID/CLOSED)
//   LOSHUT   : lower dome shutter status (OPEN/CLOSED)
//   SAFSW    : safety interlock switch status (ACTIVE/INACTIVE)
//   ELEVATION: upper dome shutter elevation in deg
//   MC:      : Mirror Cover data start
//   MCSTAT   : MC subsystem operation status
//   MCPOS    : mirror cover position (0~100)
//   CH:      : Chiller for mirror cooling data start
//   CHSTAT   : CH subsystem operation status
//   SETPOINT : chiller set point temperature, in deg C
//   PROCTEMP : chiller processed temperature, in deg C
//   EN:      : Environmental system data start
//   ENSTAT   : EN subsystem operation status
//   S1 ~ S7  : Environmental sensor #1 ~ #7 data, in deg C or RH %
//   ARCMODE  : AutoRecovery mode flag (ARC.ENABLED/ARC.DISABLED)
...
cmd_astat(char *args, MsgType msgtype, char *reply)
{
  ...
  case AUX_UP:
    sprintf(reply, "UP %s %s UTC %s %s", curdate, curtime, aux.Date, aux.UTC);
    sprintf(reply, "%s  FS: %s %s %d %s %s", reply, 
                    AuxStatusArg(aux.Statuses[AUX_IDX_FS]), 
                    AuxStatusArg(aux.FS_FilterOpStat), aux.FS_FilterNumber,
                    AuxStatusArg(aux.FS_ShutOpStat), AuxStatusArg(aux.FS_ShutStatus));
    sprintf(reply, "%s  FA: %s %+.3f %+.1f %+.1f  %d %d %d  %+.3f %+.3f %+.3f", reply,
                    ...
                    aux.FA_ActLims[SOUTH], aux.FA_ActLims[EAST], aux.FA_ActLims[WEST],
                    aux.FA_ActPoss[SOUTH], aux.FA_ActPoss[EAST], aux.FA_ActPoss[WEST]);
    sprintf(reply, "%s  DS: %s %s %s %s %.1f", reply, 
                    AuxStatusArg(aux.Statuses[AUX_IDX_DS]), 
    ...
    sprintf(reply, "%s  CH: %s %.1f %.1f", reply, 
                    AuxStatusArg(aux.Statuses[AUX_IDX_CH]), 
                    aux.CH_Setpoint, aux.CH_ProcTemp);
    sprintf(reply, "%s  EN: %s", reply, AuxStatusArg(aux.Statuses[AUX_IDX_EN]));
...
//
// AUX_UP: AUX link active
//    UP FOPSTAT FNUMBER SOPSTAT SSTATUS
//
...
//
// AUX_UP: AUX link active
//    UP FASTAT FOCUS TILTNS TILTEW SOUTHLIM EASTLIM WESTLIM SOUTHPOS EASTPOS WESTPOS
//
// AUX_DOWN: AUX link is disabled ("down")
//    DOWN 
//
//   FASTAT  : focuser operation status (NC/STANDBY/RUNNING/ERROR)
//   FOCUS   : focus position at the center of PFI(on axis), averaged of 3 actuator pos
//   TILTNS  : North-South PFI tilt angle in arcsec, positive when N is higher than S
//   TILTEW  : East-West PFI tilt angle in arcsec, positive when E is higher than W
//   SOUTHLIM: limit status of south actuator (no:0/outer:1/inner:2/both:3)
...
cmd_afilter(char *args, MsgType msgtype, char *reply)
{
  ...
  if(rtn<1) {
    strcpy(reply, "usage: filter <fnum>");
    strcat(reply, "  /  <fnum>: filter number to set, 0 ~ 4 (0:no filter)");
    return CMD_ERR;
  }
  ...
  if( fnum<0 || fnum>4 ) {
    sprintf(reply, "<fnum> value is must be 0 ~ 4", fnum);
    return CMD_ERR;
  }
...
//---------------------------------------------------------------------------
//
// aux.fttgoto - goto the focus position and the tip-tilt angle, commanded 
//               by arguments (EXEC only in case tip-tilt args used)
...
//   through monitoring the Limit statuses of both SF(full shutter) & SH(half shutter)
