...
//   In the Standalone mode, TCS Agent accepts and processes commands
//   of any client that according to IMPv2 through a UDP socket. In the
//   ISIS client mode, TCS Agent interacts only ISIS that has the ISIS 
//   ID and the ISIS port defined in Runtime configuration, and also 
//   an ISIS client terminal is enabled so the local user can interacts
//   with ISIS on the console. (e.g., TC% >IS ping)
//
// Author:
...
main(int argc, char *argv[]) 
{
  ...
  timeout.tv_sec  = 0;
  timeout.tv_usec = SELECT_TIMEOUT*1000;  // SELECT_TIMEOUT in msec
  ...
  printf("- Runtime configuration loading..\n");
  ...
  if (InitAUX(&aux,reply)<0) 
    printf("- AUX ctrl tcp link init failed..\n  > %s\n\n",reply);
  else 
    printf("- AUX ctrl tcp link initialized\n\n");
  ...
  if(aux.Link==AUX_DOWN) {
    ...
    if(client.isVerbose)
      printf("        AUX link initialization failed\n");
    TXTRESET;
    if(!aux.ArcMode)
  ...
  while (client.KeepGoing) { //
    ...
    #ifdef __DEBUG
    printf("\rDEBUG: telemetry request idle = %.3f  receive idle = %.3f\n", 
            tcs.UpdateIdle, tcs.PctcsIdle);
    fflush(stdout);
    #endif
