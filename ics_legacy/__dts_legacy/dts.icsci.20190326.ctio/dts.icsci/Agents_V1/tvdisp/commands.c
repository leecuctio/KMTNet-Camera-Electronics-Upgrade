...
/*!
  \file commands.c 
  \brief AzCam Client application command interpreter functions.
...
{
  strcpy(reply,"placeholder for status...");
  return CMD_OK;
}
...
cmd_display(char *args, MsgType msgtype, char *reply)
{
  ...
  if (img.haveImage) {  // image in memory, display it
    ...
    dt = t2 - t1;
    if (client.Debug)
      printf("  xtvload() time: %.3f seconds\n",dt);
    ...
    dt = t2 - t1;
    if (client.Debug)
      printf("  xtvcolorld() time: %.3f seconds\n",dt);
...
cmd_tv(char *args, MsgType msgtype, char *reply)
{
  ...
  if (strlen(args)>0) { // have arguments to process
    ...
    dt = t2 - t1;
    if (client.Debug) 
      printf("  ReadFITSFile() time: %.3f sec\n",dt);
