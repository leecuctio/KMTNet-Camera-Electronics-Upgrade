#ifndef XISISSERVER_H
#define XISISSERVER_H

// EXEC_ISIS (XISIS) header file, stolen from ISIS
//
//
...
// 2003 January 20
//
// Last modification: 2014 Feb 18 [v2.x]
//

/*!
  \file xisisserver.h
  \brief XISIS server application main header
*/
...
#ifndef ISIS_CONFIG
#define ISIS_CONFIG  "/lhome/dts/Config/xisis.ini"   //!< ISIS config 
#endif
...
#ifndef ISIS_LOGS
#define ISIS_LOGS  "/lhome/Logs/XISIS/xisis.log"   //!< ISIS runtime log
#endif
...
#define LONG_STR_SIZE     4096  //!< size of a generic long string
#define MED_STR_SIZE      256   //!< size of a generic medium-sized string
#define BIG_STR_SIZE      2048  //!< size of a generic big string
#define SHORT_STR_SIZE    32    //!< size of a generic short string
...
//
// Parameters are loaded from the xisis.ini file
//
...
extern struct server {
  ...
  int  numRemote;                     //!< Number of remote socket connections
...
#define MAXCLIENTS 64  //!< Maximum number of client hosts 
...
#define REMOTE     3   //!< Client uses a remote network socket
...
#define MAXREMOTE 16  //!< Maximum number of remote socket connections

/*!
  \brief ISIS remote socket port table

  Contains the parameters of up to #MAXPRESET remote socket hosts
  and ports. 
*/

extern struct remoteSockets {
  char host[MED_STR_SIZE];  //!< Remotesocket host name (IP or full name)
  int  port;                //!< Remotesocket port number
} remoteTab[MAXREMOTE];
