...
loadConfig(char *cfgFile)
{
  ...
  while(fgets(inStr, CFG_BUFSIZE, iniFP)) {
	...
	if (isis.numPreset == MAXPRESET) {
      ...
      // Remote Socket Port Assignments
      //
      // usage: REMOTE  %s
      //
      
      else if (strcasecmp(keyStr, "REMOTE")==0) {
	  getArg(valStr, 1, argStr);		      
	  sprintf(remoteTab[isis.numRemote].host, argStr);
	  getArg(valStr, 2, argStr);
	  remoteTab[isis.numRemote].port = atoi(argStr);
	  isis.numRemote++;  // increment remote port counter 
      }
