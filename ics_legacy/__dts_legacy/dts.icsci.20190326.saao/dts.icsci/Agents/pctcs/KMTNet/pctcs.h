...
typedef struct pctcs_config {
  ...
  int  LimitStatus; // v1.4.1    // Limit status (No:0, RA:1, DEC:2, Horizon:3)
...
typedef struct auxctrl_config {
  ...
  char   FitsTelID[SHORT_STR_SIZE]; // Telescope ID, got from pctcs.ini temporary (v1.4.2)
                                    // and got from AUX SW at InitAUX() finally (v1.5.0 ?)
