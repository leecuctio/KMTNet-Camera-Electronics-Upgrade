...
/* CatStr Function                                                  */
/* Purpose: Utility function to concatenate three strings together  */
/* Requires: 3 strings                                              */
/* Returns: Pointer to concatenated strings                         */

char *CatStr(char *str1, char *str2, char *str3)
{
  /* This function is specifically designed for use with outgoing   */
  /* messages on a communications port, so the first two arguments  */
  /* are assumed to be source and destination host names.  As such  */
  /* a '>' character is placed between them to achieve the required */
  /* addressing format                                              */
...
/* BCmp Function                                                                          */
/* Purpose: Performs a byte-wise comparison of the first length characters of two strings */
...
/* UserCancel Function                                              */
/* Purpose: Function called when a SIGINT signal occurs (Ctrl-C)    */
/* Requires:  Nothing                                               */
/* Returns: Result code                                             */
...
void UserCancel()
{
  ...
  /* Before quitting, notify the downstream host that the registered mount points */
  /* are no longer valid, and indicate that we are going offline                  */
  ...
  endwin(); /* Restore tty to original state */
...
/* GetArg Function                                                                     */
/* Purpose: General purpose routine for parsing arguments from a command line          */
/* Requires:  The source string of arguments, an arg number and the destination buffer */
...
void GetArg(char *argstr, int argnum, char *retarg)
{
  ...
  for(i=0;argnum>1;argnum--) {  /* Seek to the beginning of the nth argument, where a space delimits args */
    while(argstr[i]!=' ') {
      i++;
      if(i>=arglen)             /* If you hit the end, there aren't at least n arguments, so return null */
	break;
    }
  ...
  /* Now we should be on the first character of the nth argument */
  /* So, begin copying it to the return buffer until the next space or end of the arg string */

  for(j=0;(i<arglen) && ((argstr[i]!=' ') || (argstr[i]=='"')) && (argstr[i]!='\n'); i++, j++) {
    retarg[j] = (argstr[i]==NUL ? ' ' : argstr[i]); /* Replace any nulls with a space */
  }
...
void ltos(char *argbuf, long num) {
  ...
  /* The algorithm used chops one digit off at a time (via mod 10) and converts it into */
  /* a character (via adding '0').  This series of characters gets written into buf,    */
  /* but in reverse order, necessitating the final for loop which reverses them back    */
