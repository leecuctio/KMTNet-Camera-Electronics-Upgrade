...
#include <errno.h>
#include <sys/time.h>
#include "/usr/src/linux/include/scsi/sg.h"  /* Generic SCSI interface header needed only for Linux */

#define POSIX

#undef  USE_ERRLIST     /* define for SunOS, undef for Solaris & Linux */

#define INI_FILE_NAME   "Caliban.ini"
#define ARCHIVE_CMD "lpr -Pbits -s %s"   /* Save-The-Bits archive command */
#define AUTOLOG_CMD "/home/kepler/dts/bin/autologger %s"   /* prototype autolog command */
...
struct st {                         /* Global system table structure                                           */
  ...
  char oldcmdline[MED_STR_SIZE];    /* Command line history buffer                                             */
...
struct dt {                         /* Global disk table structure                                             */
  ...
  char disk[MAXDISKS][21];          /* Array containing disk name in the form DISK#.BUS#.DateCreated           */
  char alias[MAXDISKS][6];          /* Array containing disk alias for use in comm with serial host e.g. DISK# */
  char device[MAXDISKS][11];        /* Array containing physical device name e.g. /dev/sd6a                    */
...
{
  char mount[10][128];              /* Array containing mount point names                                      */
