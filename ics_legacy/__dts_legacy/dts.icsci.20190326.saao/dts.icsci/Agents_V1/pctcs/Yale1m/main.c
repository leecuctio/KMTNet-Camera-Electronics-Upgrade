...
main(int argc, char *argv[]) 
{
  ...
  if (InitPCTCS(&tcs,reply)<0) {
    REDTEXT;
    printf("PCTCS serial port comm init failed - %s\n",reply);
    printf("Try again later using the \"tcinit\" command\n");
    TXTRESET;
  }
  else
    printf("PCTCS comm link initialized.\n");
  ...
  while (client.KeepGoing) {
	...
	/*
	 * old-style fgets() handler
	 *
	if (fgets(buf,ISIS_MSGSIZE,stdin) != NULL) {
	  if (buf[strlen(buf)-1]=='\n') 
	    buf[strlen(buf)-1]='\0';
	  KeyboardCommand(buf);
	}
	*/
	...
	if (FD_ISSET(tcs.FD, &read_fd)) {
	  ...
	  if (n_tcs >= MIN_TCSBUF) parse_comsoft(&tcs,tcsbuf);
