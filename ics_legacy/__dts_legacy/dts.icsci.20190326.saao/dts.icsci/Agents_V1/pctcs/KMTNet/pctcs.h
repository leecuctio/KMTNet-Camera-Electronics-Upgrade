...
//
// Defaults below are defined for the KMTNet system
// 
// Author: 
...
//   2014 Apr 02 - modified for the KMTNet TCS
//
// KMTNet TCS Notes:
...
// default TCS info update interval (in second)
...
//  if there has been no resopnse from AUX in the interval Timeout_Telcom.
...
#define DEFAULT_AUX_CSHUTT_OPTIME  10.0
#define DEFAULT_TCS_GUIDE_STEP_RA   0.00503608
#define DEFAULT_TCS_GUIDE_STEP_DEC  0.01321939

// default minimum guiding offset - if commanded offset is smaller, TCS Agent ignore cmd
...
#define SELECT_TIMEOUT   50    // waiting-input timeout for select() in msec
#define DISPLAY_DELAY     2    // in seconds
#define FOP_TIMEOUT       6.0  // filter operating timeout in seconds
#define SOP_TIMEOUT       2.0  // shutter operating timeout in seconds
...
// TCS(PC-TCS & Telcom) control structure

typedef struct pctcs_config {

  // PC-TCS Telcom server info

  int    FDtel;                 // Telcom client socket's file descriptor for telemetry
  int    FDcmd;                 // Telcom client socket's file descriptor for command
  ...
  int    Link;                  // PC-TCS & Telcom interface status
  double PctcsTick;             // time of last successful TCS data update
  double PctcsIdle;             // seconds since last successful TCS data update
  int    PctcsTimeout;          // PC-TCS link idle timeout in seconds
  double TelcomTick;            // time of last successful TCP communication
  ...
  double UpdateTick;            // time of last command for TCS telemetry data update
  double UpdateIdle;            // seconds since last command for TCS data update
  double UpdateInt;             // TCS data update command interval
...
// AUX control structure

typedef struct auxctrl_config {

  // AUX control remote commmand server info

  int    FD;                     // AUX server file descriptor for TCP socket
  ...
  char   TelID[SHORT_STR_SIZE];  // telescope ID for AUX remote commands definition
  char   SysID[SHORT_STR_SIZE];  // system ID for AUX remote commands definition
  int    Link;                   // AUX TCP interface staus
  ...
  int    Statuses[6];           // connection and operation status of each sussystem
  ...
  double FS_ShutOpTime;         // canera shutter operation time
  int    FA_Limits[3];          // focuser actuator limit status (0:no,1:out,2:in,3:both)
  double FA_Positions[3];       // focuser acturator position in millimeters
...
//            TCP connection using non-blocking connect() with a long interval
// TCS_IDLE : PC-TCS serial link idle status (no telemetry input), 
//            automatically running input check with a long interval
...
// Index for subsys, filter, shutter, focuse actuator
...
///#define AUX_IDS_DS_UP          0  // UP: Upper dome shutter
///#define AUX_IDS_DS_LO          1  // LO: Lower dome shutter
///#define AUX_IDS_DS_SA          2  // SA: Safety interlock switch
...
#define AUX_FS_SOP_ERROR      46    // error - OpTime timeout
...
// AUX update and command processes
