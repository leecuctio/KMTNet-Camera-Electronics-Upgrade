/*!
  \mainpage dataman - Interactive Image Display Client
  ...
  Usage: dataman [rcfile]

  Where: \c rcfile is an optional runtime config file to load.  

  By default, dataman uses the runtime config file defined by
  ...
  These are the interactive commands for dataman:
  <pre>
  info           - report client information
  version        - report dataman version & compile info
  ...
  This is a typical runtime config file for the dataman agent:
  \verbinclude dataman.ini
...
/*!
  \file main.c
  \brief dataman main program and I/O event handler.
*/
...
xfer_t xfer; // Image transfer configuration data structure
disp_t tv;   // Image display parameters data structure
proc_t post; // Image PostProcessing command data structure
...
main(int argc, char *argv[]) 
{
  ...
  printf("  ---------------------------------------\n");
  printf("                  dataman\n");
  printf("  Interactive Image Data Management Agent\n\n");
  ...
  if (n!=0) {
    printf("Unable to load the runtime config file...dataman aborting\n");
    exit(1);
  }
  ...
  if (client.useISIS)
    printf("Started dataman as ISIS client node %s on %s port %d\n",
	   client.ID, client.Host, client.Port);
  else
    printf("Started dataman as standalone agent %s on %s port %d\n",
	   client.ID, client.Host, client.Port);
  ...
  if (tv.FD < 0) {
    ...
    if (FakeImage(&img,tv.NX,tv.NY,0.0)<0) {
      printf("Not good, dataman aborting...\n");
      exit(2);
    }
    tv.z1 = 0.0;
    tv.z2 = 4.0;
  ...
  printf("\nData Managemetn Client Shutting Down...\n");
