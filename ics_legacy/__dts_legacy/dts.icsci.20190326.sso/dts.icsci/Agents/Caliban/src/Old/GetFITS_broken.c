...
GetFITS(int port, char *host, char *devicename, char *alias, int numfiles)
{
  ...
  for (gfcv=0;gfcv<numfiles;gfcv++) { // Loop numfiles times 
    ...
    */    
    ...
    if (systab->debug == cb_TRUE) {
      sprintf(outbuf, "Seeking to BOF at byte %d", (BLOCK_SIZE + (BLOCK_SIZE * gfcv * systab->datalng)));
      ConsoleMsg("%s", outbuf);
      ts0 = FineSysTime();
    }
    ...
    // Allocate a buffer big enough to transfer the header segment in one chunk 
    ...
    if (systab->debug == cb_TRUE) {
      sprintf(outbuf, "Seeking to byte %d (end of header/start of data)", 
    ...
    // Since size of data unit was calculable from 
    ...
    // And again we write out the last line of the 
    ...
    /*
      gettimeofday(&tp, '\0');
      after = tp.tv_sec + (tp.tv_usec/1000000.0); 
    */ 
    after = FineSysTime();
