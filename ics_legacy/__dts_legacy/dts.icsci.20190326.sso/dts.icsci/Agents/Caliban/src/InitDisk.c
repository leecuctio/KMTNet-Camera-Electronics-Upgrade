...
int 
InitDisk(int port, char *host, char *inbuf)
{
  ...
  if(systab->headlng<=0 || systab->datalng<=0) {
    XmitMsg(port, host, "%s", 
    "ERROR: Invalid FITS block parameters.  Unable to synchronize disks");
    return(SYSERR);
  }

  // Maximum number of FITS cards possible in one file

  systab->maxcards = systab->headlng * BLOCK_SIZE / 80; 

  // Indicate that successful disk synchronization has not yet occurred
  
  disktab->ackdisk = cb_FALSE; 
  
  // Loop through the available disks we can see and report them to
  // the downstream host
  ...
  // Done listing disk devices
  XmitMsg(port, host, "%s", "FOUND ALL");
