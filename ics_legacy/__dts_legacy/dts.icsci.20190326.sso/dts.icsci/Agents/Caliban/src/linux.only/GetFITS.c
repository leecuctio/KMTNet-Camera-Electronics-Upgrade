...
int GetFITS(int port, char *host, char devicename[SHORT_STR_SIZE], char alias[SHORT_STR_SIZE], int numfiles)
{
  ...
  if((ifd=open(devicename,O_RDWR))==cb_ERROR) /* Attempt to open the spool device                             */
    {
  ...
  for(gfcv=0;gfcv<numfiles;gfcv++) /* Loop numfiles times */
    {
      ...
      ConsoleMsg("Initiating file transfer iteration #%d...\n", gfcv+1);
      ...
      SGseek( (long) (BLOCK_SIZE + (BLOCK_SIZE * gfcv * systab->datalng)) );

      ConsoleMsg("Seeking to byte %d\n", (BLOCK_SIZE + (BLOCK_SIZE * gfcv * systab->datalng)));

      if((charsin = SGread(ifd, inbuf, 80))==cb_ERROR) /* Attempt to read header data */
	{
      ...
      if(invalid == cb_TRUE) /* A valid FITS header must end with END followed by 77 spaces */
	{
      ...
      if((ofd=open(filename, 2))==cb_ERROR)                /* Attempt to open output file */
	{
      ...
      else                                                 /* Primary name already exists, so use unique name */
	{
      ...
      ConsoleMsg("Created file %s\n", filename);

      if (ChkDiskSpace((char *)(mounttab->mount[mounttab->current]), (long) ((bitpix / 8) * naxis1 * naxis2)) != cb_TRUE)
	{
	  close(ofd);
	  XmitMsg(port, host, "FATAL: Disk full on mount point %s.  Select alternate mount point and re-issue USE MOUNT command.", mounttab->mount[mounttab->current]);
	  return(0);
	}
      
      SGseek( (long) (BLOCK_SIZE + (BLOCK_SIZE * gfcv * systab->datalng)) ); /* Seek to BOF */
      ConsoleMsg("Seeking to byte %d\n", (BLOCK_SIZE + (BLOCK_SIZE * gfcv * systab->datalng)));

      headbuf = malloc(headsize+1); /* Allocate a buffer big enough to transfer the header segment in one chunk */
      
      charsin = SGread(ifd, headbuf, headsize);
      charsout = write(ofd, headbuf, headsize);
      systab->headwritten += charsout;

      free(headbuf); /* Free the allocated memory */

      ConsoleMsg("Wrote %d header bytes\n", systab->headwritten);
	...
	systab->headwritten += charsout;
	ConsoleMsg("Padded header with %d bytes\n", charsout);
      }
      
      /* Seek to end of header/beginning of data */

      SGseek((long) ((systab->headlng+1) * (BLOCK_SIZE + (BLOCK_SIZE * gfcv * systab->datalng))) );

      ConsoleMsg("Seeking to byte %d\n", ((systab->headlng+1) * (BLOCK_SIZE + (BLOCK_SIZE * gfcv * systab->datalng))));
	...
	{                                                     /* bits per pixel and image size, we know how  */
	  charsin = SGread(ifd, databuf, FITS_DATA_BLOCK_SIZE); /* many blocks to read                         */
	  charsout = write(ofd, databuf, FITS_DATA_BLOCK_SIZE);
	  systab->datawritten += charsout;
	}

      ConsoleMsg("Looped %d times\n", lcv);

      ConsoleMsg("%d data bytes written in 8192-byte chunks\n", systab->datawritten);
      
      charsin = SGread(ifd, databuf, FITS_DATA_BLOCK_SIZE);          /* And again we write out the last line of the */
      ...
      ConsoleMsg("Added an extra %d bytes\n", charsout);
      ConsoleMsg("Wrote %d total data bytes\n", systab->datawritten);
	...
	ConsoleMsg("FITS_BLOCK_SIZE - (datasize%FITS_BLOCK_SIZE) = %d\n", FITS_BLOCK_SIZE - (datasize%FITS_BLOCK_SIZE));
	ConsoleMsg("lcv = %d\n", lcv);
                                                                              /* with zeroes                 */
	if (lcv<FITS_BLOCK_SIZE) {
	  charsout = write(ofd, pad, lcv);
	  systab->datawritten += charsout;
	  ConsoleMsg("Padded data with %d bytes\n", charsout);
	}
      }
