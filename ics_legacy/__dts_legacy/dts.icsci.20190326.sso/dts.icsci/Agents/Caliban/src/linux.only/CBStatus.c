...
void CBStatus(int port, char *host)
{
  ...
  sprintf(msg, "STATUS: AUTOLOG=%c DISPLAY=%c ADDFITS=%c ARCHIVE=%c", (systab->doautolog == cb_TRUE ? 'T' : 'F'), (systab->dodisplay == cb_TRUE ? 'T' : 'F'), (systab->addfits == cb_TRUE ? 'T' : 'F'), (systab->doarchive == cb_TRUE ? 'T' : 'F'));
