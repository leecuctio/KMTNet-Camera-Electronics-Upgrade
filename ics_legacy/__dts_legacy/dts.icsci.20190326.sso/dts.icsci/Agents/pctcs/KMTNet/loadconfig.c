...
LoadConfig(const char *cfgfile)
{
  ...
  client.doLogging = DEFAULT_DOLOG;       // default: runtime logging disabled
  strcpy(client.logFile,DEFAULT_LOGFILE); // default client runtime log filename

  client.isVerbose = DEFAULT_VERBOSE;     // default: not verbose (concise)
  client.Debug = DEFAULT_DEBUG;           // default: no debugging
  ...
  while(fgets(inbuf, MAXCFGLINE, cfgFP)) {
    ...
    if ((inbuf[0]!='#') && (inbuf[0]!='\n') && inbuf[0]!=NUL) {
      ...
      // TCS HW configuration - RA/Dec guide step (arcsec/encoder count?)
        ...
        if ( tcs.GuideStepRA < 0.000001 || tcs.GuideStepRA > 0.1 ) {
          ...
          printf("  >> check the RA guide step value in config file\n");
        }
      }
        ...
        if ( tcs.GuideStepDec < 0.000001 || tcs.GuideStepDec > 0.1 ) {
          ...
          printf("  >> check the Dec guide step value in config file\n");
        }
      }
        ...
        if ( tcs.GuideMinOffRA < 0.0 || tcs.GuideMinOffRA > 60.0 ) {
          printf("  Error: GUIDE_MINOFF_RA is %.4f unrecognized\n",tcs.GuideMinOffRA);
        ...
        if ( tcs.GuideMinOffDec < 0.0 || tcs.GuideMinOffDec > 60.0 ) {
          printf("  Error: GUIDE_MINOFF_DEC is %.4f unrecognized\n",tcs.GuideMinOffDec);
      ...
      // AUX HW configuration - Filter/Shutter operation time
  ...
  if(client.Debug) {
    ...
    printf("    RA GuideStep: %.8f\n", tcs.GuideStepRA           );
    printf("    DecGuideStep: %.8f\n", tcs.GuideStepDec          );
    printf("--------------------------------------------------\n");
    printf("\n"                                                  );
  }
