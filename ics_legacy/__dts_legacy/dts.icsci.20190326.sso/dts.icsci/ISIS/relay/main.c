...
<pre>
2014 Oct 5 - new application [rwp/osu]
2014 Dec 9 - added timetagging to verbose output
</pre>
	...
	if (ReadClientSocket(&client,buf)>0) {
	  ...
	  if (relay.verboseMode) {
	    if (lenBuf > 1) {
	      strcpy(bufStr,buf);
	      bufStr[lenBuf-1]='\0';
	      strcpy(timeTag,GetFineTime());
	      printf("[%s - %d] UDP>> %s >>TTY\n",timeTag,lenBuf,bufStr);
	    }
	  }
