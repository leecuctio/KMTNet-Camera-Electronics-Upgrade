...
> **2026-08-03 개정**: 로그 아카이브 **전량**(`__isislogs/`, CTIO 634일 + SAAO 273일 + SSO 206일 = 48GB, 1,113일분)을 기계적으로 스캔해 샘플 9개월치에 없던 시퀀스와 메시지를 찾아냈다. 그 결과 아래가 새로 들어가거나 정정됐다:
...
>
> 스캔 도구는 [`../ics_sim/tools/scan_legacy_logs.py`](../ics_sim/tools/scan_legacy_logs.py) 에 남겨 두어 원본 로그가 있는 컴퓨터에서 재검증할 수 있다. 로그 자체는 **저장소 바깥**(`CEU/` 직속)이라 애초에 git 대상이 아니다. 신규 시뮬레이터 구현과 그 판단 근거는 [`../ics_sim/DevNote.md`](../ics_sim/DevNote.md) 참고.
...
`__osu_legacy/memo.txt` 에 IC 이미지의 위치가 적혀 있다:
...
> **⑤ 이미지 실물을 확보했고, 안에 소스가 들어 있었다 (2026-08-04).** `__osu_legacy/IC2_KX20160323.1381_ICSci_{CTIO,SAAO}/IC2.img`, 각 8.00 GB. raw MBR + FAT32 단일 파티션이라 마운트 없이 7-Zip 으로 바로 열린다.
>
...
> **⑥ CTIO 이미지와 SAAO 이미지는 프로그램이 완전히 동일하다 (2026-08-04 대조).**
>
> 두 `IC2.img` 를 전수 비교했다. **SHA256 은 다르지만**(각각 `4EBAEDDE…`, `08122A42…`), 차이는 **전부 런타임 산물**이다.
>
> | 비교 항목 | 결과 |
> |---|---|
> | `\FREEBASI\` 소스·컴파일러 **600개 항목** | **크기·타임스탬프까지 전부 동일** |
> | `\CCDS` `\TCS` `\DOS` `\A86` `\UTIL` `\MODS` `\PCTCP` `\0ICROOT` 등 프로그램 디렉토리 | **전부 동일** (차이 0) |
> | 실행 바이너리 | 양쪽 로그 모두 `Compile Date: 03-23-2016 / Compile Time: 18:59:51` — **같은 바이너리** |
> | 다른 것 | `KMTX\LOGS\*.LOG` · `KMTS\LOGS\*.LOG`(런타임 로그), `KMTX.INI`·`KMTS.INI`(마지막 세션 상태), `BOOTLOG.*`, `CALLLOG.TXT`. `IC.BAT`·`IC.INI` 는 **내용 동일, 타임스탬프만 다름** |
>
> 공통 경로 22,254개 중 크기·날짜가 다른 것은 **89개뿐이고 그 중 82개가 로그**다. `KMTX.INI` 의 차이도 설정이 아니라 마지막 노출 상태다(CTIO `IMGTYPE=DARK OBJECT=end EXPNUM=36564` / SAAO `IMGTYPE=OBJECT OBJECT=IC160806sw1 EXPNUM=12026`) — `PAP7KX.CCD:106` 의 `CALL DefaultIniWrite()` 가 노출마다 써 넣는 값이다.
>
> **두 VM 은 공통 조상에서 갈라졌다.** 같은 날짜의 로그를 대조하면 **2016-04-11 까지는 114개가 바이트 단위로 같고, 2016-11-15 부터 82개가 전부 다르다.** 빌드가 2016-03-23 이므로, **그 빌드를 올린 직후 한쪽을 복제해 다른 장비에 배포하고 이후 독립적으로 운용**한 것으로 읽힌다. (SAAO 이미지의 2015년 로그에 `TELID=KMTC` 가 나오는 것도 이 공통 조상 때문이지, SAAO 에서 CTIO 텔레메트리를 받았다는 뜻이 아니다.)
>
> **⑦ ICS 이미지에는 사이트 정체성이 들어 있지 않다.** 소스 전체에서 `CTIO`/`SAAO`/`SSO`/`TELID` 문자열이 **한 번도 하드코딩돼 있지 않고**, `KMTX.INI` 의 프로그램 옵션은 `INSTRUMENT=ICS` · `ICHOST=ICS` · `LOGENABLE` 세 줄뿐이며 `DEFAULT7.KX` 에도 사이트 항목이 없다. `FITSCTIO.TPL`·`FITSSAAO.TPL`·`FITSSSO.TPL`·`FITSOSU.TPL` **네 사이트 템플릿이 모든 이미지에 함께 들어 있다.**
>
...
`__osu_legacy/IC2_KX20160323.1381_ICSci_CTIO/IC2.img` 를 열어 ICS 본체 소스를 확보했다. IC/ICS 는 **C 가 아니라 FreeBASIC** 으로 작성돼 있고, 디스크 이미지에 **실행파일과 소스가 함께** 들어 있다. 배포 빌드 `KX2016-03-23:1381` 은 `PAP7` 세트이며 `PAP7KX.EXE` 의 타임스탬프(2016-03-23 18:59)가 이미지 이름과 정확히 일치한다 — **로그에 찍힌 그 바이너리의 소스가 맞다.**
...
| `../../__isislogs/ISIS.ICSci.{CTIO,SAAO,SSO}.*` | **전량 검토 완료 (2026-08-03)** — CTIO 634일(28GB, 2024-01-01~2025-09-30) + SAAO 273일(11GB, 2025) + SSO 206일(8.6GB, 2024-01-01~2024-07-25) = **48GB, 1,113일분**. 3.5·5.2·5.3·5.4·5.6·6절 신규 항목의 근거. **저장소 바깥**이라 비커밋 |
...
| `../../__osu_legacy/` | 위 백업의 **원본 전량과 부속 자료**(10,291 파일 / 16.6 GB). **저장소 바깥**이라 비커밋:<br>· `dts.icsci.20190326.{ctio,saao,sso}/` — 압축 푼 원본 442 MB. 커밋본에서 뺀 `Tools/`(서드파티 109 MB/사이트) · `catalog/` · `osc/` · 빌드산출물(`.o` `.a` `.tar`) · 홈 디렉토리 dotfile(**`.ssh`/`.gnupg` 포함 — 자격증명 우려로 제외**)이 들어 있다<br>· `dts.icsci.20190326.*.zip` — 원본 압축본<br>· `memo.txt` — IC2 이미지 경로 (1.3.1③의 근거)<br>· `IC2_KX20160323.1381_ICSci_{CTIO,SAAO}/IC2.img` — **ICS VM 디스크 이미지 실물, 각 8.00 GB. 검토 완료 (2026-08-04)**. raw MBR + FAT32 이라 7-Zip 으로 마운트 없이 열린다. 안에 **IC/ICS 의 FreeBASIC 소스 전량**이 실행파일과 함께 들어 있었다(`\FREEBASI\{KMTX,KMTS,KMTG,SHARE}\`, `PAP3`~`PAP7` 세대). **1.3.1⑤ · 3.4.1 · 5.2.1 · 5.6.6절의 근거** |
